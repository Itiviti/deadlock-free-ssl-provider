...
public class DerValue {
    ...
    protected ullink.security.util.DerInputBuffer buffer;

    /**
     * The DER-encoded data of the value.
     */
    public final ullink.security.util.DerInputStream data;
    ...
    public DerValue(byte tag, byte[] data) {
        this.tag = tag;
        buffer = new ullink.security.util.DerInputBuffer(data.clone());
        length = data.length;
        this.data = new ullink.security.util.DerInputStream(buffer);
     ...
     * package private
     */
    DerValue(ullink.security.util.DerInputBuffer in) throws IOException {
        // XXX must also parse BER-encoded constructed
        // values such as sequences, sets...

        tag = (byte)in.read();
        byte lenByte = (byte)in.read();
        length = ullink.security.util.DerInputStream.getLength((lenByte & 0xff), in);
        if (length == -1) {  // indefinite length encoding found
            ullink.security.util.DerInputBuffer inbuf = in.dup();
            ...
            inbuf = new ullink.security.util.DerInputBuffer(derIn.convert(indefData));
            if (tag != inbuf.read())
                throw new IOException
                        ("Indefinite length encoding not supported");
            length = ullink.security.util.DerInputStream.getLength(inbuf);
            buffer = inbuf.dup();
            buffer.truncate(length);
            data = new ullink.security.util.DerInputStream(buffer);
    ...
    private ullink.security.util.DerInputStream init(byte stringTag, String value) throws IOException {
        String enc = null;
        ...
        length = buf.length;
        buffer = new ullink.security.util.DerInputBuffer(buf);
        ullink.security.util.DerInputStream result = new ullink.security.util.DerInputStream(buffer);
     ...
     * helper routine
     */
    private ullink.security.util.DerInputStream init(boolean fullyBuffered, InputStream in)
            throws IOException {
        ...
        if (length == -1) { // indefinite length encoding found
            ...
            if (tag != in.read())
                throw new IOException
                        ("Indefinite length encoding not supported");
            length = ullink.security.util.DerInputStream.getLength(in);
        }
        if (length == 0)
            return null;
        ...
        buffer = new ullink.security.util.DerInputBuffer(bytes);
        return new ullink.security.util.DerInputStream(buffer);
    }

    /**
     * Encode an ASN1/DER encoded datum onto a DER output stream.
     */
    public void encode(ullink.security.util.DerOutputStream out)
    throws IOException {
        out.write(tag);
    ...
    public final ullink.security.util.DerInputStream getData() {
        return data;
    }
    ...
    public ullink.security.util.ObjectIdentifier getOID() throws IOException {
        ...
        return new ullink.security.util.ObjectIdentifier(buffer);
    }
    ...
    public byte[] getOctetString() throws IOException {
        ...
        if (isConstructed()) {
            ullink.security.util.DerInputStream in = new ullink.security.util.DerInputStream(bytes);
            bytes = null;
     ...
     */
    public ullink.security.util.BitArray getUnalignedBitString() throws IOException {
        if (tag != tag_BitString)
            throw new IOException(
                "DerValue.getBitString, not a bit string " + tag);
     ...
     * @return the bit string held in this value
     */
    public ullink.security.util.BitArray getUnalignedBitString(boolean tagImplicit)
    throws IOException {
        if (!tagImplicit) {
            if (tag != tag_BitString)
                throw new IOException("DerValue.getBitString, not a bit string "
     ...
     */
    public ullink.security.util.DerInputStream toDerInputStream() throws IOException {
