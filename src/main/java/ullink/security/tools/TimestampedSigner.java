...
import ullink.security.tools.jarsigner.ContentSigner;
import ullink.security.tools.jarsigner.ContentSignerParameters;
...
public final class TimestampedSigner extends ContentSigner {
    ...
    public byte[] generateSignedData(ContentSignerParameters parameters,
        ...
        ullink.security.x509.AlgorithmId digestAlgorithmId = ullink.security.x509.AlgorithmId.get(digestAlgorithm);
        ...
        if (!(issuerName instanceof ullink.security.x509.X500Name)) {
            // must extract the original encoded form of DN for subsequent
            // name comparison checks (converting to a String and back to
            // an encoded DN could cause the types of String attribute
            // values to be changed)
            ullink.security.x509.X509CertInfo tbsCert = new
                    ullink.security.x509.X509CertInfo(signerCertificateChain[0].getTBSCertificate());
        ...
        if (omitContent) {
            contentInfo = new ullink.security.pkcs.ContentInfo(ullink.security.pkcs.ContentInfo.DATA_OID, null);
        } else {
            contentInfo = new ullink.security.pkcs.ContentInfo(content);
        }

        // Generate the timestamp token
        byte[] signature = parameters.getSignature();
        ullink.security.pkcs.SignerInfo signerInfo = null;
        if (applyTimestamp) {
            ...
            ullink.security.pkcs.PKCS9Attributes unsignedAttrs =
                new ullink.security.pkcs.PKCS9Attributes(new ullink.security.pkcs.PKCS9Attribute[]{
                    new ullink.security.pkcs.PKCS9Attribute(
                        ullink.security.pkcs.PKCS9Attribute.SIGNATURE_TIMESTAMP_TOKEN_STR,
                        tsToken)});
            signerInfo = new ullink.security.pkcs.SignerInfo((ullink.security.x509.X500Name)issuerName, serialNumber,
                digestAlgorithmId, null, ullink.security.x509.AlgorithmId.get(keyAlgorithm),
                    signature, unsignedAttrs);
        } else {
            signerInfo = new ullink.security.pkcs.SignerInfo((ullink.security.x509.X500Name)issuerName, serialNumber,
                digestAlgorithmId, ullink.security.x509.AlgorithmId.get(keyAlgorithm), signature);
        }

        ullink.security.pkcs.SignerInfo[] signerInfos = {signerInfo};
        ullink.security.x509.AlgorithmId[] algorithms = {digestAlgorithmId};

        // Create the PKCS #7 signed data message
        ullink.security.pkcs.PKCS7 p7 =
            new ullink.security.pkcs.PKCS7(algorithms, contentInfo, signerCertificateChain,
                signerInfos);
    ...
    public static String getTimestampingUrl(X509Certificate tsaCertificate) {
        ...
        try {
            ...
            ullink.security.x509.AccessDescription description;
            ullink.security.x509.GeneralName location;
            ullink.security.x509.URIName uri;
            for (int i = 0; i < derValue.length; i++) {
                description = new ullink.security.x509.AccessDescription(derValue[i]);
                if (description.getAccessMethod().equals(AD_TIMESTAMPING_Id)) {
                    location = description.getAccessLocation();
                    if (location.getType() == ullink.security.x509.GeneralNameInterface.NAME_URI) {
                        uri = (ullink.security.x509.URIName) location.getName();
    ...
    private byte[] generateTimestampToken(byte[] toBeTimestamped)
        ...
        ullink.security.timestamp.TSRequest tsQuery = new ullink.security.timestamp.TSRequest(digest, "SHA-1");
        // Generate a nonce
        ...
        ullink.security.timestamp.Timestamper tsa = new ullink.security.timestamp.HttpTimestamper(tsaUrl); // use supplied TSA
        ullink.security.timestamp.TSResponse tsReply = tsa.generateTimestamp(tsQuery);
        ...
        ullink.security.pkcs.PKCS7 tsToken = tsReply.getToken();

        ullink.security.timestamp.TimestampToken tst = new ullink.security.timestamp.TimestampToken(tsToken.getContentInfo().getData());
        if (!tst.getHashAlgorithm().equals(
                new ullink.security.x509.AlgorithmId(new ObjectIdentifier("1.3.14.3.2.26")))) {
