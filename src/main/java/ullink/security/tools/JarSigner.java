...
public class JarSigner {
        ...
        try {
            ...
            for (Enumeration<? extends ZipEntry> enum_=zipFile.entries();
                ...
                if (ze.getName().startsWith(META_INF)) {
                    ...
                    if (ullink.security.util.SignatureFileVerifier.isBlockOrSF(
            ...
            ullink.security.util.ManifestDigester manDig = new ullink.security.util.ManifestDigester(mfRawBytes);
    ...
    private boolean signatureRelated(String name) {
        ...
        if (ucName.startsWith(META_INF) &&
            ullink.security.util.SignatureFileVerifier.isBlockOrSF(ucName)) {
    ...
    void checkCertUsage(X509Certificate userCert, boolean[] bad) {
        ...
        try {
            ...
            if (netscapeEx != null) {
                ullink.security.util.DerInputStream in = new ullink.security.util.DerInputStream(netscapeEx);
                byte[] encoded = in.getOctetString();
                encoded = new ullink.security.util.DerValue(encoded).getUnalignedBitString()
                        .toByteArray();

                ullink.security.x509.NetscapeCertTypeExtension extn =
                        new ullink.security.x509.NetscapeCertTypeExtension(encoded);

                Boolean val = (Boolean)extn.get(
                        ullink.security.x509.NetscapeCertTypeExtension.OBJECT_SIGNING);
                if (!val) {
                    if (bad != null) {
                        bad[2] = true;
                    } else {
    ...
    char[] getPass(String prompt)
    {
        ...
        System.err.flush();
        try {
            char[] pass = ullink.security.util.Password.readPassword(System.in);
    ...
    private ContentSigner loadSigningMechanism(String signerClassName,
        ...
        cpString = ullink.security.util.PathList.appendPath(System.getProperty("env.class.path"), cpString);
        cpString = ullink.security.util.PathList.appendPath(System.getProperty("java.class.path"), cpString);
        cpString = ullink.security.util.PathList.appendPath(signerClassPath, cpString);
        URL[] urls = ullink.security.util.PathList.pathToURLs(cpString);
...
class SignatureFile {
    ...
    public SignatureFile(MessageDigest digests[],
                         Manifest mf,
                         ullink.security.util.ManifestDigester md,
                         String baseName,
...

    {
        ...
        ullink.security.util.ManifestDigester.Entry mde =
                md.get(ullink.security.util.ManifestDigester.MF_MAIN_ATTRS, false);
        if (mde != null) {
            for (int i=0; i < digests.length; i++) {
                mattr.putValue(digests[i].getAlgorithm() +
                        "-Digest-" + ullink.security.util.ManifestDigester.MF_MAIN_ATTRS,
        ...
        Block(SignatureFile sfg, PrivateKey privateKey, String sigalg,
            ...
            if (!(issuerName instanceof ullink.security.x509.X500Name)) {
                ...
                ullink.security.x509.X509CertInfo tbsCert = new
                        ullink.security.x509.X509CertInfo(certChain[0].getTBSCertificate());
            ...
            ullink.security.x509.AlgorithmId sigAlg = ullink.security.x509.AlgorithmId.get(signatureAlgorithm);
            ullink.security.x509.AlgorithmId digEncrAlg = ullink.security.x509.AlgorithmId.get(keyAlgorithm);
