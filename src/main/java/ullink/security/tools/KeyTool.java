...
package ullink.security.tools;

import sun.misc.BASE64Encoder;
import ullink.security.pkcs.PKCS10;
import ullink.security.provider.IdentityDatabase;
import ullink.security.provider.SystemIdentity;
import ullink.security.provider.SystemSigner;
...
import ullink.security.util.Password;
import ullink.security.util.PathList;
...
public final class KeyTool {
        ...
        throws Exception
    {
        ...
        ullink.security.x509.X500Name subject =
            new ullink.security.x509.X500Name(((X509Certificate)cert).getSubjectDN().toString());
        ullink.security.x509.X500Signer signer = new ullink.security.x509.X500Signer(signature, subject);
        ...
        throws Exception
    {
        ...
        ullink.security.x509.CertAndKeyGen keypair =
                new ullink.security.x509.CertAndKeyGen(keyAlgName, sigAlgName, providerName);


        // If DN is provided, parse it. Otherwise, prompt the user for it.
        ullink.security.x509.X500Name x500Name;
        if (dname == null) {
            x500Name = getX500Name();
        } else {
            x500Name = new ullink.security.x509.X500Name(dname);
        }
        ...
        throws Exception
    {
        ...
        ullink.security.x509.X509CertImpl certImpl = new ullink.security.x509.X509CertImpl(encoded);
        ullink.security.x509.X509CertInfo certInfo = (ullink.security.x509.X509CertInfo)certImpl.get(ullink.security.x509.X509CertImpl.NAME
                                                           + "." +
                                                           ullink.security.x509.X509CertImpl.INFO);

        // Extend its validity
        Date firstDate = getStartDate(startDate);
        Date lastDate = new Date();
        lastDate.setTime(firstDate.getTime() + validity*1000L*24L*60L*60L);
        ullink.security.x509.CertificateValidity interval = new ullink.security.x509.CertificateValidity(firstDate,
                                                               lastDate);
        certInfo.set(ullink.security.x509.X509CertInfo.VALIDITY, interval);

        // Make new serial number
        certInfo.set(ullink.security.x509.X509CertInfo.SERIAL_NUMBER, new ullink.security.x509.CertificateSerialNumber
                     ((int)(firstDate.getTime()/1000)));

        // Set owner and issuer fields
        ullink.security.x509.X500Name owner;
        if (dname == null) {
            // Get the owner name from the certificate
            owner = (ullink.security.x509.X500Name)certInfo.get(ullink.security.x509.X509CertInfo.SUBJECT + "." +
                                           ullink.security.x509.CertificateSubjectName.DN_NAME);
        } else {
            // Use the owner name specified at the command line
            owner = new ullink.security.x509.X500Name(dname);
            certInfo.set(ullink.security.x509.X509CertInfo.SUBJECT + "." +
                         ullink.security.x509.CertificateSubjectName.DN_NAME, owner);
        }
        // Make issuer same as owner (self-signed!)
        certInfo.set(ullink.security.x509.X509CertInfo.ISSUER + "." +
                     ullink.security.x509.CertificateIssuerName.DN_NAME, owner);

        // The inner and outer signature algorithms have to match.
        // The way we achieve that is really ugly, but there seems to be no
        // other solution: We first sign the cert, then retrieve the
        // outer sigalg and use it to set the inner sigalg
        ullink.security.x509.X509CertImpl newCert = new ullink.security.x509.X509CertImpl(certInfo);
        newCert.sign(privKey, sigAlgName);
        ullink.security.x509.AlgorithmId sigAlgid = (ullink.security.x509.AlgorithmId)newCert.get(ullink.security.x509.X509CertImpl.SIG_ALG);
        certInfo.set(ullink.security.x509.CertificateAlgorithmId.NAME + "." +
                     ullink.security.x509.CertificateAlgorithmId.ALGORITHM, sigAlgid);

        // first upgrade to version 3

        certInfo.set(ullink.security.x509.X509CertInfo.VERSION,
                        new ullink.security.x509.CertificateVersion(ullink.security.x509.CertificateVersion.V3));

        // Sign the new certificate
        newCert = new ullink.security.x509.X509CertImpl(certInfo);
        newCert.sign(privKey, sigAlgName);
        ...
        throws Exception
    {
        ...
        */
        ...
        int extnum = 0;
        if (cert instanceof ullink.security.x509.X509CertImpl) {
            ullink.security.x509.X509CertImpl impl = (ullink.security.x509.X509CertImpl)cert;
            ...
            if (cert.getNonCriticalExtensionOIDs() != null) {
                for (String extOID : cert.getNonCriticalExtensionOIDs()) {
                    ...
                    ullink.security.x509.Extension ext = impl.getExtension(new ObjectIdentifier(extOID));
                    if (ext != null) {
     ...
     */
    private ullink.security.x509.X500Name getX500Name() throws IOException {
        BufferedReader in;
        ...
        ullink.security.x509.X500Name name;
        ...
        do {
            ...
            country = inputString(in,
                    ...
                    country);
            name = new ullink.security.x509.X500Name(commonName, organizationalUnit, organization,
                                city, state, country);
            MessageFormat form = new MessageFormat
