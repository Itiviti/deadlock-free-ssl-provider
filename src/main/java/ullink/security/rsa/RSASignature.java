...
public abstract class RSASignature extends SignatureSpi {
    ...
    private ullink.security.rsa.RSAPadding padding;
    ...
    private void initCommon(RSAKey rsaKey, SecureRandom random)
            throws InvalidKeyException {
        resetDigest();
        int keySize = ullink.security.rsa.RSACore.getByteLength(rsaKey);
        try {
            padding = ullink.security.rsa.RSAPadding.getInstance
                (ullink.security.rsa.RSAPadding.PAD_BLOCKTYPE_1, keySize, random);
    ...
    protected byte[] engineSign() throws SignatureException {
        ...
        try {
            ...
            byte[] encrypted = ullink.security.rsa.RSACore.rsa(padded, privateKey);
            return encrypted;
    ...
    protected boolean engineVerify(byte[] sigBytes) throws SignatureException {
        byte[] digest = getDigestValue();
        try {
            byte[] decrypted = ullink.security.rsa.RSACore.rsa(sigBytes, publicKey);
