...
public final class RSAKeyFactory extends KeyFactorySpi {
    ...
    public static RSAKey toRSAKey(Key key) throws InvalidKeyException {
        if ((key instanceof ullink.security.rsa.RSAPrivateKeyImpl) ||
            (key instanceof ullink.security.rsa.RSAPrivateCrtKeyImpl) ||
    ...
    private PrivateKey translatePrivateKey(PrivateKey key)
        ...
        if (key instanceof RSAPrivateCrtKey) {
            if (key instanceof ullink.security.rsa.RSAPrivateCrtKeyImpl) {
                return key;
            }
            RSAPrivateCrtKey rsaKey = (RSAPrivateCrtKey)key;
            try {
                return new ullink.security.rsa.RSAPrivateCrtKeyImpl(
                    rsaKey.getModulus(),
                    rsaKey.getPublicExponent(),
            ...
            try {
                return new ullink.security.rsa.RSAPrivateKeyImpl(
                    rsaKey.getModulus(),
                    rsaKey.getPrivateExponent()
                );
            } catch (RuntimeException e) {
            ...
            byte[] encoded = key.getEncoded();
            return ullink.security.rsa.RSAPrivateCrtKeyImpl.newKey(encoded);
        } else {
    ...
    private PrivateKey generatePrivate(KeySpec keySpec)
        ...
        if (keySpec instanceof PKCS8EncodedKeySpec) {
            ...
            return ullink.security.rsa.RSAPrivateCrtKeyImpl.newKey(pkcsSpec.getEncoded());
