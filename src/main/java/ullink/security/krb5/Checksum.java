...
import ullink.security.krb5.internal.crypto.CksumType;
...
public class Checksum {
    ...
    public Checksum(int new_cksumType, byte[] data)
        throws KdcErrException, ullink.security.krb5.KrbCryptoException {
     ...
     */
         // KrbSafe, KrbTgsReq
    public Checksum(int new_cksumType, byte[] data,
                        ullink.security.krb5.EncryptionKey key, int usage)
        throws KdcErrException, KrbApErrException, ullink.security.krb5.KrbCryptoException {
        cksumType = new_cksumType;
        CksumType cksumEngine = CksumType.getInstance(cksumType);
        if (!cksumEngine.isSafe())
     ...
     */
    public boolean verifyKeyedChecksum(byte[] data, ullink.security.krb5.EncryptionKey key,
                                        int usage)
        throws KdcErrException, KrbApErrException, ullink.security.krb5.KrbCryptoException {
        CksumType cksumEngine = CksumType.getInstance(cksumType);
        if (!cksumEngine.isSafe())
    ...
    */
     ...
     *
     */
    private Checksum(DerValue encoding) throws ullink.security.krb5.Asn1Exception, IOException {
        DerValue der;
     ...
     * @exception ullink.security.krb5.Asn1Exception if an error occurs while decoding an
     * ASN1 encoded data.
