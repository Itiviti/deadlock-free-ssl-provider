...
rs.
    ...
    */
    public static ullink.security.krb5.Credentials acquireServiceCreds(
                String service, ullink.security.krb5.Credentials ccreds)
    throws ullink.security.krb5.KrbException, IOException {
        ullink.security.krb5.ServiceName sname = new ullink.security.krb5.ServiceName(service);
        String serviceRealm = sname.getRealmString();
        String localRealm = ccreds.getClient().getRealmString();
        String defaultRealm = ullink.security.krb5.Config.getInstance().getDefaultRealm();

        if (localRealm == null) {
            ullink.security.krb5.PrincipalName temp = null;
        ...
        */
        ...
        String[] realms = ullink.security.krb5.Realm.getRealmsList(localRealm, serviceRealm);
        ...
        int i = 0, k = 0;
        ullink.security.krb5.Credentials cTgt = null, newTgt = null, theTgt = null;
        ullink.security.krb5.ServiceName tempService = null;
        String realm = null, newTgtRealm = null, theTgtRealm = null;

        for (cTgt = ccreds, i = 0; i < realms.length;)
        {
            tempService = new ullink.security.krb5.ServiceName(ullink.security.krb5.PrincipalName.TGS_DEFAULT_SRV_NAME,
            ...
            if (newTgt == null)
            {
                     ...
                     newTgt == null && k > i; k--)
                {
                    ...
                    tempService = new ullink.security.krb5.ServiceName(
                                       ullink.security.krb5.PrincipalName.TGS_DEFAULT_SRV_NAME,
                                       realms[k], realms[i]);
                    if (DEBUG)
                    {
        ...
        ullink.security.krb5.Credentials theCreds = null;
        ...
        if (theCreds != null)
        {
            if (DEBUG)
            {
                ...
                ullink.security.krb5.Credentials.printDebug(theCreds);
            }
            return theCreds;
        }
        throw new KrbApErrException(ullink.security.krb5.internal.Krb5.KRB_AP_ERR_GEN_CRED,
                                    "No service creds");
    }
    ...
    */
    private static ullink.security.krb5.Credentials serviceCreds(
            ullink.security.krb5.ServiceName service, ullink.security.krb5.Credentials ccreds)
            throws ullink.security.krb5.KrbException, IOException {
        return new ullink.security.krb5.KrbTgsReq(ccreds, service).sendAndGetCreds();
    }
}
