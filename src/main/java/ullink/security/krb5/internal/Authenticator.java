...
public class Authenticator {
    ...
    ullink.security.krb5.Checksum cksum; //optional
    public int cusec;
    public ullink.security.krb5.internal.KerberosTime ctime;
    ullink.security.krb5.EncryptionKey subKey; //optional
    Integer seqNumber; //optional
    public ullink.security.krb5.internal.AuthorizationData authorizationData; //optional

        public Authenticator (
                ullink.security.krb5.Realm new_crealm,
                ullink.security.krb5.PrincipalName new_cname,
                ullink.security.krb5.Checksum new_cksum,
                int new_cusec,
                ullink.security.krb5.internal.KerberosTime new_ctime,
                ullink.security.krb5.EncryptionKey new_subKey,
                Integer new_seqNumber,
                ullink.security.krb5.internal.AuthorizationData new_authorizationData
        ) {
                authenticator_vno = ullink.security.krb5.internal.Krb5.AUTHNETICATOR_VNO;
                crealm = new_crealm;
                cname = new_cname;
                cksum = new_cksum;
                cusec = new_cusec;
        ...
        public Authenticator(byte[] data)
                throws ullink.security.krb5.Asn1Exception, IOException, KrbApErrException, ullink.security.krb5.RealmException {
        ...
        authenticator_vno = subDer.getData().getBigInteger().intValue();
            ...
            throw new KrbApErrException(ullink.security.krb5.internal.Krb5.KRB_AP_ERR_BADVERSION);
                crealm = ullink.security.krb5.Realm.parse(der.getData(), (byte) 0x01, false);
                cname = ullink.security.krb5.PrincipalName.parse(der.getData(), (byte) 0x02, false);
                cksum = ullink.security.krb5.Checksum.parse(der.getData(), (byte) 0x03, true);
                subDer = der.getData().getDerValue();
                ...
                ctime = ullink.security.krb5.internal.KerberosTime.parse(der.getData(), (byte) 0x05, false);
                if (der.getData().available() > 0) {
                        subKey = ullink.security.krb5.EncryptionKey.parse(der.getData(), (byte) 0x06, true);
                }
                else {
                        subKey = null;
                        seqNumber = null;
                ...
                if (der.getData().available() > 0) {
                        authorizationData = ullink.security.krb5.internal.AuthorizationData.parse(der.getData(), (byte) 0x08, true);
                }
    ...
    public final ullink.security.krb5.Checksum getChecksum() {
        return cksum;
    }
    ...
    public final ullink.security.krb5.EncryptionKey getSubKey() {
        return subKey;
    }
