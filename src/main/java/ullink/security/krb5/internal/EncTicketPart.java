...
public class EncTicketPart {
        ...
        public ullink.security.krb5.internal.TransitedEncoding transited;
        ...
        public EncTicketPart(
                ...
                ullink.security.krb5.internal.TransitedEncoding new_transited,
        ...
        public EncTicketPart(byte[] data)
                throws ullink.security.krb5.Asn1Exception, ullink.security.krb5.KrbException, IOException {
    ...
    */
    ...
    private static String getHexBytes(byte[] bytes, int len)
        ...
        private void init(DerValue encoding)
            ...
            key = ullink.security.krb5.EncryptionKey.parse(der.getData(), (byte) 0x01, false);
            crealm = ullink.security.krb5.Realm.parse(der.getData(), (byte) 0x02, false);
            cname = ullink.security.krb5.PrincipalName.parse(der.getData(), (byte) 0x03, false);
            transited = ullink.security.krb5.internal.TransitedEncoding.parse(der.getData(), (byte) 0x04, false);
            ...
            if (der.getData().available() > 0) {
                caddr = ullink.security.krb5.internal.HostAddresses.parse(der.getData(), (byte) 0x09, true);
            }
            if (der.getData().available() > 0) {
                authorizationData = ullink.security.krb5.internal.AuthorizationData.parse(der.getData(), (byte) 0x0A, true);
            }
            if (der.getData().available() > 0)
                throw new ullink.security.krb5.Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
