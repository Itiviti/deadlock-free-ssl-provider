...
package ullink.security.krb5.internal.crypto;

import ullink.security.krb5.internal.KdcErrException;
import ullink.security.krb5.internal.Krb5;
import ullink.security.krb5.internal.KrbApErrException;
...
public abstract class EType {
    ...
    public static EType getInstance  (int eTypeConst)
        ...
        switch (eTypeConst) {
        case ullink.security.krb5.EncryptedData.ETYPE_NULL:
            eType = new NullEType();
            eTypeName = "NullEType";
            break;
        case ullink.security.krb5.EncryptedData.ETYPE_DES_CBC_CRC:
            eType = new ullink.security.krb5.internal.crypto.DesCbcCrcEType();
            eTypeName = "DesCbcCrcEType";
            break;
        case ullink.security.krb5.EncryptedData.ETYPE_DES_CBC_MD5:
            eType = new ullink.security.krb5.internal.crypto.DesCbcMd5EType();
            eTypeName = "DesCbcMd5EType";
            break;

        case ullink.security.krb5.EncryptedData.ETYPE_DES3_CBC_HMAC_SHA1_KD:
            eType = new ullink.security.krb5.internal.crypto.Des3CbcHmacSha1KdEType();
            eTypeName =
                "Des3CbcHmacSha1KdEType";
            break;

        case ullink.security.krb5.EncryptedData.ETYPE_AES128_CTS_HMAC_SHA1_96:
            eType = new ullink.security.krb5.internal.crypto.Aes128CtsHmacSha1EType();
            eTypeName =
                "Aes128CtsHmacSha1EType";
            break;

        case ullink.security.krb5.EncryptedData.ETYPE_AES256_CTS_HMAC_SHA1_96:
            eType = new ullink.security.krb5.internal.crypto.Aes256CtsHmacSha1EType();
            eTypeName =
                "Aes256CtsHmacSha1EType";
            break;

        case ullink.security.krb5.EncryptedData.ETYPE_ARCFOUR_HMAC:
            eType = new ullink.security.krb5.internal.crypto.ArcFourHmacEType();
            eTypeName = "ArcFourHmacEType";
            break;
    ...
    public abstract byte[] encrypt(byte[] data, byte[] key, int usage)
        throws ullink.security.krb5.KrbCryptoException;

    public abstract byte[] encrypt(byte[] data, byte[] key, byte[] ivec,
        int usage) throws ullink.security.krb5.KrbCryptoException;

    public abstract byte[] decrypt(byte[] cipher, byte[] key, int usage)
        throws KrbApErrException, ullink.security.krb5.KrbCryptoException;

    public abstract byte[] decrypt(byte[] cipher, byte[] key, byte[] ivec,
        int usage) throws KrbApErrException, ullink.security.krb5.KrbCryptoException;
    ...
    private static final int[] BUILTIN_ETYPES = new int[] {
        ullink.security.krb5.EncryptedData.ETYPE_DES_CBC_MD5,
        ullink.security.krb5.EncryptedData.ETYPE_DES_CBC_CRC,
        ullink.security.krb5.EncryptedData.ETYPE_ARCFOUR_HMAC,
        ullink.security.krb5.EncryptedData.ETYPE_DES3_CBC_HMAC_SHA1_KD,
        ullink.security.krb5.EncryptedData.ETYPE_AES128_CTS_HMAC_SHA1_96,
        ullink.security.krb5.EncryptedData.ETYPE_AES256_CTS_HMAC_SHA1_96,
    };
    ...
    public static int[] getDefaults(String configName) {
        try {
            return ullink.security.krb5.Config.getInstance().defaultEtype(configName);
        } catch (ullink.security.krb5.KrbException exc) {
            if (DEBUG) {
     ...
     * we have keys.
     */
    public static int[] getDefaults(String configName, ullink.security.krb5.EncryptionKey[] keys)
        throws ullink.security.krb5.KrbException {
        int[] answer = getDefaults(configName);
        if (answer == null) {
            throw new ullink.security.krb5.KrbException("No supported encryption types listed in "
                + configName);
        }

        List<Integer> list = new ArrayList<Integer> (answer.length);
        for (int i = 0; i < answer.length; i++) {
            if (ullink.security.krb5.EncryptionKey.findKey(answer[i], keys) != null) {
                list.add(answer[i]);
            }
        }
        int len = list.size();
