...
public final class Aes256CtsHmacSha1EType extends EType {
    ...
    public int checksumSize() {
        return ullink.security.krb5.internal.crypto.Aes256.getChecksumLength();
    }
    ...
    public byte[] encrypt(byte[] data, byte[] key, byte[] ivec, int usage)
        throws KrbCryptoException {
        try {
            return ullink.security.krb5.internal.crypto.Aes256.encrypt(key, usage, ivec, data, 0, data.length);
    ...
    public byte[] decrypt(byte[] cipher, byte[] key, byte[] ivec, int usage)
        throws KrbApErrException, KrbCryptoException {
        try {
            return ullink.security.krb5.internal.crypto.Aes256.decrypt(key, usage, ivec, cipher, 0, cipher.length);
        } catch (GeneralSecurityException e) {
            KrbCryptoException ke = new KrbCryptoException(e.getMessage());
            ke.initCause(e);
            throw ke;
