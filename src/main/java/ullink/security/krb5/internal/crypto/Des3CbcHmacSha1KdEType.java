...
public final class Des3CbcHmacSha1KdEType extends EType {
    ...
    public int checksumSize() {
        return ullink.security.krb5.internal.crypto.Des3.getChecksumLength();
    }
    ...
    public byte[] encrypt(byte[] data, byte[] key, byte[] ivec, int usage)
        throws KrbCryptoException {
        try {
            return ullink.security.krb5.internal.crypto.Des3.encrypt(key, usage, ivec, data, 0, data.length);
    ...
    public byte[] decrypt(byte[] cipher, byte[] key, byte[] ivec, int usage)
        throws KrbApErrException, KrbCryptoException {
        try {
            return ullink.security.krb5.internal.crypto.Des3.decrypt(key, usage, ivec, cipher, 0, cipher.length);
        } catch (GeneralSecurityException e) {
            KrbCryptoException ke = new KrbCryptoException(e.getMessage());
            ke.initCause(e);
            throw ke;
