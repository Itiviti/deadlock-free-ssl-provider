...
public class KRBCred {
        public ullink.security.krb5.internal.Ticket[] tickets = null;
        public EncryptedData encPart;

        private int pvno;
        private int msgType;

        public KRBCred(ullink.security.krb5.internal.Ticket[] new_tickets, EncryptedData new_encPart) throws IOException {
                pvno = ullink.security.krb5.internal.Krb5.PVNO;
                msgType = ullink.security.krb5.internal.Krb5.KRB_CRED;
                if (new_tickets != null) {
                    tickets = new ullink.security.krb5.internal.Ticket[new_tickets.length];
                    for (int i = 0; i < new_tickets.length; i++) {
                        if (new_tickets[i] == null) {
                            throw new IOException("Cannot create a KRBCred");
                        } else {
                            tickets[i] = (ullink.security.krb5.internal.Ticket)new_tickets[i].clone();
                        }
                    }
                }
                encPart = new_encPart;
        ...
        if (((encoding.getTag() & (byte)0x1F) != (byte)0x16)
                        ...
                        throw new Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
        DerValue der, subDer;
            ...
            if (msgType != ullink.security.krb5.internal.Krb5.KRB_CRED)
                throw new KrbApErrException(ullink.security.krb5.internal.Krb5.KRB_AP_ERR_MSG_TYPE);
                }
                else
                        throw new Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
        subDer = der.getData().getDerValue();
            ...
            Vector<ullink.security.krb5.internal.Ticket> v = new Vector<ullink.security.krb5.internal.Ticket> ();
            while (subsubDer.getData().available() > 0) {
                                v.addElement(new ullink.security.krb5.internal.Ticket(subsubDer.getData().getDerValue()));
                        }
            if (v.size() > 0) {
                                tickets = new ullink.security.krb5.internal.Ticket[v.size()];
                                v.copyInto(tickets);
                        }
                }
                else
