...
public class KRBSafe {
    public int pvno;
    public int msgType;
    public ullink.security.krb5.internal.KRBSafeBody safeBody;
    public Checksum cksum;

    public KRBSafe(ullink.security.krb5.internal.KRBSafeBody new_safeBody, Checksum new_cksum) {
        pvno = ullink.security.krb5.internal.Krb5.PVNO;
        msgType = ullink.security.krb5.internal.Krb5.KRB_SAFE;
    ...
    RealmException, KrbApErrException, IOException {
        ...
        if ((subDer.getTag() & 0x1F) == 0x01) {
            ...
            if (msgType != ullink.security.krb5.internal.Krb5.KRB_SAFE)
                throw new KrbApErrException(ullink.security.krb5.internal.Krb5.KRB_AP_ERR_MSG_TYPE);
        }

        else
            throw new Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
        safeBody = ullink.security.krb5.internal.KRBSafeBody.parse(der.getData(), (byte) 0x02, false);
