 ...
 * @author Yanni Zhang
 */
public class CacheTable extends Hashtable<String, ullink.security.krb5.internal.rcache.ReplayCache> {
    ...
    public synchronized void put(String principal, AuthTime time, long currTime) {
        ullink.security.krb5.internal.rcache.ReplayCache rc = super.get(principal);
        if (rc == null) {
            if (DEBUG) {
                System.out.println("replay cache for " + principal + " is null.");
            }
            rc = new ullink.security.krb5.internal.rcache.ReplayCache(principal, this);
