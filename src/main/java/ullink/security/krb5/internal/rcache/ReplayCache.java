...
package ullink.security.krb5.internal.rcache;
 ...
 * @author Yanni Zhang
 */
public class ReplayCache extends LinkedList<ullink.security.krb5.internal.rcache.AuthTime> {
     ...
     */
    public synchronized void put(ullink.security.krb5.internal.rcache.AuthTime t, long currentTime) {

        if (this.size() == 0) {
            addFirst(t);
        }
        else {
            ullink.security.krb5.internal.rcache.AuthTime temp = getFirst();
            if (temp.kerberosTime < t.kerberosTime) {
                ...
                ListIterator<ullink.security.krb5.internal.rcache.AuthTime> it = listIterator(1);
                while (it.hasNext()) {
                    temp = it.next();
                    if (temp.kerberosTime < t.kerberosTime) {
                        add(indexOf(temp), t);
        ...
        ListIterator<ullink.security.krb5.internal.rcache.AuthTime> it = listIterator(0);
        ullink.security.krb5.internal.rcache.AuthTime temp = null;
        int index = -1;
        while (it.hasNext()) {
    ...
    private void printList() {
        ...
        for (int i = 0; i < total.length; i++) {
            System.out.println("object " + i + ": " + ((ullink.security.krb5.internal.rcache.AuthTime)total[i]).kerberosTime + "/"
                               + ((ullink.security.krb5.internal.rcache.AuthTime)total[i]).cusec);
        }
    }
