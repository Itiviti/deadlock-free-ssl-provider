...
public class KDCReq {
        public ullink.security.krb5.internal.KDCReqBody reqBody;

        private int pvno;
        private int msgType;
        private ullink.security.krb5.internal.PAData[] pAData = null; //optional

        public KDCReq(ullink.security.krb5.internal.PAData[] new_pAData, ullink.security.krb5.internal.KDCReqBody new_reqBody,
    ...
    */
    protected void init(DerValue encoding, int req_type) throws Asn1Exception,
            ...
            if (this.pvno != ullink.security.krb5.internal.Krb5.PVNO)
            ...
            if (this.msgType != req_type)
                ...
                if ((subDer.getTag() & 0x01F) == 0x03) {
                        ...
                        Vector<ullink.security.krb5.internal.PAData> v = new Vector<ullink.security.krb5.internal.PAData> ();
                        while (subsubDer.getData().available() > 0) {
                                v.addElement(new ullink.security.krb5.internal.PAData(subsubDer.getData().getDerValue()));
                        }
                        if (v.size() > 0) {
                                pAData = new ullink.security.krb5.internal.PAData[v.size()];
                                v.copyInto(pAData);
                        }
                }
                else pAData = null;
                ...
                if ((subDer.getTag() & 0x01F) == 0x04) {
                        DerValue subsubDer = subDer.getData().getDerValue();
                        reqBody = new ullink.security.krb5.internal.KDCReqBody(subsubDer, msgType);
                }
                else
                        throw new Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
        }
