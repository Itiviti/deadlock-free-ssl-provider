...
public class KRBError implements java.io.Serializable {
    ...
    private int pvno;
    private int msgType;
    private ullink.security.krb5.internal.KerberosTime cTime; //optional
    private Integer cuSec; //optional
    private ullink.security.krb5.internal.KerberosTime sTime;
    private Integer suSec;
    private int errorCode;
    private ullink.security.krb5.Realm crealm; //optional
    private ullink.security.krb5.PrincipalName cname; //optional
    private ullink.security.krb5.Realm realm;
    private ullink.security.krb5.PrincipalName sname;
    private String eText; //optional
    private byte[] eData; //optional
    private ullink.security.krb5.Checksum eCksum; //optional
    ...
    public KRBError(
                    ...
                    ullink.security.krb5.internal.KerberosTime new_cTime,
                    Integer new_cuSec,
                    ullink.security.krb5.internal.KerberosTime new_sTime,
                    Integer new_suSec,
                    int new_errorCode,
                    ullink.security.krb5.Realm new_crealm,
                    ullink.security.krb5.PrincipalName new_cname,
                    ullink.security.krb5.Realm new_realm,
                    ullink.security.krb5.PrincipalName new_sname,
                    String new_eText,
                    byte[] new_eData
                        ) throws IOException, ullink.security.krb5.Asn1Exception {
        pvno = ullink.security.krb5.internal.Krb5.PVNO;
        msgType = ullink.security.krb5.internal.Krb5.KRB_ERROR;
        cTime = new_cTime;
        cuSec = new_cuSec;
        sTime = new_sTime;
        suSec = new_suSec;
    ...
    public KRBError(
                    ...
                    String new_eText,
                    byte[] new_eData,
                    ullink.security.krb5.Checksum new_eCksum
                        ) throws IOException, ullink.security.krb5.Asn1Exception {
        pvno = ullink.security.krb5.internal.Krb5.PVNO;
        msgType = ullink.security.krb5.internal.Krb5.KRB_ERROR;
        cTime = new_cTime;
    ...
    public KRBError(byte[] data) throws ullink.security.krb5.Asn1Exception,
            ullink.security.krb5.RealmException, ullink.security.krb5.internal.KrbApErrException, IOException {
        init(new DerValue(data));
        parseEData(eData);
    }

    public KRBError(DerValue encoding) throws ullink.security.krb5.Asn1Exception,
            ullink.security.krb5.RealmException, ullink.security.krb5.internal.KrbApErrException, IOException {
        init(encoding);
        showDebug();
        parseEData(eData);
    ...
    private void parseEData(byte[] data) throws IOException {
        ...
        if (errorCode == ullink.security.krb5.internal.Krb5.KDC_ERR_PREAUTH_REQUIRED
                || errorCode == ullink.security.krb5.internal.Krb5.KDC_ERR_PREAUTH_FAILED) {
            try {
    ...
    private void parsePAData(byte[] data)
        ...
        while (derPA.data.available() > 0) {
            ...
            ullink.security.krb5.internal.PAData pa_data = new ullink.security.krb5.internal.PAData(tmp);
            ...
            switch(pa_type) {
                case ullink.security.krb5.internal.Krb5.PA_ENC_TIMESTAMP:
                    if (DEBUG) {
                        System.out.println("\t PA-ENC-TIMESTAMP");
                    }
                    break;
                case ullink.security.krb5.internal.Krb5.PA_ETYPE_INFO:
                    if (pa_value != null) {
                        DerValue der = new DerValue(pa_value);
                        DerValue value = der.data.getDerValue();
                        ullink.security.krb5.internal.ETypeInfo info = new ullink.security.krb5.internal.ETypeInfo(value);
                        etype = info.getEType();
                        salt = info.getSalt();
                        if (DEBUG) {
                    ...
                    break;
                case ullink.security.krb5.internal.Krb5.PA_ETYPE_INFO2:
                    if (pa_value != null) {
                        DerValue der = new DerValue(pa_value);
                        DerValue value = der.data.getDerValue();
                        ullink.security.krb5.internal.ETypeInfo2 info2 = new ullink.security.krb5.internal.ETypeInfo2(value);
    ...
    public final ullink.security.krb5.internal.KerberosTime getServerTime() {
        return sTime;
    }

    public final ullink.security.krb5.internal.KerberosTime getClientTime() {
        return cTime;
    }
     ...
     * @exception ullink.security.krb5.internal.KrbApErrException if the value read from the DER-encoded data
    ...
    private void init(DerValue encoding) throws ullink.security.krb5.Asn1Exception,
        ...
        if ((subDer.getTag() & (byte)0x1F) == (byte)0x01) {
            ...
            if (msgType != ullink.security.krb5.internal.Krb5.KRB_ERROR) {
        ...
        cTime = ullink.security.krb5.internal.KerberosTime.parse(der.getData(), (byte) 0x02, true);
        if ((der.getData().peekByte() & 0x1F) == 0x03) {
            subDer = der.getData().getDerValue();
            cuSec = new Integer(subDer.getData().getBigInteger().intValue());
        }
        else cuSec = null;
        sTime = ullink.security.krb5.internal.KerberosTime.parse(der.getData(), (byte) 0x04, false);
        subDer = der.getData().getDerValue();
        if ((subDer.getTag() & (byte)0x1F) == (byte)0x05) {
            suSec = new Integer (subDer.getData().getBigInteger().intValue());
        }
        else  throw new ullink.security.krb5.Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
        subDer = der.getData().getDerValue();
        if ((subDer.getTag() & (byte)0x1F) == (byte)0x06) {
            errorCode = subDer.getData().getBigInteger().intValue();
        }
        else  throw new ullink.security.krb5.Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
        crealm = ullink.security.krb5.Realm.parse(der.getData(), (byte) 0x07, true);
        cname = ullink.security.krb5.PrincipalName.parse(der.getData(), (byte) 0x08, true);
        realm = ullink.security.krb5.Realm.parse(der.getData(), (byte) 0x09, false);
        sname = ullink.security.krb5.PrincipalName.parse(der.getData(), (byte) 0x0A, false);
        eText = null;
        eData = null;
        eCksum = null;
        ...
        if (der.getData().available() >0) {
            eCksum = ullink.security.krb5.Checksum.parse(der.getData(), (byte) 0x0D, true);
        }
        if (der.getData().available() >0)
            throw new ullink.security.krb5.Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
    }
    ...
    private void showDebug() {
        if (DEBUG) {
            ...
            System.out.println("\t error Message is " + ullink.security.krb5.internal.Krb5.getErrorMessage(errorCode));
            if (crealm != null) {
