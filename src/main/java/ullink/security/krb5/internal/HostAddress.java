...
public class HostAddress implements Cloneable {
    ...
    public InetAddress getInetAddress() throws UnknownHostException {
        ...
        if (addrType == ullink.security.krb5.internal.Krb5.ADDRTYPE_INET ||
            addrType == ullink.security.krb5.internal.Krb5.ADDRTYPE_INET6) {
    ...
    private int getAddrType(InetAddress inetAddress) {
        ...
        if (inetAddress instanceof Inet4Address)
            addressType = ullink.security.krb5.internal.Krb5.ADDRTYPE_INET;
        else if (inetAddress instanceof Inet6Address)
            addressType = ullink.security.krb5.internal.Krb5.ADDRTYPE_INET6;
        return (addressType);
    }
    ...
    public HostAddress(int new_addrType, byte[] new_address)
        ...
        switch(new_addrType) {
        case ullink.security.krb5.internal.Krb5.ADDRTYPE_INET:        //Internet address
            if (new_address.length != 4)
                throw new KrbApErrException(0, "Invalid Internet address");
            break;
        case ullink.security.krb5.internal.Krb5.ADDRTYPE_CHAOS:
            if (new_address.length != 2) //CHAOSnet address
                throw new KrbApErrException(0, "Invalid CHAOSnet address");
            break;
        case ullink.security.krb5.internal.Krb5.ADDRTYPE_ISO:   // ISO address
            break;
        case ullink.security.krb5.internal.Krb5.ADDRTYPE_IPX:   // XNS address
            if (new_address.length != 6)
                throw new KrbApErrException(0, "Invalid XNS address");
            break;
        case ullink.security.krb5.internal.Krb5.ADDRTYPE_APPLETALK:  //AppleTalk DDP address
            if (new_address.length != 3)
                throw new KrbApErrException(0, "Invalid DDP address");
            break;
        case ullink.security.krb5.internal.Krb5.ADDRTYPE_DECNET:    //DECnet Phase IV address
            if (new_address.length != 2)
                throw new KrbApErrException(0, "Invalid DECnet Phase IV address");
            break;
        case ullink.security.krb5.internal.Krb5.ADDRTYPE_INET6:     //Internet IPv6 address
