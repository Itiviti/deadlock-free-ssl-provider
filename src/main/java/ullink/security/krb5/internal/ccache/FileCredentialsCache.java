...
import ullink.security.krb5.internal.LoginOptions;
...
public class FileCredentialsCache extends CredentialsCache
    implements ullink.security.krb5.internal.ccache.FileCCacheConstants {
    public int version;
    public ullink.security.krb5.internal.ccache.Tag tag; // optional
    public PrincipalName primaryPrincipal;
    public Realm primaryRealm;
    private Vector<ullink.security.krb5.internal.ccache.Credentials> credentialsList;
    ...
    synchronized void init(PrincipalName principal, String name)
        ...
        ullink.security.krb5.internal.ccache.CCacheOutputStream cos =
            new ullink.security.krb5.internal.ccache.CCacheOutputStream(new FileOutputStream(name));
    ...
    synchronized void load(String name) throws IOException, KrbException {
        PrincipalName p;
        ullink.security.krb5.internal.ccache.CCacheInputStream cis =
            new ullink.security.krb5.internal.ccache.CCacheInputStream(new FileInputStream(name));
        ...
        credentialsList = new Vector<ullink.security.krb5.internal.ccache.Credentials> ();
        while (cis.available() > 0) {
            ullink.security.krb5.internal.ccache.Credentials cred = cis.readCred(version);
            if (cred != null) {
    ...
    public synchronized void update(ullink.security.krb5.internal.ccache.Credentials c) {
        if (credentialsList != null) {
            if (credentialsList.isEmpty()) {
                credentialsList.addElement(c);
            } else {
                ullink.security.krb5.internal.ccache.Credentials tmp = null;
    ...
    public synchronized void save() throws IOException, Asn1Exception {
        ullink.security.krb5.internal.ccache.CCacheOutputStream cos
            = new ullink.security.krb5.internal.ccache.CCacheOutputStream(new FileOutputStream(cacheName));
        cos.writeHeader(primaryPrincipal, version);
        ullink.security.krb5.internal.ccache.Credentials[] tmp = null;
     ...
     */
    public synchronized ullink.security.krb5.internal.ccache.Credentials[] getCredsList() {
        if ((credentialsList == null) || (credentialsList.isEmpty())) {
            return null;
        } else {
            ullink.security.krb5.internal.ccache.Credentials[] tmp = new ullink.security.krb5.internal.ccache.Credentials[credentialsList.size()];
            for (int i = 0; i < credentialsList.size(); i++) {
    ...
    public ullink.security.krb5.internal.ccache.Credentials getCreds(LoginOptions options,
                                PrincipalName sname, Realm srealm) {
        if (options == null) {
            return getCreds(sname, srealm);
        } else {
            ullink.security.krb5.internal.ccache.Credentials[] list = getCredsList();
            if (list == null) {
                return null;
            } else {
     ...
     */
    public ullink.security.krb5.internal.ccache.Credentials getCreds(PrincipalName sname, Realm srealm) {
        ullink.security.krb5.internal.ccache.Credentials[] list = getCredsList();
        if (list == null) {
            return null;
        } else {
    ...
    public ullink.security.krb5.internal.ccache.Credentials getDefaultCreds() {
        ullink.security.krb5.internal.ccache.Credentials[] list = getCredsList();
        if (list == null) {
            return null;
        } else {
