...
import ullink.security.krb5.internal.util.KrbDataInputStream;
...
public class CCacheInputStream extends KrbDataInputStream implements FileCCacheConstants {
    ...
    public ullink.security.krb5.internal.ccache.Tag readTag() throws IOException {
        ...
        ullink.security.krb5.internal.ccache.Tag result;
        if (tag == -1) {
        }
        result = new ullink.security.krb5.internal.ccache.Tag(len, tag, time_offset, usec_offset);
        return result;
    }
    /*
    ...
    public ullink.security.krb5.PrincipalName readPrincipal(int version) throws IOException, ullink.security.krb5.RealmException {
        int type, length, namelength, kret;
        ullink.security.krb5.PrincipalName p;
        String[] pname = null;
        String realm;
        /* Read principal type */
        ...
        if (isRealm(result[0])) {
            ...
            p = new ullink.security.krb5.PrincipalName(pname, type);
            p.setRealm(realm);
        }
        else p = new ullink.security.krb5.PrincipalName(result, type);
        return p;
    }
    ...
    boolean isRealm(String str) {
        try {
            ullink.security.krb5.Realm r = new ullink.security.krb5.Realm(str);
        }
        catch (Exception e) {
            return false;
        }
    ...
    ullink.security.krb5.EncryptionKey readKey(int version) throws IOException {
        int keyType, keyLen;
        keyType = read(2);
        ...
        return new ullink.security.krb5.EncryptionKey(bytes, keyType, new Integer(version));
    }
    ...
    ullink.security.krb5.internal.HostAddress[] readAddr() throws IOException, ullink.security.krb5.internal.KrbApErrException {
        int numAddrs, addrType, addrLength;
        numAddrs = read(4);
        if (numAddrs > 0) {
            ullink.security.krb5.internal.HostAddress[] addrs = new ullink.security.krb5.internal.HostAddress[numAddrs];
            for (int i = 0; i < numAddrs; i++) {
                ...
                addrs[i] = new ullink.security.krb5.internal.HostAddress(addrType, result);
            }
            return addrs;
        }
        return null;
    ...
    ullink.security.krb5.internal.AuthorizationDataEntry[] readAuth() throws IOException {
        int num, adtype, adlength;
        num = read(4);
        if (num > 0) {
            ullink.security.krb5.internal.AuthorizationDataEntry[] auData = new ullink.security.krb5.internal.AuthorizationDataEntry[num];
            byte[] data = null;
            for (int i = 0; i < num; i++) {
                adtype = read(2);
                adlength = read(4);
                ...
                auData[i] = new ullink.security.krb5.internal.AuthorizationDataEntry(adtype, data);
            }
            return auData;
        }
        else return null;
    ...
    boolean[] readFlags() throws IOException {
        boolean[] flags = new boolean[ullink.security.krb5.internal.Krb5.TKT_OPTS_MAX+1];
        int ticketFlags;
        ticketFlags = read(4);
     ...
     * returns null.
     */
    ullink.security.krb5.internal.ccache.Credentials readCred(int version) throws IOException, ullink.security.krb5.RealmException, ullink.security.krb5.internal.KrbApErrException, ullink.security.krb5.Asn1Exception {
        ullink.security.krb5.PrincipalName cpname = readPrincipal(version);
        if (DEBUG)
            System.out.println(">>>DEBUG <CCacheInputStream>  client principal is " + cpname.toString());
        ullink.security.krb5.PrincipalName spname = readPrincipal(version);
        if (DEBUG)
            System.out.println(">>>DEBUG <CCacheInputStream> server principal is " + spname.toString());
        ullink.security.krb5.EncryptionKey key = readKey(version);
        if (DEBUG)
            System.out.println(">>>DEBUG <CCacheInputStream> key type: " + key.getEType());
        long times[] = readTimes();
        ullink.security.krb5.internal.KerberosTime authtime = new ullink.security.krb5.internal.KerberosTime(times[0]);
        ullink.security.krb5.internal.KerberosTime starttime = new ullink.security.krb5.internal.KerberosTime(times[1]);
        ullink.security.krb5.internal.KerberosTime endtime = new ullink.security.krb5.internal.KerberosTime(times[2]);
        ullink.security.krb5.internal.KerberosTime renewTill = new ullink.security.krb5.internal.KerberosTime(times[3]);
        ...
        ullink.security.krb5.internal.TicketFlags tFlags = new ullink.security.krb5.internal.TicketFlags(flags);
        ullink.security.krb5.internal.HostAddress addr[] = readAddr();
        ullink.security.krb5.internal.HostAddresses addrs = null;
        if (addr != null) {
            addrs = new ullink.security.krb5.internal.HostAddresses(addr);
        }
        ullink.security.krb5.internal.AuthorizationDataEntry[] auDataEntry = readAuth();
        ullink.security.krb5.internal.AuthorizationData auData = null;
        if (auData != null) {
            auData = new ullink.security.krb5.internal.AuthorizationData(auDataEntry);
        }
        byte[] ticketData = readData();
        byte[] ticketData2 = readData();

        try {
            return new ullink.security.krb5.internal.ccache.Credentials(cpname, spname, key, authtime, starttime,
                endtime, renewTill, skey, tFlags,
                addrs, auData,
                ticketData != null ? new ullink.security.krb5.internal.Ticket(ticketData) : null,
                ticketData2 != null ? new ullink.security.krb5.internal.Ticket(ticketData2) : null);
