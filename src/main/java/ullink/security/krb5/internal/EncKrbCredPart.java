...
public class EncKrbCredPart {
        ...
        public ullink.security.krb5.internal.KerberosTime timeStamp; //optional

        private Integer nonce; //optional
        private Integer usec; //optional
        private ullink.security.krb5.internal.HostAddress sAddress; //optional
        private ullink.security.krb5.internal.HostAddresses rAddress; //optional

        public EncKrbCredPart(
                KrbCredInfo[] new_ticketInfo,
                ullink.security.krb5.internal.KerberosTime new_timeStamp,
                Integer new_usec,
                Integer new_nonce,
                ullink.security.krb5.internal.HostAddress new_sAddress,
                ullink.security.krb5.internal.HostAddresses new_rAddress
        ) throws IOException {
        ...
        if (der.getTag() != DerValue.tag_Sequence)
           ...
           DerValue derValues[] = subDer.getData().getSequence(1);
                ...
                if (der.getData().available() >0) {
                        timeStamp = ullink.security.krb5.internal.KerberosTime.parse(der.getData(), (byte) 0x02, true);
                }
        if (der.getData().available() >0) {
        ...
        if (der.getData().available() >0) {
                        sAddress = ullink.security.krb5.internal.HostAddress.parse(der.getData(), (byte) 0x04, true);
                }
        if (der.getData().available() >0) {
                        rAddress = ullink.security.krb5.internal.HostAddresses.parse(der.getData(), (byte) 0x05, true);
                }
        if (der.getData().available() >0)
                        throw new Asn1Exception(ullink.security.krb5.internal.Krb5.ASN1_BAD_ID);
        }
