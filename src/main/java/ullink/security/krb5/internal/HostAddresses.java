...
import ullink.security.krb5.Asn1Exception;
...
public class HostAddresses implements Cloneable {
    ...
    private ullink.security.krb5.internal.HostAddress[] addresses = null;
    private volatile int hashCode = 0;

    public HostAddresses(ullink.security.krb5.internal.HostAddress[] new_addresses) throws IOException {
        if (new_addresses != null) {
           addresses = new ullink.security.krb5.internal.HostAddress[new_addresses.length];
           for (int i = 0; i < new_addresses.length; i++) {
                if (new_addresses[i] == null) {
                   throw new IOException("Cannot create a HostAddress");
                } else {
                   addresses[i] = (ullink.security.krb5.internal.HostAddress)new_addresses[i].clone();
                }
           }
        }
    }
    ...
    public HostAddresses() throws UnknownHostException {
        addresses = new ullink.security.krb5.internal.HostAddress[1];
        addresses[0] = new ullink.security.krb5.internal.HostAddress();
    }
    ...
    public HostAddresses(PrincipalName serverPrincipal)
        ...
        ullink.security.krb5.internal.HostAddress hAddrs[] = new ullink.security.krb5.internal.HostAddress[addr.length];

        for (int i = 0; i < addr.length; i++) {
            hAddrs[i] = new ullink.security.krb5.internal.HostAddress(addr[i]);
        }
    ...
    public Object clone() {
        ...
        if (addresses != null) {
            new_hostAddresses.addresses = new ullink.security.krb5.internal.HostAddress[addresses.length];
            for (int i = 0; i < addresses.length; i++) {
                new_hostAddresses.addresses[i] =
                        (ullink.security.krb5.internal.HostAddress)addresses[i].clone();
            }
        }
        return new_hostAddresses;
    }

    public boolean inList(ullink.security.krb5.internal.HostAddress addr) {
        if (addresses != null) {
            for (int i = 0; i < addresses.length; i++)
    ...
    */
    public HostAddresses(DerValue encoding)
        ...
        Vector<ullink.security.krb5.internal.HostAddress> tempAddresses = new Vector<ullink.security.krb5.internal.HostAddress> ();
        DerValue der = null;
        while (encoding.getData().available() > 0) {
            der = encoding.getData().getDerValue();
            tempAddresses.addElement(new ullink.security.krb5.internal.HostAddress(der));
        }
        if (tempAddresses.size() > 0) {
            addresses = new ullink.security.krb5.internal.HostAddress[tempAddresses.size()];
    ...
    */
    ...
    public static HostAddresses getLocalAddresses() throws IOException
    {
        ...
        try {
            ...
            ullink.security.krb5.internal.HostAddress[] hAddresses = new ullink.security.krb5.internal.HostAddress[inetAddresses.length];
            for (int i = 0; i < inetAddresses.length; i++)
                {
                    hAddresses[i] = new ullink.security.krb5.internal.HostAddress(inetAddresses[i]);
                }
            if (DEBUG) {
    ...
    public HostAddresses(InetAddress[] inetAddresses)
    {
        ...
        addresses = new ullink.security.krb5.internal.HostAddress[inetAddresses.length];
        for (int i = 0; i < inetAddresses.length; i++)
            addresses[i] = new ullink.security.krb5.internal.HostAddress(inetAddresses[i]);
    }
}
