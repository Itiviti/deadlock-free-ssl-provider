...
package ullink.security.krb5.internal.ktab;

import ullink.security.action.GetPropertyAction;
import ullink.security.krb5.internal.KerberosTime;
...
public class KeyTab implements KeyTabConstants {
    ...
    private Vector<ullink.security.krb5.internal.ktab.KeyTabEntry> entries = new Vector<ullink.security.krb5.internal.ktab.KeyTabEntry> ();

    private KeyTab(String filename) throws IOException, ullink.security.krb5.RealmException {
        init(filename);
    }
    ...
    private static String getDefaultKeyTab() {
        if (name != null) {
            ...
            String kname = null;
            try {
                String keytab_names = ullink.security.krb5.Config.getInstance().getDefault
    ...
    private synchronized void init(String filename)
        throws IOException, ullink.security.krb5.RealmException {

        if (filename != null) {
            ullink.security.krb5.internal.ktab.KeyTabInputStream kis =
                new ullink.security.krb5.internal.ktab.KeyTabInputStream(new FileInputStream(filename));
            load(kis);
            kis.close();
            name = filename;
        }
    ...
    private void load(ullink.security.krb5.internal.ktab.KeyTabInputStream kis)
        throws IOException, ullink.security.krb5.RealmException {
        ...
        int entryLength = 0;
        ullink.security.krb5.internal.ktab.KeyTabEntry entry;
     ...
     */
    public ullink.security.krb5.EncryptionKey readServiceKey(ullink.security.krb5.PrincipalName service) {
        ullink.security.krb5.internal.ktab.KeyTabEntry entry = null;
        if (entries != null) {
            ...
            for (int i = entries.size()-1; i >= 0; i--) {
                ...
                if (entry.service.match(service)) {
                    if (EType.isSupported(entry.keyType)) {
                        return new ullink.security.krb5.EncryptionKey(entry.keyblock,
                                             entry.keyType,
     ...
     */
    public ullink.security.krb5.EncryptionKey[] readServiceKeys(ullink.security.krb5.PrincipalName service) {
        ullink.security.krb5.internal.ktab.KeyTabEntry entry;
        ullink.security.krb5.EncryptionKey key;
        int size = entries.size();
        ArrayList<ullink.security.krb5.EncryptionKey> keys = new ArrayList<ullink.security.krb5.EncryptionKey> (size);
        if (entries != null) {
            for (int i = size-1; i >= 0; i--) {
                entry = entries.elementAt(i);
                if (entry.service.match(service)) {
                    if (EType.isSupported(entry.keyType)) {
                        key = new ullink.security.krb5.EncryptionKey(entry.keyblock,
                                            entry.keyType,
                                            new Integer(entry.keyVersion));
                        keys.add(key);
                        if (DEBUG) {
        ...
        if (size == 0)
            return null;
        ullink.security.krb5.EncryptionKey[] retVal = new ullink.security.krb5.EncryptionKey[size];

        // Sort keys according to default_tkt_enctypes
        int pos = 0;
        ullink.security.krb5.EncryptionKey k;
        if (DEBUG) {
     ...
     */
    public boolean findServiceEntry(ullink.security.krb5.PrincipalName service) {
        ullink.security.krb5.internal.ktab.KeyTabEntry entry;
        if (entries != null) {
     ...
     */
    public void addEntry(ullink.security.krb5.PrincipalName service, char[] psswd)
         throws ullink.security.krb5.KrbException {

        ullink.security.krb5.EncryptionKey[] encKeys = ullink.security.krb5.EncryptionKey.acquireSecretKeys(
        ...
        for (int i = 0; encKeys != null && i < encKeys.length; i++) {
            ...
            int kvno = 1;
            if (result != -1) {
                ullink.security.krb5.internal.ktab.KeyTabEntry oldEntry = entries.elementAt(result);
            ...
            ullink.security.krb5.internal.ktab.KeyTabEntry newEntry = new ullink.security.krb5.internal.ktab.KeyTabEntry(service,
                            service.getRealm(),
                            new KerberosTime(System.currentTimeMillis()),
                                               kvno, keyType, keyValue);
            if (entries == null)
                entries = new Vector<ullink.security.krb5.internal.ktab.KeyTabEntry> ();
     ...
     * in the list.
     */
    private int retrieveEntry(ullink.security.krb5.PrincipalName service, int keyType) {
        int found = -1;
        ullink.security.krb5.internal.ktab.KeyTabEntry e;
        if (entries != null) {
            for (int i = 0; i < entries.size(); i++) {
     ...
     */
    public ullink.security.krb5.internal.ktab.KeyTabEntry[] getEntries() {
        if (entries != null) {
            ullink.security.krb5.internal.ktab.KeyTabEntry[] kentries = new ullink.security.krb5.internal.ktab.KeyTabEntry[entries.size()];
    ...
    public synchronized static KeyTab create(String name)
        ...
        ullink.security.krb5.internal.ktab.KeyTabOutputStream kos =
                new ullink.security.krb5.internal.ktab.KeyTabOutputStream(new FileOutputStream(name));
     ...
     */
    public void deleteEntry(ullink.security.krb5.PrincipalName service) {
    ...
    public synchronized void createVersion(File file) throws IOException {
        ullink.security.krb5.internal.ktab.KeyTabOutputStream kos =
                new ullink.security.krb5.internal.ktab.KeyTabOutputStream(new FileOutputStream(file));
