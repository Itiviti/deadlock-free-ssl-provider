...
import ullink.security.krb5.internal.util.KerberosFlags;
...
public class TicketFlags extends KerberosFlags {
    public TicketFlags() {
        super(ullink.security.krb5.internal.Krb5.TKT_OPTS_MAX + 1);
    }

    public TicketFlags (boolean[] flags) throws Asn1Exception {
        super(flags);
        if (flags.length > ullink.security.krb5.internal.Krb5.TKT_OPTS_MAX + 1) {
            throw new Asn1Exception(ullink.security.krb5.internal.Krb5.BITSTRING_BAD_LENGTH);
        }
    }

    public TicketFlags(int size, byte[] data) throws Asn1Exception {
        super(size, data);
        if ((size > data.length * BITS_PER_UNIT) || (size > ullink.security.krb5.internal.Krb5.TKT_OPTS_MAX + 1))
            throw new Asn1Exception(ullink.security.krb5.internal.Krb5.BITSTRING_BAD_LENGTH);
    }
    ...
    public boolean match(LoginOptions options) {
        ...
        if (this.get(ullink.security.krb5.internal.Krb5.TKT_OPTS_FORWARDABLE) == (options.get(ullink.security.krb5.internal.KDCOptions.FORWARDABLE))) {
            if (this.get(ullink.security.krb5.internal.Krb5.TKT_OPTS_PROXIABLE) == (options.get(ullink.security.krb5.internal.KDCOptions.PROXIABLE))) {
                if (this.get(ullink.security.krb5.internal.Krb5.TKT_OPTS_RENEWABLE) == (options.get(ullink.security.krb5.internal.KDCOptions.RENEWABLE))) {
                    matched = true;
                }
            }
        }
    ...
    public boolean match(TicketFlags flags) {
        ...
        for (int i = 0; i <= ullink.security.krb5.internal.Krb5.TKT_OPTS_MAX; i++) {
