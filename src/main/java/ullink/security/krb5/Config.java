...
public class Config {
    ...
    public String getDefault(String name, String section) {
        ...
        if (name.equalsIgnoreCase("kdc") &&
            ...
            (java.security.AccessController.doPrivileged(
                new GetPropertyAction("java.security.krb5.kdc")) != null)) {
    ...
    private String getFileName() {
        String name =
            java.security.AccessController.doPrivileged(
                                new GetPropertyAction("java.security.krb5.conf"));
        if (name != null) {
            boolean temp =
                java.security.AccessController.doPrivileged(
            ...
            if (temp) {
                ...
                if (osname.startsWith("Windows")) {
                    try {
                        ullink.security.krb5.Credentials.ensureLoaded();
                    } catch (Exception e) {
                        // ignore exceptions
                    }
                    if (ullink.security.krb5.Credentials.alreadyLoaded) {
    ...
    public int getType(String input) {
        ...
        if (input.startsWith("d") || (input.startsWith("D"))) {
            if (input.equalsIgnoreCase("des-cbc-crc")) {
                result = ullink.security.krb5.EncryptedData.ETYPE_DES_CBC_CRC;
            } else if (input.equalsIgnoreCase("des-cbc-md5")) {
                result = ullink.security.krb5.EncryptedData.ETYPE_DES_CBC_MD5;
            } else if (input.equalsIgnoreCase("des-mac")) {
                result = ullink.security.krb5.Checksum.CKSUMTYPE_DES_MAC;
            } else if (input.equalsIgnoreCase("des-mac-k")) {
                result = ullink.security.krb5.Checksum.CKSUMTYPE_DES_MAC_K;
            } else if (input.equalsIgnoreCase("des-cbc-md4")) {
                result = ullink.security.krb5.EncryptedData.ETYPE_DES_CBC_MD4;
                ...
                result = ullink.security.krb5.EncryptedData.ETYPE_DES3_CBC_HMAC_SHA1_KD;
            }
            ...
            if (input.equalsIgnoreCase("aes128-cts") ||
                ...
                result = ullink.security.krb5.EncryptedData.ETYPE_AES128_CTS_HMAC_SHA1_96;
            } else if (input.equalsIgnoreCase("aes256-cts") ||
                input.equalsIgnoreCase("aes256-cts-hmac-sha1-96")) {
                result = ullink.security.krb5.EncryptedData.ETYPE_AES256_CTS_HMAC_SHA1_96;
            // ARCFOUR-HMAC
            } else if (input.equalsIgnoreCase("arcfour-hmac") ||
                   input.equalsIgnoreCase("arcfour-hmac-md5")) {
                result = ullink.security.krb5.EncryptedData.ETYPE_ARCFOUR_HMAC;
            }
        // RC4-HMAC
        } else if (input.equalsIgnoreCase("rc4-hmac")) {
            result = ullink.security.krb5.EncryptedData.ETYPE_ARCFOUR_HMAC;
        } else if (input.equalsIgnoreCase("CRC32")) {
            result = ullink.security.krb5.Checksum.CKSUMTYPE_CRC32;
        } else if (input.startsWith("r") || (input.startsWith("R"))) {
            if (input.equalsIgnoreCase("rsa-md5")) {
                result = ullink.security.krb5.Checksum.CKSUMTYPE_RSA_MD5;
            } else if (input.equalsIgnoreCase("rsa-md5-des")) {
                result = ullink.security.krb5.Checksum.CKSUMTYPE_RSA_MD5_DES;
            }
        } else if (input.equalsIgnoreCase("hmac-sha1-des3-kd")) {
            result = ullink.security.krb5.Checksum.CKSUMTYPE_HMAC_SHA1_DES3_KD;
        } else if (input.equalsIgnoreCase("hmac-sha1-96-aes128")) {
            result = ullink.security.krb5.Checksum.CKSUMTYPE_HMAC_SHA1_96_AES128;
        } else if (input.equalsIgnoreCase("hmac-sha1-96-aes256")) {
            result = ullink.security.krb5.Checksum.CKSUMTYPE_HMAC_SHA1_96_AES256;
        } else if (input.equalsIgnoreCase("hmac-md5-rc4") ||
                input.equalsIgnoreCase("hmac-md5-arcfour") ||
                input.equalsIgnoreCase("hmac-md5-enc")) {
            result = ullink.security.krb5.Checksum.CKSUMTYPE_HMAC_MD5_ARCFOUR;
        } else if (input.equalsIgnoreCase("NULL")) {
            result = ullink.security.krb5.EncryptedData.ETYPE_NULL;
        }
