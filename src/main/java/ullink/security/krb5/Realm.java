...
public class Realm implements Cloneable {
    ...
    public Realm(DerValue encoding)
        throws ullink.security.krb5.Asn1Exception, RealmException, IOException {
        if (encoding == null) {
    ...
    public static String parseRealmAtSeparator(String name)
        ...
        int i = 0;
        while (i < temp.length()) {
            if (temp.charAt(i) == ullink.security.krb5.PrincipalName.NAME_REALM_SEPARATOR) {
     ...
     *
     */
    public static Realm parse(DerInputStream data, byte explicitTag, boolean optional) throws ullink.security.krb5.Asn1Exception, IOException, RealmException {
    ...
    private static String[] doInitialParse(String cRealm, String sRealm)
        throws ullink.security.krb5.KrbException {
            if (cRealm == null || sRealm == null){
                throw new ullink.security.krb5.KrbException(Krb5.API_INVALID_ARG);
            }
            if (DEBUG) {
    ...
    private static String[] parseCapaths(String cRealm, String sRealm) throws ullink.security.krb5.KrbException {
        String[] retList = null;

        ullink.security.krb5.Config cfg = null;
        try {
            cfg = ullink.security.krb5.Config.getInstance();
        } catch (Exception exc) {
            if (DEBUG) {
        ...
        do {
            ...
            if (intermediaries != null &&
                !intermediaries.equals(ullink.security.krb5.PrincipalName.REALM_COMPONENT_SEPARATOR_STR))
            {
                if (DEBUG) {
                    System.out.println(">>> Realm parseCapaths: loop " +
                ...
                while (strTok.hasMoreTokens())
                {
                    ...
                    if (!tempRealm.equals(ullink.security.krb5.PrincipalName.
        ...
        throws ullink.security.krb5.KrbException
    {
        ...
        new StringTokenizer(cRealm,
                            ullink.security.krb5.PrincipalName.REALM_COMPONENT_SEPARATOR_STR);
    ...
    private static String substring(String realm, int componentIndex)
    {
        ...
        while(i < len && j != componentIndex) {
            if (realm.charAt(i++) != ullink.security.krb5.PrincipalName.REALM_COMPONENT_SEPARATOR)
                continue;
            j++;
        }
