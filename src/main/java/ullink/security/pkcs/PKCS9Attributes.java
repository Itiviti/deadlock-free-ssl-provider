...
public class PKCS9Attributes {
     ...
     */
    private final Hashtable<ullink.security.util.ObjectIdentifier, ullink.security.pkcs.PKCS9Attribute> attributes =
        new Hashtable<ullink.security.util.ObjectIdentifier, ullink.security.pkcs.PKCS9Attribute>(3);

    /**
     * The keys of this hashtable are the OIDs of permitted attributes.
     */
    private final Hashtable<ullink.security.util.ObjectIdentifier, ullink.security.util.ObjectIdentifier> permittedAttributes;
     ...
     *
     * @see ullink.security.pkcs.PKCS9Attribute
     */
    public PKCS9Attributes(ullink.security.util.ObjectIdentifier[] permittedAttributes,
                           ullink.security.util.DerInputStream in) throws IOException {
        if (permittedAttributes != null) {
            this.permittedAttributes =
                new Hashtable<ullink.security.util.ObjectIdentifier, ullink.security.util.ObjectIdentifier>(
     ...
     * @see ullink.security.pkcs.PKCS9Attribute
     */
    public PKCS9Attributes(ullink.security.util.DerInputStream in) throws IOException {
        this(in, false);
    }
     ...
     * @see ullink.security.pkcs.PKCS9Attribute
     */
    public PKCS9Attributes(ullink.security.util.DerInputStream in,
     ...
     * @see ullink.security.pkcs.PKCS9Attribute
     */
    public PKCS9Attributes(ullink.security.pkcs.PKCS9Attribute[] attribs)
     ...
     * unsupported attribute, or duplicate attribute.
     */
    private byte[] decode(ullink.security.util.DerInputStream in) throws IOException {

        ullink.security.util.DerValue val = in.getDerValue();

        // save the DER encoding with its proper tag byte.
        byte[] derEncoding = val.toByteArray();
        derEncoding[0] = ullink.security.util.DerValue.tag_SetOf;

        ullink.security.util.DerInputStream derIn = new ullink.security.util.DerInputStream(derEncoding);
        ullink.security.util.DerValue[] derVals = derIn.getSet(3,true);

        ullink.security.pkcs.PKCS9Attribute attrib;
        ullink.security.util.ObjectIdentifier oid;
        ...
        for (int i=0; i < derVals.length; i++) {
            ...
            try {
                attrib = new ullink.security.pkcs.PKCS9Attribute(derVals[i]);
    ...
    private byte[] generateDerEncoding() throws IOException {
        ...
        out.putOrderedSetOf(ullink.security.util.DerValue.tag_SetOf,
     ...
     */
    public ullink.security.pkcs.PKCS9Attribute getAttribute(ullink.security.util.ObjectIdentifier oid) {
     ...
     * Get an attribute from this set.
     */
    public ullink.security.pkcs.PKCS9Attribute getAttribute(String name) {
        return attributes.get(ullink.security.pkcs.PKCS9Attribute.getOID(name));
    }
     ...
     */
    public ullink.security.pkcs.PKCS9Attribute[] getAttributes() {
        ullink.security.pkcs.PKCS9Attribute[] attribs = new ullink.security.pkcs.PKCS9Attribute[attributes.size()];
        ullink.security.util.ObjectIdentifier oid;

        int j = 0;
        for (int i=1; i < ullink.security.pkcs.PKCS9Attribute.PKCS9_OIDS.length &&
                      j < attribs.length; i++) {
            attribs[j] = getAttribute(ullink.security.pkcs.PKCS9Attribute.PKCS9_OIDS[i]);
     ...
     */
    public Object getAttributeValue(ullink.security.util.ObjectIdentifier oid)
    throws IOException {
        try {
    ...
    public Object getAttributeValue(String name) throws IOException {
        ullink.security.util.ObjectIdentifier oid = ullink.security.pkcs.PKCS9Attribute.getOID(name);
    ...
    public String toString() {
        ...
        ullink.security.util.ObjectIdentifier oid;
        ullink.security.pkcs.PKCS9Attribute value;

        boolean first = true;
        for (int i = 1; i < ullink.security.pkcs.PKCS9Attribute.PKCS9_OIDS.length; i++) {
            value = getAttribute(ullink.security.pkcs.PKCS9Attribute.PKCS9_OIDS[i]);
     ...
     */
    static ullink.security.util.DerEncoder[] castToDerEncoder(Object[] objs) {

        ullink.security.util.DerEncoder[] encoders = new ullink.security.util.DerEncoder[objs.length];

        for (int i=0; i < encoders.length; i++)
            encoders[i] = (ullink.security.util.DerEncoder) objs[i];
