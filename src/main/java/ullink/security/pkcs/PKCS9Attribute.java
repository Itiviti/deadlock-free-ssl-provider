...
package ullink.security.pkcs;

import sun.misc.HexDumpEncoder;
import ullink.security.x509.CertificateExtensions;
 ...
 */
public class PKCS9Attribute implements ullink.security.util.DerEncoder {

    /* Are we debugging ? */
    private static final ullink.security.util.Debug debug = ullink.security.util.Debug.getInstance("jar");

    /**
     * Array of attribute OIDs defined in PKCS9, by number.
     */
    static final ullink.security.util.ObjectIdentifier[] PKCS9_OIDS = new ullink.security.util.ObjectIdentifier[18];

    static {   // static initializer for PKCS9_OIDS
        for (int i = 1; i < PKCS9_OIDS.length - 2; i++) {
            PKCS9_OIDS[i] =
                ullink.security.util.ObjectIdentifier.newInternal(new int[]{1, 2, 840, 113549, 1, 9, i});
        }
        ...
        PKCS9_OIDS[PKCS9_OIDS.length - 2] =
            ullink.security.util.ObjectIdentifier.newInternal(new int[]{1, 2, 840, 113549, 1, 9, 16, 2, 12});
        PKCS9_OIDS[PKCS9_OIDS.length - 1] =
            ullink.security.util.ObjectIdentifier.newInternal(new int[]{1, 2, 840, 113549, 1, 9, 16, 2, 14});
    }

    // first element [0] not used
    public static final ullink.security.util.ObjectIdentifier EMAIL_ADDRESS_OID = PKCS9_OIDS[1];
    public static final ullink.security.util.ObjectIdentifier UNSTRUCTURED_NAME_OID = PKCS9_OIDS[2];
    public static final ullink.security.util.ObjectIdentifier CONTENT_TYPE_OID = PKCS9_OIDS[3];
    public static final ullink.security.util.ObjectIdentifier MESSAGE_DIGEST_OID = PKCS9_OIDS[4];
    public static final ullink.security.util.ObjectIdentifier SIGNING_TIME_OID = PKCS9_OIDS[5];
    public static final ullink.security.util.ObjectIdentifier COUNTERSIGNATURE_OID = PKCS9_OIDS[6];
    public static final ullink.security.util.ObjectIdentifier CHALLENGE_PASSWORD_OID = PKCS9_OIDS[7];
    public static final ullink.security.util.ObjectIdentifier UNSTRUCTURED_ADDRESS_OID = PKCS9_OIDS[8];
    public static final ullink.security.util.ObjectIdentifier EXTENDED_CERTIFICATE_ATTRIBUTES_OID
                                         = PKCS9_OIDS[9];
    public static final ullink.security.util.ObjectIdentifier ISSUER_SERIALNUMBER_OID = PKCS9_OIDS[10];
    // [11], [12] are RSA DSI proprietary
    // [13] ==> signingDescription, S/MIME, not used anymore
    public static final ullink.security.util.ObjectIdentifier EXTENSION_REQUEST_OID = PKCS9_OIDS[14];
    public static final ullink.security.util.ObjectIdentifier SMIME_CAPABILITY_OID = PKCS9_OIDS[15];
    public static final ullink.security.util.ObjectIdentifier SIGNING_CERTIFICATE_OID = PKCS9_OIDS[16];
    public static final ullink.security.util.ObjectIdentifier SIGNATURE_TIMESTAMP_TOKEN_OID =
                                PKCS9_OIDS[17];
     ...
     */
    private static final Hashtable<String, ullink.security.util.ObjectIdentifier> NAME_OID_TABLE =
        new Hashtable<String, ullink.security.util.ObjectIdentifier>(18);
     ...
     */
    private static final Hashtable<ullink.security.util.ObjectIdentifier, String> OID_NAME_TABLE =
        new Hashtable<ullink.security.util.ObjectIdentifier, String>(16);
    static {
    ...
    private static final Byte[][] PKCS9_VALUE_TAGS = {
        null,
        {new Byte(ullink.security.util.DerValue.tag_IA5String)},   // EMailAddress
        {new Byte(ullink.security.util.DerValue.tag_IA5String)},   // UnstructuredName
        {new Byte(ullink.security.util.DerValue.tag_ObjectId)},    // ContentType
        {new Byte(ullink.security.util.DerValue.tag_OctetString)}, // MessageDigest
        {new Byte(ullink.security.util.DerValue.tag_UtcTime)},     // SigningTime
        {new Byte(ullink.security.util.DerValue.tag_Sequence)},    // Countersignature
        {new Byte(ullink.security.util.DerValue.tag_PrintableString),
         new Byte(ullink.security.util.DerValue.tag_T61String)},   // ChallengePassword
        {new Byte(ullink.security.util.DerValue.tag_PrintableString),
         new Byte(ullink.security.util.DerValue.tag_T61String)},   // UnstructuredAddress
        {new Byte(ullink.security.util.DerValue.tag_SetOf)},       // ExtendedCertificateAttributes
        {new Byte(ullink.security.util.DerValue.tag_Sequence)},    // issuerAndSerialNumber
        null,
        null,
        null,
        {new Byte(ullink.security.util.DerValue.tag_Sequence)},    // extensionRequest
        {new Byte(ullink.security.util.DerValue.tag_Sequence)},    // SMIMECapability
        {new Byte(ullink.security.util.DerValue.tag_Sequence)},    // SigningCertificate
        {new Byte(ullink.security.util.DerValue.tag_Sequence)}     // SignatureTimestampToken
    };
    ...
    static {
        try {
            ...
            VALUE_CLASSES[3] =        // ContentType
                Class.forName("ObjectIdentifier");
            ...
            VALUE_CLASSES[14] =        // ExtensionRequest
                Class.forName("CertificateExtensions");
     ...
     *
     */
    public PKCS9Attribute(ullink.security.util.ObjectIdentifier oid, Object value)
    ...
    throws IllegalArgumentException {
        ullink.security.util.ObjectIdentifier oid = getOID(name);
    ...
    private void init(ullink.security.util.ObjectIdentifier oid, Object value)
        throws IllegalArgumentException {
     ...
     */
    public PKCS9Attribute(ullink.security.util.DerValue derVal) throws IOException {

        ullink.security.util.DerInputStream derIn = new ullink.security.util.DerInputStream(derVal.toByteArray());
        ullink.security.util.DerValue[] val =  derIn.getSequence(2);
        ...
        // get the oid
        ullink.security.util.ObjectIdentifier oid = val[0].getOID();
        index = indexOf(oid, PKCS9_OIDS, 1);
        if (index == -1) {
            if (debug != null) {
        ...
        ullink.security.util.DerValue[] elems = new ullink.security.util.DerInputStream(val[1].toByteArray()).getSet(1);
        ...
        case 5:     // signing time
            value = (new ullink.security.util.DerInputStream(elems[0].toByteArray())).getUTCTime();
            break;

        case 6:     // countersignature
            { // open scope
                ullink.security.pkcs.SignerInfo[] values = new ullink.security.pkcs.SignerInfo[elems.length];
                for (int i=0; i < elems.length; i++)
                    values[i] =
                        new ullink.security.pkcs.SignerInfo(elems[i].toDerInputStream());
                value = values;
            } // close scope
            break;
        ...
        case 14:     // ExtensionRequest
            value = new CertificateExtensions(
                       new ullink.security.util.DerInputStream(elems[0].toByteArray()));
            break;
        ...
        case 16:     // SigningCertificate attribute
            value = new ullink.security.pkcs.SigningCertificateInfo(elems[0].toByteArray());
            break;
    ...
    public void derEncode(OutputStream out) throws IOException {
        ullink.security.util.DerOutputStream temp = new ullink.security.util.DerOutputStream();
        temp.putOID(getOID());
        switch (index) {
        case 1:     // email address
        case 2:     // unstructured name
            { // open scope
                ...
                ullink.security.util.DerOutputStream[] temps = new
                        ullink.security.util.DerOutputStream[values.length];

                for (int i=0; i < values.length; i++) {
                    temps[i] = new ullink.security.util.DerOutputStream();
                    temps[i].putIA5String( values[i]);
                }
                temp.putOrderedSetOf(ullink.security.util.DerValue.tag_Set, temps);
            } // close scope
            break;

        case 3:     // content type
            {
                ullink.security.util.DerOutputStream temp2 = new ullink.security.util.DerOutputStream();
                temp2.putOID((ullink.security.util.ObjectIdentifier) value);
                temp.write(ullink.security.util.DerValue.tag_Set, temp2.toByteArray());
            }
            break;
        ...
        case 6:     // countersignature
            temp.putOrderedSetOf(ullink.security.util.DerValue.tag_Set, (ullink.security.util.DerEncoder[]) value);
            break;
        ...
        case 17:    // SignatureTimestampToken
            temp.write(ullink.security.util.DerValue.tag_Set, (byte[])value);
            break;
        ...
        derOut.write(ullink.security.util.DerValue.tag_Sequence, temp.toByteArray());
     ...
     */
    public ullink.security.util.ObjectIdentifier getOID() {
     ...
     * the name.
     */
    public static ullink.security.util.ObjectIdentifier getOID(String name) {
     ...
     * the oid.
     */
    public static String getName(ullink.security.util.ObjectIdentifier oid) {
