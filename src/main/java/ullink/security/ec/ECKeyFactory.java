...
public final class ECKeyFactory extends KeyFactorySpi {
    ...
    private static void checkKey(ECKey key) throws InvalidKeyException {
        ...
        if (key instanceof ECPublicKey) {
            ...
            if (key instanceof ullink.security.ec.ECPrivateKeyImpl) {
                return;
            }
        } else {
    ...
    private PrivateKey implTranslatePrivateKey(PrivateKey key)
        ...
        if (key instanceof ECPrivateKey) {
            ...
            return new ullink.security.ec.ECPrivateKeyImpl(
                ecKey.getS(),
                ecKey.getParams()
            );
        } else if ("PKCS#8".equals(key.getFormat())) {
            return new ullink.security.ec.ECPrivateKeyImpl(key.getEncoded());
        } else {
    ...
    private PrivateKey implGeneratePrivate(KeySpec keySpec)
        ...
        if (keySpec instanceof PKCS8EncodedKeySpec) {
            ...
            return new ullink.security.ec.ECPrivateKeyImpl(pkcsSpec.getEncoded());
        } else if (keySpec instanceof ECPrivateKeySpec) {
            ECPrivateKeySpec ecSpec = (ECPrivateKeySpec)keySpec;
            return new ullink.security.ec.ECPrivateKeyImpl(ecSpec.getS(), ecSpec.getParams());
        } else {
