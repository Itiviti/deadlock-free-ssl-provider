...
public class PolicyFile extends java.security.Policy {
    ...
    private static final ullink.security.util.Debug debug = ullink.security.util.Debug.getInstance("policy");
            ...
            public Boolean run() {
                ...
                if (allowSystemProperties) {
                    ...
                    if (extra_policy != null) {
                        ...
                        try {
                            extra_policy =
                                ullink.security.util.PropertyExpander.expand(extra_policy);
                            URL policyURL;
                ...
                while ((policy_uri = Security.getProperty(urlname+n)) != null) {
                    try {
                        URL policy_url = null;
                        String expanded_uri = ullink.security.util.PropertyExpander.expand
    ...
    private boolean init(URL policy, PolicyInfo newInfo) {
        ...
        ullink.security.provider.PolicyParser pp = new ullink.security.provider.PolicyParser(expandProperties);
        ...
        try {
            ...
            if (notUtf8) {
                isr = new InputStreamReader
                                (ullink.security.util.PolicyUtil.getInputStream(policy));
            } else {
                isr = new InputStreamReader
                                (ullink.security.util.PolicyUtil.getInputStream(policy), "UTF-8");
            }
            ...
            try {
                keyStore = ullink.security.util.PolicyUtil.getKeyStore
                        (policy,
                                pp.getKeyStoreUrl(),
                                pp.getKeyStoreType(),
            ...
            Enumeration<ullink.security.provider.PolicyParser.GrantEntry> enum_ = pp.grantElements();
            while (enum_.hasMoreElements()) {
                ullink.security.provider.PolicyParser.GrantEntry ge = enum_.nextElement();
                addGrantEntry(ge, keyStore, newInfo);
            }
        } catch (ullink.security.provider.PolicyParser.ParsingException pe) {
            MessageFormat form = new MessageFormat(ullink.security.util.ResourcesMgr.getString
        ...
        AccessController.doPrivileged(new PrivilegedAction<Void>() {
            public Void run() {
                PolicyEntry pe = new PolicyEntry(new CodeSource(null,
                    (Certificate[]) null));
                pe.add(ullink.security.util.SecurityConstants.LOCAL_LISTEN_PERMISSION);
                pe.add(new PropertyPermission("java.version",
                    ullink.security.util.SecurityConstants.PROPERTY_READ_ACTION));
                ...
                String[] extCodebases = ullink.security.provider.PolicyParser.parseExtDirs(
                        ullink.security.provider.PolicyParser.EXTDIRS_EXPANSION, 0);
                if (extCodebases != null && extCodebases.length > 0) {
                    for (int i = 0; i < extCodebases.length; i++) {
                        try {
                            ...
                            pe.add(ullink.security.util.SecurityConstants.ALL_PERMISSION);
     ...
     */
    private CodeSource getCodeSource(ullink.security.provider.PolicyParser.GrantEntry ge, KeyStore keyStore,
     ...
     */
    private void addGrantEntry(ullink.security.provider.PolicyParser.GrantEntry ge,
        ...
        if (debug != null) {
            ...
            if (ge.principals != null && ge.principals.size() > 0) {
                ListIterator<ullink.security.provider.PolicyParser.PrincipalEntry> li =
                                                ge.principals.listIterator();
                while (li.hasNext()) {
                    ullink.security.provider.PolicyParser.PrincipalEntry pppe = li.next();
        ...
        try {
            ...
            Enumeration<ullink.security.provider.PolicyParser.PermissionEntry> enum_ =
                                                ge.permissionElements();
            while (enum_.hasMoreElements()) {
                ullink.security.provider.PolicyParser.PermissionEntry pe = enum_.nextElement();
                ...
                try {
                    ...
                    MessageFormat form = new MessageFormat
                        (ullink.security.util.ResourcesMgr.getString
                                (POLICY +
    ...
    private static final Permission getKnownInstance(Class claz,
        ...
        if (claz.equals(FilePermission.class)) {
            ...
            return ullink.security.util.SecurityConstants.ALL_PERMISSION;
...
*/
            ...
            if (certs != null) {
                for (int k=0; k < certs.length; k++) {
                    ...
                    if (idMap == null &&
                        ...
                        perms.add(ullink.security.util.SecurityConstants.ALL_PERMISSION);
                    }
                }
            }
        }
    ...
    private void addPermissions(Permissions perms,
        ...
        List<ullink.security.provider.PolicyParser.PrincipalEntry> entryPs = entry.getPrincipals();
        if (debug != null) {
            ArrayList<ullink.security.provider.PolicyParser.PrincipalEntry> accPs =
                        new ArrayList<ullink.security.provider.PolicyParser.PrincipalEntry>();
            if (principals != null) {
                for (int i = 0; i < principals.length; i++) {
                    accPs.add(new ullink.security.provider.PolicyParser.PrincipalEntry
        ...
        for (int i = 0; i < entryPs.size(); i++) {
            ullink.security.provider.PolicyParser.PrincipalEntry pppe = entryPs.get(i);
    ...
    private boolean checkEntryPs(Principal[] pList,
                                ullink.security.provider.PolicyParser.PrincipalEntry pppe) {

        for (int i = 0; i < pList.length; i++) {

            if (pppe.principalClass.equals
                        (ullink.security.provider.PolicyParser.PrincipalEntry.WILDCARD_CLASS) ||
                pppe.principalClass.equals
                        (pList[i].getClass().getName())) {

                if (pppe.principalName.equals
                        (ullink.security.provider.PolicyParser.PrincipalEntry.WILDCARD_NAME) ||
    ...
    private void expandSelf(SelfPermission sp,
                            List<ullink.security.provider.PolicyParser.PrincipalEntry> entryPs,
                            Principal[] pdp,
                            Permissions perms) {
        ...
        while ((v = sp.getSelfName().indexOf(SELF, startIndex)) != -1) {
            ...
            // expand SELF
            ListIterator<ullink.security.provider.PolicyParser.PrincipalEntry> pli =
                                                entryPs.listIterator();
            while (pli.hasNext()) {
                ullink.security.provider.PolicyParser.PrincipalEntry pppe = pli.next();
    ...
    private String[][] getPrincipalInfo
        (ullink.security.provider.PolicyParser.PrincipalEntry pe, Principal[] pdp) {
    ...
    private boolean replacePrincipals(
        List<ullink.security.provider.PolicyParser.PrincipalEntry> principals, KeyStore keystore) {

        if (principals == null || principals.size() == 0 || keystore == null)
            return true;

        ListIterator<ullink.security.provider.PolicyParser.PrincipalEntry> i = principals.listIterator();
        while (i.hasNext()) {
            ullink.security.provider.PolicyParser.PrincipalEntry pppe = i.next();
            if (pppe.principalClass.equals(ullink.security.provider.PolicyParser.REPLACE_NAME)) {
    ...
    private void expandPermissionName(ullink.security.provider.PolicyParser.PermissionEntry pe,
        ...
        PolicyInfo myInfo)
    {
        ...
        synchronized(PolicyFile.class) {
            if (scope == null) {
                ...
                if (is instanceof IdentityDatabase) {
                    scope = is;
                } else {
                    // leave scope null
                }
    ...
    private static boolean isTrusted(Identity id) {
            ...
            } else if (id instanceof ullink.security.provider.SystemSigner) {
                ullink.security.provider.SystemSigner sysid = (ullink.security.provider.SystemSigner)id;
                if (sysid.isTrusted()) {
                    return true;
                }
            }
    ...
    private static class PolicyEntry {
        ...
        private final List<ullink.security.provider.PolicyParser.PrincipalEntry> principals;
         ...
         *
         */
        PolicyEntry(CodeSource cs, List<ullink.security.provider.PolicyParser.PrincipalEntry> principals)
        {
            this.codesource = cs;
        ...
        List<ullink.security.provider.PolicyParser.PrincipalEntry> getPrincipals() {
        ...
        @Override public String toString(){
            StringBuilder sb = new StringBuilder();
            sb.append(ullink.security.util.ResourcesMgr.getString("("));
                              ...
                              Certificate certs[])
        {
            ...
            if (type == null) {
                throw new NullPointerException
                    (ullink.security.util.ResourcesMgr.getString("type can't be null"));
            }
            this.type = type;
            this.name = name;
