...
class PKIXMasterCertPathValidator {
        ...
        for (int i = 0; i < cpSize; i++) {
            ...
            for (int j = 0; j < certPathCheckers.size(); j++) {
                ...
                try {
                    ...
                    if (ocspCause != null &&
                        currChecker instanceof ullink.security.provider.certpath.CrlRevocationChecker) {
                        throw ocspCause;
                    }
                    /*
    ...
    private static boolean isRevocationCheck(PKIXCertPathChecker checker,
        ...
        if (checker instanceof ullink.security.provider.certpath.OCSPChecker && index + 1 < checkers.size()) {
            PKIXCertPathChecker nextChecker = checkers.get(index + 1);
            if (nextChecker instanceof ullink.security.provider.certpath.CrlRevocationChecker) {
                return true;
            }
        }
        return false;
