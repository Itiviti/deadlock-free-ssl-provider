...
public class PKIXCertPathValidator extends CertPathValidatorSpi {
    ...
    private ullink.security.provider.certpath.BasicChecker basicChecker;
        ...
        for (TrustAnchor anchor : anchors) {
            ...
            try {
                PolicyNodeImpl rootNode = new PolicyNodeImpl(null,
                    ullink.security.provider.certpath.PolicyChecker.ANY_POLICY, null, false,
                    Collections.singleton(ullink.security.provider.certpath.PolicyChecker.ANY_POLICY), false);
        ...
        if (pkixParam.isRevocationEnabled()) {
            ...
            ocspEnabled = AccessController.doPrivileged(
                new GetBooleanSecurityPropertyAction
                    (ullink.security.provider.certpath.OCSPChecker.OCSP_ENABLE_PROP));
            ...
            PolicyNodeImpl rootNode) throws CertPathValidatorException
    {
        ...
        basicChecker = new ullink.security.provider.certpath.BasicChecker(anchor, testDate, sigProvider, false);
        ullink.security.provider.certpath.AlgorithmChecker algorithmChecker= ullink.security.provider.certpath.AlgorithmChecker.getInstance();
        ullink.security.provider.certpath.KeyChecker keyChecker = new ullink.security.provider.certpath.KeyChecker(certPathLen,
            pkixParam.getTargetCertConstraints());
        ullink.security.provider.certpath.ConstraintsChecker constraintsChecker =
            new ullink.security.provider.certpath.ConstraintsChecker(certPathLen);

        ullink.security.provider.certpath.PolicyChecker policyChecker =
            new ullink.security.provider.certpath.PolicyChecker(pkixParam.getInitialPolicies(), certPathLen,
                              ...
                              rootNode);
        ullink.security.provider.certpath.UntrustedChecker untrustedChecker = new ullink.security.provider.certpath.UntrustedChecker();
        ...
        if (pkixParam.isRevocationEnabled()) {
            ...
            if (ocspEnabled) {
                ullink.security.provider.certpath.OCSPChecker ocspChecker =
                    new ullink.security.provider.certpath.OCSPChecker(cpOriginal, pkixParam, onlyEECert);
                certPathCheckers.add(ocspChecker);
            }

            // Always use CRLs
            ullink.security.provider.certpath.CrlRevocationChecker revocationChecker = new
                    ullink.security.provider.certpath.CrlRevocationChecker(anchor, pkixParam, certList, onlyEECert);
        ...
        ullink.security.provider.certpath.PKIXMasterCertPathValidator masterValidator =
            new ullink.security.provider.certpath.PKIXMasterCertPathValidator(certPathCheckers);
