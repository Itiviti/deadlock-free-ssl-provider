...
import ullink.security.action.GetBooleanAction;
...
class DistributionPointFetcher {
        ...
        boolean[] reasonsMask, TrustAnchor anchor) throws CertStoreException
    {
        ...
        try {
            ullink.security.x509.X509CertImpl certImpl = ullink.security.x509.X509CertImpl.toImpl(cert);
            if (debug != null) {
                debug.println("DistributionPointFetcher.getCRLs: Checking "
                        + "CRLDPs for " + certImpl.getSubjectX500Principal());
            }
            ullink.security.x509.CRLDistributionPointsExtension ext =
            ...
            List<ullink.security.x509.DistributionPoint> points = (List<ullink.security.x509.DistributionPoint>)ext.get(
                                        ullink.security.x509.CRLDistributionPointsExtension.POINTS);
            Set<X509CRL> results = new HashSet<X509CRL>();
            for (Iterator<ullink.security.x509.DistributionPoint> t = points.iterator();
                 t.hasNext() && !Arrays.equals(reasonsMask, ALL_REASONS); ) {
                ullink.security.x509.DistributionPoint point = t.next();
    ...
    private Collection<X509CRL> getCRLs(X509CRLSelector selector,
        ullink.security.x509.X509CertImpl certImpl, ullink.security.x509.DistributionPoint point, boolean[] reasonsMask,
        boolean signFlag, PublicKey prevKey, String provider,
        List<CertStore> certStores, TrustAnchor anchor)
    {
        // check for full name
        ullink.security.x509.GeneralNames fullName = point.getFullName();
        if (fullName == null) {
            // check for relative name
            ullink.security.x509.RDN relativeName = point.getRelativeName();
            if (relativeName == null) {
                return Collections.emptySet();
            }
            try {
                ullink.security.x509.GeneralNames crlIssuers = point.getCRLIssuer();
                if (crlIssuers == null) {
                    fullName = getFullNames
                        ((ullink.security.x509.X500Name) certImpl.getIssuerDN(), relativeName);
                } else {
                    ...
                    if (crlIssuers.size() != 1) {
                        ...
                        fullName = getFullNames
                            ((ullink.security.x509.X500Name) crlIssuers.get(0).getName(), relativeName);
                    }
                }
        ...
        for (Iterator<ullink.security.x509.GeneralName> t = fullName.iterator(); t.hasNext(); ) {
            ullink.security.x509.GeneralName name = t.next();
            if (name.getType() == ullink.security.x509.GeneralNameInterface.NAME_DIRECTORY) {
                ullink.security.x509.X500Name x500Name = (ullink.security.x509.X500Name) name.getName();
                possibleCRLs.addAll(
                    getCRLs(x500Name, certImpl.getIssuerX500Principal(),
                            certStores));
            } else if (name.getType() == ullink.security.x509.GeneralNameInterface.NAME_URI) {
                ullink.security.x509.URIName uriName = (ullink.security.x509.URIName)name.getName();
     ...
     */
    private X509CRL getCRL(ullink.security.x509.URIName name) {
        URI uri = name.getURI();
        if (debug != null) {
     ...
     */
    private Collection<X509CRL> getCRLs(ullink.security.x509.X500Name name,
     ...
     */
    boolean verifyCRL(ullink.security.x509.X509CertImpl certImpl, ullink.security.x509.DistributionPoint point,
        ...
        ullink.security.x509.X509CRLImpl crlImpl = ullink.security.x509.X509CRLImpl.toImpl(crl);
        ullink.security.x509.IssuingDistributionPointExtension idpExt =
            crlImpl.getIssuingDistributionPointExtension();
        ullink.security.x509.X500Name certIssuer = (ullink.security.x509.X500Name) certImpl.getIssuerDN();
        ullink.security.x509.X500Name crlIssuer = (ullink.security.x509.X500Name) crlImpl.getIssuerDN();
        ...
        // certificate issuer.
        ullink.security.x509.GeneralNames pointCrlIssuers = point.getCRLIssuer();
        ullink.security.x509.X500Name pointCrlIssuer = null;
        if (pointCrlIssuers != null) {
            if (idpExt == null ||
                ((Boolean) idpExt.get
                    (ullink.security.x509.IssuingDistributionPointExtension.INDIRECT_CRL)).equals
                        (Boolean.FALSE)) {
                return false;
            }
            boolean match = false;
            for (Iterator<ullink.security.x509.GeneralName> t = pointCrlIssuers.iterator();
                 !match && t.hasNext(); ) {
                ullink.security.x509.GeneralNameInterface name = t.next().getName();
                if (crlIssuer.equals(name) == true) {
                    pointCrlIssuer = (ullink.security.x509.X500Name) name;
                    match = true;
                }
            }
            if (match == false) {
        ...
        if (idpExt != null) {
            ullink.security.x509.DistributionPointName idpPoint = (ullink.security.x509.DistributionPointName)
                idpExt.get(ullink.security.x509.IssuingDistributionPointExtension.POINT);
            if (idpPoint != null) {
                ullink.security.x509.GeneralNames idpNames = idpPoint.getFullName();
                if (idpNames == null) {
                    ullink.security.x509.RDN relativeName = idpPoint.getRelativeName();
                    if (relativeName == null) {
                        if (debug != null) {
                ...
                if (point.getFullName() != null ||
                    ...
                    ullink.security.x509.GeneralNames pointNames = point.getFullName();
                    if (pointNames == null) {
                        ullink.security.x509.RDN relativeName = point.getRelativeName();
                        if (relativeName == null) {
                            if (debug != null) {
                    ...
                    boolean match = false;
                    for (Iterator<ullink.security.x509.GeneralName> i = idpNames.iterator();
                         !match && i.hasNext(); ) {
                        ullink.security.x509.GeneralNameInterface idpName = i.next().getName();
                        if (debug != null) {
                            debug.println("idpName: " + idpName);
                        }
                        for (Iterator<ullink.security.x509.GeneralName> p = pointNames.iterator();
                             !match && p.hasNext(); ) {
                            ullink.security.x509.GeneralNameInterface pointName = p.next().getName();
                            if (debug != null) {
                ...
                // field of the DP
                    ...
                    for (Iterator<ullink.security.x509.GeneralName> t = pointCrlIssuers.iterator();
                         !match && t.hasNext(); ) {
                        ullink.security.x509.GeneralNameInterface crlIssuerName = t.next().getName();
                        for (Iterator<ullink.security.x509.GeneralName> i = idpNames.iterator();
                             !match && i.hasNext(); ) {
                            ullink.security.x509.GeneralNameInterface idpName = i.next().getName();
            ...
            Boolean b = (Boolean)
                idpExt.get(ullink.security.x509.IssuingDistributionPointExtension.ONLY_USER_CERTS);
            ...
            // cert is a CA cert
            b = (Boolean)
                idpExt.get(ullink.security.x509.IssuingDistributionPointExtension.ONLY_CA_CERTS);
            if (b.equals(Boolean.TRUE) && certImpl.getBasicConstraints() == -1) {
                if (debug != null) {
            ...
            b = (Boolean) idpExt.get
                (ullink.security.x509.IssuingDistributionPointExtension.ONLY_ATTRIBUTE_CERTS);
        ...
        ullink.security.x509.ReasonFlags reasons = null;
        if (idpExt != null) {
            reasons = (ullink.security.x509.ReasonFlags)
                idpExt.get(ullink.security.x509.IssuingDistributionPointExtension.REASONS);
        }
        ...
        if (unresCritExts != null) {
            unresCritExts.remove
                (ullink.security.x509.PKIXExtensions.IssuingDistributionPoint_Id.toString());
     ...
     * GeneralNames object.
     */
    private ullink.security.x509.GeneralNames getFullNames(ullink.security.x509.X500Name issuer, ullink.security.x509.RDN rdn)
        throws IOException {
        List<ullink.security.x509.RDN> rdns = new ArrayList<ullink.security.x509.RDN>(issuer.rdns());
        rdns.add(rdn);
        ullink.security.x509.X500Name fullName = new ullink.security.x509.X500Name(rdns.toArray(new ullink.security.x509.RDN[0]));
        ullink.security.x509.GeneralNames fullNames = new ullink.security.x509.GeneralNames();
        fullNames.add(new ullink.security.x509.GeneralName(fullName));
        return fullNames;
    }
}
