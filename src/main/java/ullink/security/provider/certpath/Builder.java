...
public abstract class Builder {
     ...
     */
    static int distance(ullink.security.x509.GeneralNameInterface base,
        ullink.security.x509.GeneralNameInterface test, int incomparable) {
     ...
     */
    static int hops(ullink.security.x509.GeneralNameInterface base, ullink.security.x509.GeneralNameInterface test,
        int incomparable) {
        ...
        if (base.getType() != ullink.security.x509.GeneralNameInterface.NAME_DIRECTORY) {
            if (debug != null) {
        ...
        ullink.security.x509.X500Name baseName = (ullink.security.x509.X500Name)base;
        ullink.security.x509.X500Name testName = (ullink.security.x509.X500Name)test;
        ullink.security.x509.X500Name commonName = baseName.commonAncestor(testName);
        if (commonName == null) {
            if (debug != null) {
     ...
     */
    static int targetDistance(ullink.security.x509.NameConstraintsExtension constraints,
            X509Certificate cert, ullink.security.x509.GeneralNameInterface target)
            throws IOException {
        ...
        ullink.security.x509.X509CertImpl certImpl;
        try {
            certImpl = ullink.security.x509.X509CertImpl.toImpl(cert);
        } catch (CertificateException e) {
            throw (IOException)new IOException("Invalid certificate").initCause(e);
        }
        /* see if certificate subject matches target */
        ullink.security.x509.X500Name subject = ullink.security.x509.X500Name.asX500Name(certImpl.getSubjectX500Principal());
        ...
        if (altNameExt != null) {
            ullink.security.x509.GeneralNames altNames =
                (ullink.security.x509.GeneralNames)altNameExt.get(altNameExt.SUBJECT_NAME);
            /* see if any alternative name matches target */
            if (altNames != null) {
                for (int j = 0, n = altNames.size(); j < n; j++) {
                    ullink.security.x509.GeneralNameInterface altName = altNames.get(j).getName();
        ...
        ullink.security.x509.NameConstraintsExtension ncExt = certImpl.getNameConstraintsExtension();
        if (ncExt == null) {
            return -1;
        }
        ...
        if (constraints != null) {
            ...
            constraints = (ullink.security.x509.NameConstraintsExtension) ncExt.clone();
        }
        ...
        ullink.security.x509.GeneralSubtrees permitted = (ullink.security.x509.GeneralSubtrees)
            constraints.get(constraints.PERMITTED_SUBTREES);
        ullink.security.x509.GeneralSubtrees excluded = (ullink.security.x509.GeneralSubtrees)
        ...
        for (int i = 0, n = permitted.size(); i < n; i++) {
            ullink.security.x509.GeneralNameInterface perName = permitted.get(i).getName().getName();
    ...
    Set<String> getMatchingPolicies() {
        if (matchingPolicies != null) {
            ...
            if ((!initialPolicies.isEmpty()) &&
                (!initialPolicies.contains(ullink.security.provider.certpath.PolicyChecker.ANY_POLICY)) &&
                (buildParams.isPolicyMappingInhibited()))
            {
                initialPolicies.add(ullink.security.provider.certpath.PolicyChecker.ANY_POLICY);
    ...
    boolean addMatchingCerts(X509CertSelector selector,
        ...
        if (targetCert != null) {
            ...
            if (selector.match(targetCert) && !ullink.security.x509.X509CertImpl.isSelfSigned
        ...
        for (CertStore store : certStores) {
            try {
                ...
                for (Certificate cert : certs) {
                    if (!ullink.security.x509.X509CertImpl.isSelfSigned
