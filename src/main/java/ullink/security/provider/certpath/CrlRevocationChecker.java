...
class CrlRevocationChecker extends PKIXCertPathChecker {
        ...
        Set<X509Certificate> stackedCerts) throws CertPathValidatorException
    {
        ...
        try {
            ...
            ullink.security.provider.certpath.DistributionPointFetcher store =
                ullink.security.provider.certpath.DistributionPointFetcher.getInstance();
        ...
        int reasonCode = 0;
        ullink.security.x509.X509CRLEntryImpl entry = null;
        for (X509CRL crl : mApprovedCRLs) {
            X509CRLEntry e = crl.getRevokedCertificate(currCert);
            if (e != null) {
                try {
                    entry = ullink.security.x509.X509CRLEntryImpl.toImpl(e);
                    ...
                    reasonCode = (reason == null
                        ? ullink.security.x509.CRLReasonCodeExtension.UNSPECIFIED
                        : reason.intValue());
                } catch (Exception ex) {
                ...
                if (unresCritExts != null && !unresCritExts.isEmpty()) {
                    ...
                    unresCritExts.remove
                        (ullink.security.x509.PKIXExtensions.ReasonCode_Id.toString());
                    unresCritExts.remove
                        (ullink.security.x509.PKIXExtensions.CertificateIssuer_Id.toString());
    ...
    private void buildToNewKey(X509Certificate currCert,
        ...
        if (ullink.security.provider.certpath.Builder.USE_AIA == true) {
            ...
            ullink.security.x509.AuthorityInfoAccessExtension aiaExt = null;
    ...
    private static String reasonToString(int reasonCode) {
        switch (reasonCode) {
            case ullink.security.x509.CRLReasonCodeExtension.UNSPECIFIED:
              return "unspecified";
            case ullink.security.x509.CRLReasonCodeExtension.KEY_COMPROMISE:
              return "key compromise";
            case ullink.security.x509.CRLReasonCodeExtension.CA_COMPROMISE:
              return "CA compromise";
            case ullink.security.x509.CRLReasonCodeExtension.AFFLIATION_CHANGED:
              return "affiliation changed";
            case ullink.security.x509.CRLReasonCodeExtension.SUPERSEDED:
              return "superseded";
            case ullink.security.x509.CRLReasonCodeExtension.CESSATION_OF_OPERATION:
              return "cessation of operation";
            case ullink.security.x509.CRLReasonCodeExtension.CERTIFICATE_HOLD:
              return "certificate hold";
            case ullink.security.x509.CRLReasonCodeExtension.REMOVE_FROM_CRL:
        ...
        boolean[] reasonsMask) throws CertPathValidatorException
    {
        try {
            ...
            List<ullink.security.x509.DistributionPoint> points = null;
            if (ext == null) {
                ...
                ullink.security.x509.DistributionPoint point = new ullink.security.x509.DistributionPoint
                    (new ullink.security.x509.GeneralNames().add(new ullink.security.x509.GeneralName(certIssuer)),
                     null, null);
                points = Collections.singletonList(point);
            } else {
                points = (List<ullink.security.x509.DistributionPoint>)ext.get(
                                        ullink.security.x509.CRLDistributionPointsExtension.POINTS);
            }
            Set<X509CRL> results = new HashSet<X509CRL>();
            ullink.security.provider.certpath.DistributionPointFetcher dpf =
                ullink.security.provider.certpath.DistributionPointFetcher.getInstance();
