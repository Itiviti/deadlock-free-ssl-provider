...
class PolicyChecker extends PKIXCertPathChecker {
    ...
    public Set<String> getSupportedExtensions() {
        if (supportedExts == null) {
            ...
            supportedExts.add(ullink.security.x509.PKIXExtensions.CertificatePolicies_Id.toString());
            supportedExts.add(ullink.security.x509.PKIXExtensions.PolicyMappings_Id.toString());
            supportedExts.add(ullink.security.x509.PKIXExtensions.PolicyConstraints_Id.toString());
            supportedExts.add(ullink.security.x509.PKIXExtensions.InhibitAnyPolicy_Id.toString());
        ...
        throws CertPathValidatorException
    {
        ...
        if (unresCritExts != null && !unresCritExts.isEmpty()) {
            unresCritExts.remove(ullink.security.x509.PKIXExtensions.CertificatePolicies_Id.toString());
            unresCritExts.remove(ullink.security.x509.PKIXExtensions.PolicyMappings_Id.toString());
            unresCritExts.remove(ullink.security.x509.PKIXExtensions.PolicyConstraints_Id.toString());
            unresCritExts.remove(ullink.security.x509.PKIXExtensions.InhibitAnyPolicy_Id.toString());
        }
    }
        ...
        throws CertPathValidatorException
    {
        ...
        try {
            currCertImpl = ullink.security.x509.X509CertImpl.toImpl(currCert);
     ...
     * occurs
     */
    static int mergeExplicitPolicy(int explicitPolicy, ullink.security.x509.X509CertImpl currCert,
        boolean finalCert) throws CertPathValidatorException
    {
        if ((explicitPolicy > 0) && !ullink.security.x509.X509CertImpl.isSelfIssued(currCert)) {
            explicitPolicy--;
        }

        try {
            ullink.security.x509.PolicyConstraintsExtension polConstExt
                = currCert.getPolicyConstraintsExtension();
            if (polConstExt == null)
                return explicitPolicy;
            int require = ((Integer)
                polConstExt.get(ullink.security.x509.PolicyConstraintsExtension.REQUIRE)).intValue();
            if (debug != null) {
     ...
     * occurs
     */
    static int mergePolicyMapping(int policyMapping, ullink.security.x509.X509CertImpl currCert)
        throws CertPathValidatorException
    {
        if ((policyMapping > 0) && !ullink.security.x509.X509CertImpl.isSelfIssued(currCert)) {
            policyMapping--;
        }
        ...
        try {
            ...
            int inhibit = ((Integer)
                polConstExt.get(ullink.security.x509.PolicyConstraintsExtension.INHIBIT)).intValue();
            if (debug != null)
    ...
    static int mergeInhibitAnyPolicy(int inhibitAnyPolicy,
        ullink.security.x509.X509CertImpl currCert) throws CertPathValidatorException
    {
        if ((inhibitAnyPolicy > 0) && !ullink.security.x509.X509CertImpl.isSelfIssued(currCert)) {
            inhibitAnyPolicy--;
        }

        try {
            ullink.security.x509.InhibitAnyPolicyExtension inhAnyPolExt = (ullink.security.x509.InhibitAnyPolicyExtension)
                currCert.getExtension(ullink.security.x509.PKIXExtensions.InhibitAnyPolicy_Id);
            if (inhAnyPolExt == null)
                return inhibitAnyPolicy;

            int skipCerts = ((Integer)
                inhAnyPolExt.get(ullink.security.x509.InhibitAnyPolicyExtension.SKIP_CERTS)).intValue();
            if (debug != null)
    ...
    static PolicyNodeImpl processPolicies(int certIndex, Set<String> initPolicies,
        ...
        ullink.security.x509.X509CertImpl currCert, boolean finalCert)
        throws CertPathValidatorException
    {
        boolean policiesCritical = false;
        List<ullink.security.x509.PolicyInformation> policyInfo;
        ...
        ullink.security.x509.CertificatePoliciesExtension currCertPolicies
        ...
        if ((currCertPolicies != null) && (rootNode != null)) {
            ...
            try {
                policyInfo = (List<ullink.security.x509.PolicyInformation>)
                    currCertPolicies.get(ullink.security.x509.CertificatePoliciesExtension.POLICIES);
            ...
            for (ullink.security.x509.PolicyInformation curPolInfo : policyInfo) {
                String curPolicy =
            ...
            if (foundAnyPolicy) {
                if ((inhibitAnyPolicy > 0) ||
                        (!finalCert && ullink.security.x509.X509CertImpl.isSelfIssued(currCert))) {
                    if (debug != null) {
                        debug.println("PolicyChecker.processPolicies() "
     ...
     */
    private static PolicyNodeImpl processPolicyMappings(ullink.security.x509.X509CertImpl currCert,
        int certIndex, int policyMapping, PolicyNodeImpl rootNode,
        boolean policiesCritical, Set<PolicyQualifierInfo> anyQuals)
        throws CertPathValidatorException
    {
        ullink.security.x509.PolicyMappingsExtension polMappingsExt
        ...
        List<ullink.security.x509.CertificatePolicyMap> maps = null;
        try {
            maps = (List<ullink.security.x509.CertificatePolicyMap>)polMappingsExt.get
                                        (ullink.security.x509.PolicyMappingsExtension.MAP);
        } catch (IOException e) {
            if (debug != null) {
                debug.println("PolicyChecker.processPolicyMappings() "
                    + "mapping exception");
        ...
        for (int j = 0; j < maps.size(); j++) {
            ullink.security.x509.CertificatePolicyMap polMap = maps.get(j);
            String issuerDomain
    ...
    private static PolicyNodeImpl removeInvalidNodes(PolicyNodeImpl rootNode,
        ...
        throws CertPathValidatorException
    {
        List<ullink.security.x509.PolicyInformation> policyInfo = null;
        try {
            policyInfo = (List<ullink.security.x509.PolicyInformation>)
                currCertPolicies.get(ullink.security.x509.CertificatePoliciesExtension.POLICIES);
        } catch (IOException ioe) {
