 ...
 * <p>
 * @see BuildStep
 * @see ullink.security.provider.certpath.Vertex
 * <p>
 * @author  seth proctor
 * @since   1.4
 */
public class AdjacencyList {
    ...
    private List<List<ullink.security.provider.certpath.Vertex>> mOrigList;
     ...
     */
    public AdjacencyList(List<List<ullink.security.provider.certpath.Vertex>> list) {
     ...
     * at the start.
     */
    private boolean buildList(List<List<ullink.security.provider.certpath.Vertex>> theList, int index,
        BuildStep follow) {
        ...
        List<ullink.security.provider.certpath.Vertex> l = theList.get(index);
        ...
        try {
            ...
            for (ullink.security.provider.certpath.Vertex v : l) {
                if (v.getIndex() != -1) {
            ...
            if (allNegOne) {
                ...
                if (allXcps) {
                    ...
                    List<ullink.security.provider.certpath.Vertex> possibles = new ArrayList<ullink.security.provider.certpath.Vertex>();
                    for (ullink.security.provider.certpath.Vertex v : l) {
                        if (v.getThrowable() == null)
                            possibles.add(v);
                    }
    ...
    public String toString() {
        ...
        int i = 0;
        for (List<ullink.security.provider.certpath.Vertex> l : mOrigList) {
            out = out + "LinkedList[" + i++ + "]:\n";

            for (ullink.security.provider.certpath.Vertex step : l) {
                try {
