 ...
 * @author      Sean Mullan
 */
class ForwardBuilder extends ullink.security.provider.certpath.Builder {
    ...
    Collection<X509Certificate> getMatchingCerts
        (ullink.security.provider.certpath.State currentState, List<CertStore> certStores)
        throws CertStoreException, CertificateException, IOException
    {
        ...
        ullink.security.provider.certpath.ForwardState currState = (ullink.security.provider.certpath.ForwardState) currentState;
     ...
     */
    private void getMatchingEECerts(ullink.security.provider.certpath.ForwardState currentState,
     ...
     * and requirements specified in the parameters and PKIX state.
     */
    private void getMatchingCACerts(ullink.security.provider.certpath.ForwardState currentState,
        ...
        if (!currentState.isInitial() && ullink.security.provider.certpath.Builder.USE_AIA) {
            // check for AuthorityInformationAccess extension
            ullink.security.x509.AuthorityInfoAccessExtension aiaExt =
     ...
     */
    private boolean getCerts(ullink.security.x509.AuthorityInfoAccessExtension aiaExt,
        Collection<X509Certificate> certs) {
        if (ullink.security.provider.certpath.Builder.USE_AIA == false) {
            return false;
        }
        List<ullink.security.x509.AccessDescription> adList = aiaExt.getAccessDescriptions();
        if (adList == null || adList.isEmpty()) {
            return false;
        }

        boolean add = false;
        for (ullink.security.x509.AccessDescription ad : adList) {
    ...
    static class PKIXCertComparator implements Comparator<X509Certificate> {
        ...
        public int compare(X509Certificate oCert1, X509Certificate oCert2) {
            ...
            ullink.security.x509.X500Name cIssuer1Name = ullink.security.x509.X500Name.asX500Name(cIssuer1);
            ullink.security.x509.X500Name cIssuer2Name = ullink.security.x509.X500Name.asX500Name(cIssuer2);
            ...
            for (X500Principal tSubject : trustedSubjectDNs) {
                ullink.security.x509.X500Name tSubjectName = ullink.security.x509.X500Name.asX500Name(tSubject);
                int distanceTto1 =
                    ullink.security.provider.certpath.Builder.distance(tSubjectName, cIssuer1Name, -1);
                int distanceTto2 =
                    ullink.security.provider.certpath.Builder.distance(tSubjectName, cIssuer2Name, -1);
                if (debug != null) {
            ...
            for (X500Principal tSubject : trustedSubjectDNs) {
                ...
                int distanceTto1 = ullink.security.provider.certpath.Builder.distance
                        (tSubjectName, cIssuer1Name, Integer.MAX_VALUE);
                int distanceTto2 = ullink.security.provider.certpath.Builder.distance
            ...
            for (X500Principal tSubject : trustedSubjectDNs) {
                ullink.security.x509.X500Name tSubjectName = ullink.security.x509.X500Name.asX500Name(tSubject);
                ullink.security.x509.X500Name tAo1 = tSubjectName.commonAncestor(cIssuer1Name);
                ullink.security.x509.X500Name tAo2 = tSubjectName.commonAncestor(cIssuer2Name);
                if (debug != null) {
                ...
                if (tAo1 != null || tAo2 != null) {
                    if (tAo1 != null && tAo2 != null) {
                        int hopsTto1 = ullink.security.provider.certpath.Builder.hops
                                (tSubjectName, cIssuer1Name, Integer.MAX_VALUE);
                        int hopsTto2 = ullink.security.provider.certpath.Builder.hops
                                (tSubjectName, cIssuer2Name, Integer.MAX_VALUE);
                        if (debug != null) {
            ...
            ullink.security.x509.X500Name cSubject1Name = ullink.security.x509.X500Name.asX500Name(cSubject1);
            ullink.security.x509.X500Name cSubject2Name = ullink.security.x509.X500Name.asX500Name(cSubject2);

            if (debug != null) {
                debug.println(METHOD_NME + " o1 Subject: " + cSubject1);
                debug.println(METHOD_NME + " o2 Subject: " + cSubject2);
            }
            int distanceStoI1 = ullink.security.provider.certpath.Builder.distance
                    (cSubject1Name, cIssuer1Name, Integer.MAX_VALUE);
            int distanceStoI2 = ullink.security.provider.certpath.Builder.distance
     ...
     */
    void verifyCert(X509Certificate cert, ullink.security.provider.certpath.State currentState,
        List<X509Certificate> certPathList) throws GeneralSecurityException
    {
        ...
        if (certPathList != null) {
            ...
            for (X509Certificate cpListCert : certPathList) {
                ullink.security.x509.X509CertImpl cpListCertImpl = ullink.security.x509.X509CertImpl.toImpl(cpListCert);
                ullink.security.x509.PolicyMappingsExtension policyMappingsExt
        ...
        if (!isTrustedCert) {
             ...
             */
            ullink.security.provider.certpath.AlgorithmChecker.check(cert);
            ...
            if (!unresCritExts.isEmpty()) {
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.BasicConstraints_Id.toString());
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.NameConstraints_Id.toString());
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.CertificatePolicies_Id.toString());
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.PolicyMappings_Id.toString());
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.PolicyConstraints_Id.toString());
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.InhibitAnyPolicy_Id.toString());
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.SubjectAlternativeName_Id.toString());
                unresCritExts.remove(ullink.security.x509.PKIXExtensions.KeyUsage_Id.toString());
                unresCritExts.remove(
                    ullink.security.x509.PKIXExtensions.ExtendedKeyUsage_Id.toString());
        ...
        if (!isTrustedCert) {
             ...
             */
            ullink.security.provider.certpath.KeyChecker.verifyCAKeyUsage(cert);
        }
        ...
        if (buildParams.isRevocationEnabled()) {
            ...
            if (ullink.security.provider.certpath.CrlRevocationChecker.certCanSignCrl(cert)) {
    ...
    boolean isPathCompleted(X509Certificate cert) {
        for (TrustAnchor anchor : trustAnchors) {
            ...
            if (buildParams.isRevocationEnabled()) {
                try {
                    ullink.security.provider.certpath.CrlRevocationChecker crlChecker = new ullink.security.provider.certpath.CrlRevocationChecker
