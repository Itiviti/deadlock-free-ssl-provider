...
import ullink.security.util.Cache;
import ullink.security.util.Debug;
import ullink.security.x509.AccessDescription;
import ullink.security.x509.GeneralNameInterface;
import ullink.security.x509.URIName;
...
class URICertStore extends CertStoreSpi {
    ...
    URICertStore(CertStoreParameters params)
        ...
        if (uri.getScheme().toLowerCase().equals("ldap")) {
            ldap = true;
            ldapCertStore =
                ullink.security.provider.certpath.LDAPCertStore.getInstance(ullink.security.provider.certpath.LDAPCertStore.getParameters(uri));
    ...
    public synchronized Collection<X509Certificate> engineGetCertificates
        ...
        if (ldap) {
            ...
            try {
                xsel = new ullink.security.provider.certpath.LDAPCertStore.LDAPCertSelector
    ...
    public synchronized Collection<X509CRL> engineGetCRLs(CRLSelector selector)
        ...
        if (ldap) {
            ...
            try {
                xsel = new ullink.security.provider.certpath.LDAPCertStore.LDAPCRLSelector(xsel, null, ldapPath);
            } catch (IOException ioe) {
                throw new CertStoreException(ioe);
            }
