...
import ullink.security.action.GetBooleanSecurityPropertyAction;
import ullink.security.util.Debug;
import ullink.security.x509.PKIXExtensions;
...
public final class SunCertPathBuilder extends CertPathBuilderSpi {
    ...
    public CertPathBuilderResult engineBuild(CertPathParameters params)
        ...
        List<List<ullink.security.provider.certpath.Vertex>> adjList = new ArrayList<List<ullink.security.provider.certpath.Vertex>>();
        ...
        if (result == null) {
            ...
            if (result == null) {
                throw new ullink.security.provider.certpath.SunCertPathBuilderException("unable to find valid "
                    + "certification path to requested target",
                    new ullink.security.provider.certpath.AdjacencyList(adjList));
            }
        }
        return result;
    }
    ...
    private CertPathBuilderResult buildCertPath(boolean buildForward,
        boolean searchAllCertStores, List<List<ullink.security.provider.certpath.Vertex>> adjList)
        ...
        try {
            if (pathCompleted) {
                ...
                return new ullink.security.provider.certpath.SunCertPathBuilderResult(
     ...
     *
     */
    private void buildReverse(List<List<ullink.security.provider.certpath.Vertex>> adjacencyList,
        LinkedList<X509Certificate> certPathList) throws Exception
    {
        ...
        ullink.security.provider.certpath.ReverseState currentState = new ullink.security.provider.certpath.ReverseState();
        /* Initialize adjacency list */
        adjacencyList.clear();
        adjacencyList.add(new LinkedList<ullink.security.provider.certpath.Vertex>());
        ...
        while (iter.hasNext()) {
            ...
            currentState.crlChecker =
                new ullink.security.provider.certpath.CrlRevocationChecker(null, buildParams, null, onlyEECert);
            currentState.untrustedChecker = new ullink.security.provider.certpath.UntrustedChecker();
            try {
                depthFirstSearchReverse(null, currentState,
                new ullink.security.provider.certpath.ReverseBuilder(buildParams, targetSubjectDN), adjacencyList,
                certPathList);
            } catch (Exception e) {
     ...
     */
    private void buildForward(List<List<ullink.security.provider.certpath.Vertex>> adjacencyList,
        ...
        throws GeneralSecurityException, IOException
    {
        ...
        /* Initialize current state */
        ullink.security.provider.certpath.ForwardState currentState = new ullink.security.provider.certpath.ForwardState();
        ...
        depthFirstSearchForward(targetSubjectDN, currentState,
          new ullink.security.provider.certpath.ForwardBuilder
     ...
     */
    void depthFirstSearchForward(X500Principal dN, ullink.security.provider.certpath.ForwardState currentState,
        ullink.security.provider.certpath.ForwardBuilder builder, List<List<ullink.security.provider.certpath.Vertex>> adjList,
        LinkedList<X509Certificate> certPathList)
        throws GeneralSecurityException, IOException
    {
         ...
         */
        List<ullink.security.provider.certpath.Vertex> vertices = addVertices
         ...
         */
               ...
               vertices:
        for (ullink.security.provider.certpath.Vertex vertex : vertices) {
            /**
             ...
             */
            ullink.security.provider.certpath.ForwardState nextState = (ullink.security.provider.certpath.ForwardState) currentState.clone();
            ...
            if (builder.isPathCompleted(cert)) {
                ...
                ullink.security.provider.certpath.BasicChecker basicChecker = null;
                if (debug != null)
                    debug.println("SunCertPathBuilder.depthFirstSearchForward()"
                ...
                initExpPolSet.add(ullink.security.provider.certpath.PolicyChecker.ANY_POLICY);

                ullink.security.provider.certpath.PolicyNodeImpl rootNode = new ullink.security.provider.certpath.PolicyNodeImpl(null,
                    ullink.security.provider.certpath.PolicyChecker.ANY_POLICY, null, false, initExpPolSet, false);

                ullink.security.provider.certpath.PolicyChecker policyChecker
                    = new ullink.security.provider.certpath.PolicyChecker(buildParams.getInitialPolicies(),
                                appendedCerts.size(),
                                buildParams.isExplicitPolicyRequired(),
                                buildParams.isPolicyMappingInhibited(),
                                buildParams.isAnyPolicyInhibited(),
                ...
                if (nextState.keyParamsNeeded()) {
                    ...
                    basicChecker = new ullink.security.provider.certpath.BasicChecker(anchor,
                                           builder.date,
                    ...
                    if (buildParams.isRevocationEnabled()) {
                        userCheckers.add(mustCheck, new ullink.security.provider.certpath.CrlRevocationChecker
             ...
             */
            adjList.add(new LinkedList<ullink.security.provider.certpath.Vertex>());
     ...
     * @param currentState the current PKIX validation state
     */
    void depthFirstSearchReverse(X500Principal dN, ullink.security.provider.certpath.ReverseState currentState,
        ullink.security.provider.certpath.ReverseBuilder builder, List<List<ullink.security.provider.certpath.Vertex>> adjList,
        LinkedList<X509Certificate> certPathList)
        throws GeneralSecurityException, IOException
    {
        if (debug != null)
        ...
        for (ullink.security.provider.certpath.Vertex vertex : vertices) {
             ...
             * the next matching cert is tried.
             */
            ullink.security.provider.certpath.ReverseState nextState = (ullink.security.provider.certpath.ReverseState) currentState.clone();
            X509Certificate cert = (X509Certificate) vertex.getCertificate();
            try {
                builder.verifyCert(cert, nextState, certPathList);
            } catch (GeneralSecurityException gse) {
            ...
            if (builder.isPathCompleted(cert)) {
                ...
                ullink.security.provider.certpath.PolicyNodeImpl rootNode = nextState.rootNode;

                if (rootNode == null)
                    policyTreeResult = null;
                else {
                    policyTreeResult = rootNode.copyTree();
                    ((ullink.security.provider.certpath.PolicyNodeImpl)policyTreeResult).setImmutable();
                }
                ...
                {
                    finalPublicKey =
                        ullink.security.provider.certpath.BasicChecker.makeInheritedParamsKey
     ...
     * adjacency list.
     */
    private List<ullink.security.provider.certpath.Vertex> addVertices(Collection<X509Certificate> certs,
        List<List<ullink.security.provider.certpath.Vertex>> adjList) {
        List<ullink.security.provider.certpath.Vertex> l = adjList.get(adjList.size() - 1);

        for (X509Certificate cert : certs) {
           ullink.security.provider.certpath.Vertex v = new ullink.security.provider.certpath.Vertex(cert);
           l.add(v);
        }
    ...
    private static class CertStoreComparator implements Comparator<CertStore> {
        public int compare(CertStore store1, CertStore store2) {
            if (ullink.security.provider.certpath.Builder.isLocalCertStore(store1)) {
                return -1;
            } else {
                return 1;
            }
