...
class EndEntityChecker {
    ...
    private final static String OID_EXTENDED_KEY_USAGE =
                                ullink.security.validator.SimpleValidator.OID_EXTENDED_KEY_USAGE;
    ...
    private void checkRemainingExtensions(Set<String> exts)
        ...
        exts.remove(ullink.security.validator.SimpleValidator.OID_BASIC_CONSTRAINTS);
    ...
    private void checkTLSClient(X509Certificate cert)
        ...
        if (checkKeyUsage(cert, KU_SIGNATURE) == false) {
            throw new ullink.security.validator.ValidatorException
                ...
                ullink.security.validator.ValidatorException.T_EE_EXTENSIONS, cert);
        }

        if (checkEKU(cert, exts, OID_EKU_TLS_CLIENT) == false) {
            throw new ullink.security.validator.ValidatorException("Extended key usage does not "
                + "permit use for TLS client authentication",
                ullink.security.validator.ValidatorException.T_EE_EXTENSIONS, cert);
        }

        if (!ullink.security.validator.SimpleValidator.getNetscapeCertTypeBit(cert, NSCT_SSL_CLIENT)) {
            throw new ullink.security.validator.ValidatorException
        ...
        exts.remove(ullink.security.validator.SimpleValidator.OID_KEY_USAGE);
        exts.remove(ullink.security.validator.SimpleValidator.OID_EXTENDED_KEY_USAGE);
        exts.remove(ullink.security.validator.SimpleValidator.OID_NETSCAPE_CERT_TYPE);
    ...
    private void checkTLSServer(X509Certificate cert, String parameter)
        ...
        if (!ullink.security.validator.SimpleValidator.getNetscapeCertTypeBit(cert, NSCT_SSL_SERVER)) {
            throw new ullink.security.validator.ValidatorException
    ...
    private void checkCodeSigning(X509Certificate cert)
        ...
        if (variant.equals(Validator.VAR_JCE_SIGNING) == false) {
            if (!ullink.security.validator.SimpleValidator.getNetscapeCertTypeBit(cert, NSCT_CODE_SIGNING)) {
                throw new ullink.security.validator.ValidatorException
