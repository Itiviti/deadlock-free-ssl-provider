...
package ullink.security.validator;

import ullink.security.provider.certpath.AlgorithmChecker;
import ullink.security.provider.certpath.UntrustedChecker;
...
import ullink.security.x509.NetscapeCertTypeExtension;
...
public final class SimpleValidator extends Validator {
    ...
    X509Certificate[] engineValidate(X509Certificate[] chain,
        ...
        for (int i = chain.length - 2; i >= 0; i--) {
            ...
            try {
                ...
                throw new ullink.security.validator.ValidatorException(
                    "Untrusted certificate: " + cert.getSubjectX500Principal(),
                    ullink.security.validator.ValidatorException.T_UNTRUSTED_CERT, cert, cpve);
            }
            ...
            try {
                ...
                throw new ullink.security.validator.ValidatorException
                        (ullink.security.validator.ValidatorException.T_ALGORITHM_DISABLED, cert, cpve);
            }
            ...
            if (cert.getIssuerX500Principal().equals(
                ...
                throw new ullink.security.validator.ValidatorException
                        (ullink.security.validator.ValidatorException.T_NAME_CHAINING, cert);
            }
            ...
            try {
                ...
                throw new ullink.security.validator.ValidatorException
                        (ullink.security.validator.ValidatorException.T_SIGNATURE_ERROR, cert, e);
            }
    ...
    private void checkExtensions(X509Certificate cert, int index)
        ...
        if (!critSet.isEmpty()) {
            throw new ullink.security.validator.ValidatorException
                ...
                ullink.security.validator.ValidatorException.T_CA_EXTENSIONS, cert);
        }
    }
    ...
    private void checkBasicConstraints(X509Certificate cert,
        ...
        if (constraints < 0) {
            throw new ullink.security.validator.ValidatorException("End user tried to act as a CA",
                ullink.security.validator.ValidatorException.T_CA_EXTENSIONS, cert);
        }
        if (index - 1 > constraints) {
            throw new ullink.security.validator.ValidatorException("Violated path length constraints",
                ullink.security.validator.ValidatorException.T_CA_EXTENSIONS, cert);
        }
    }
    ...
    private X509Certificate[] buildTrustedChain(X509Certificate[] chain)
        ...
        throw new ullink.security.validator.ValidatorException(ullink.security.validator.ValidatorException.T_NO_TRUST_ANCHOR);
    }
