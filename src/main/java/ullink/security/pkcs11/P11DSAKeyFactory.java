...
import static ullink.security.pkcs11.TemplateManager.O_IMPORT;
    ...
    PublicKey implTranslatePublicKey(PublicKey key) throws InvalidKeyException {
        try {
            if (key instanceof DSAPublicKey) {
                ...
                key = new ullink.security.provider.DSAPublicKey(encoded);
    ...
    PrivateKey implTranslatePrivateKey(PrivateKey key)
        ...
        try {
            if (key instanceof DSAPrivateKey) {
                ...
                // let Sun provider parse for us, then recurse
                byte[] encoded = key.getEncoded();
                key = new ullink.security.provider.DSAPrivateKey(encoded);
    ...
    protected PublicKey engineGeneratePublic(KeySpec keySpec)
        ...
        if (keySpec instanceof X509EncodedKeySpec) {
            try {
                ...
                PublicKey key = new ullink.security.provider.DSAPublicKey(encoded);
                return implTranslatePublicKey(key);
    ...
    protected PrivateKey engineGeneratePrivate(KeySpec keySpec)
        ...
        if (keySpec instanceof PKCS8EncodedKeySpec) {
            try {
                ...
                PrivateKey key = new ullink.security.provider.DSAPrivateKey(encoded);
                return implTranslatePrivateKey(key);
