    ...
    private static final class P11DSAPublicKey extends P11Key
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    Key key = new ullink.security.provider.DSAPublicKey
    ...
    private static final class P11DSAPrivateKey extends P11Key
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    Key key = new ullink.security.provider.DSAPrivateKey
    ...
    private static final class P11ECPrivateKey extends P11Key
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    Key key = new ECPrivateKeyImpl(s, params);
                    encoded = key.getEncoded();
                } catch (InvalidKeyException e) {
                    throw new ProviderException(e);
                }
    ...
    private static final class P11ECPublicKey extends P11Key
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    Key key = new ECPublicKeyImpl(w, params);
                    encoded = key.getEncoded();
                } catch (InvalidKeyException e) {
                    throw new ProviderException(e);
                }
