...
public class RDN {
    ...
    final ullink.security.x509.AVA[] assertion;

    // cached immutable List of the AVAs
    private volatile List<ullink.security.x509.AVA> avaList;
    ...
    public RDN(String name, Map<String, String> keywordMap) throws IOException {
        ...
        int searchOffset = 0;
        int avaOffset = 0;
        List<ullink.security.x509.AVA> avaVec = new ArrayList<ullink.security.x509.AVA>(3);
        ...
        while (nextPlus >= 0) {
            ...
            if (nextPlus > 0 && name.charAt(nextPlus - 1) != '\\'
                ...
                ullink.security.x509.AVA ava = new ullink.security.x509.AVA(new StringReader(avaString), keywordMap);
                avaVec.add(ava);
        ...
        assertion = avaVec.toArray(new ullink.security.x509.AVA[avaVec.size()]);
    }
    ...
    RDN(String name, String format, Map<String, String> keywordMap)
        ...
        while (nextPlus >= 0) {
            ...
            if (nextPlus > 0 && name.charAt(nextPlus - 1) != '\\' ) {
                ...
                // Parse AVA, and store it in vector
                ullink.security.x509.AVA ava = new ullink.security.x509.AVA
                    (new StringReader(avaString), ullink.security.x509.AVA.RFC2253, keywordMap);
                avaVec.add(ava);
        ...
        ullink.security.x509.AVA ava = new ullink.security.x509.AVA(new StringReader(avaString), ullink.security.x509.AVA.RFC2253, keywordMap);
        avaVec.add(ava);
    ...
    RDN(DerValue rdn) throws IOException {
        ...
        assertion = new ullink.security.x509.AVA[avaset.length];
        for (int i = 0; i < avaset.length; i++) {
            assertion[i] = new ullink.security.x509.AVA(avaset[i]);
        }
    }
     ...
     */
    RDN(int i) { assertion = new ullink.security.x509.AVA[i]; }

    public RDN(ullink.security.x509.AVA ava) {
        if (ava == null) {
            throw new NullPointerException();
        }
        assertion = new ullink.security.x509.AVA[] { ava };
    }

    public RDN(ullink.security.x509.AVA[] avas) {
     ...
     */
    public List<ullink.security.x509.AVA> avas() {
        List<ullink.security.x509.AVA> list = avaList;
        if (list == null) {
    ...
    private String toRFC2253StringInternal
        ...
        if (!canonical) {
            ...
            List<ullink.security.x509.AVA> avaList = new ArrayList<ullink.security.x509.AVA>(assertion.length);
            for (int i = 0; i < assertion.length; i++) {
...
class AVAComparator implements Comparator<ullink.security.x509.AVA> {

    private static final Comparator<ullink.security.x509.AVA> INSTANCE = new AVAComparator();

    private AVAComparator() {
        // empty
    }

    static Comparator<ullink.security.x509.AVA> getInstance() {
        return INSTANCE;
    }
     ...
     */
    public int compare(ullink.security.x509.AVA a1, ullink.security.x509.AVA a2) {
