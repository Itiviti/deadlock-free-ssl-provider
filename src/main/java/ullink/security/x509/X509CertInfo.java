...
public class X509CertInfo implements CertAttrSet<String> {
    ...
    public static final String VERSION = ullink.security.x509.CertificateVersion.NAME;
    public static final String SERIAL_NUMBER = ullink.security.x509.CertificateSerialNumber.NAME;
    public static final String ALGORITHM_ID = ullink.security.x509.CertificateAlgorithmId.NAME;
    public static final String ISSUER = ullink.security.x509.CertificateIssuerName.NAME;
    public static final String VALIDITY = ullink.security.x509.CertificateValidity.NAME;
    public static final String SUBJECT = ullink.security.x509.CertificateSubjectName.NAME;
    public static final String KEY = ullink.security.x509.CertificateX509Key.NAME;
    public static final String ISSUER_ID = ullink.security.x509.CertificateIssuerUniqueIdentity.NAME;
    public static final String SUBJECT_ID = ullink.security.x509.CertificateSubjectUniqueIdentity.NAME;
    public static final String EXTENSIONS = ullink.security.x509.CertificateExtensions.NAME;

    // X509.v1 data
    protected ullink.security.x509.CertificateVersion version = new ullink.security.x509.CertificateVersion();
    protected ullink.security.x509.CertificateSerialNumber serialNum = null;
    protected ullink.security.x509.CertificateAlgorithmId algId = null;
    protected ullink.security.x509.CertificateIssuerName issuer = null;
    protected ullink.security.x509.CertificateValidity interval = null;
    protected ullink.security.x509.CertificateSubjectName subject = null;
    protected ullink.security.x509.CertificateX509Key pubKey = null;

    // X509.v2 & v3 extensions
    protected ullink.security.x509.CertificateIssuerUniqueIdentity issuerUniqueId = null;
    protected ullink.security.x509.CertificateSubjectUniqueIdentity subjectUniqueId = null;

    // X509.v3 extensions
    protected ullink.security.x509.CertificateExtensions extensions = null;
    ...
    public String toString() {
        ...
        if (extensions != null) {
            ...
            for (int i = 0; i < objs.length; i++) {
                sb.append("\n[" + (i+1) + "]: ");
                ullink.security.x509.Extension ext = (ullink.security.x509.Extension)objs[i];
                try {
                    if (ullink.security.x509.OIDMap.getClass(ext.getExtensionId()) == null) {
            ...
            Map<String, ullink.security.x509.Extension> invalid = extensions.getUnparseableExtensions();
            if (invalid.isEmpty() == false) {
                sb.append("\nUnparseable certificate extensions: " + invalid.size());
                int i = 1;
                for (ullink.security.x509.Extension ext : invalid.values()) {
                    sb.append("\n[" + (i++) + "]: ");
                    sb.append(ext);
                }
            }
    ...
    throws CertificateException, IOException {
        ullink.security.x509.X509AttributeName attrName = new ullink.security.x509.X509AttributeName(name);
        ...
        case ATTR_EXTENSIONS:
            if (suffix == null) {
                ...
                if (extensions == null)
                    extensions = new ullink.security.x509.CertificateExtensions();
    ...
    throws CertificateParsingException, IOException {
        ...
        if (tmp.isContextSpecific((byte)0)) {
            version = new ullink.security.x509.CertificateVersion(tmp);
            tmp = in.getDerValue();
        }

        // Serial number ... an integer
        serialNum = new ullink.security.x509.CertificateSerialNumber(tmp);

        // Algorithm Identifier
        algId = new ullink.security.x509.CertificateAlgorithmId(in);

        // Issuer name
        issuer = new ullink.security.x509.CertificateIssuerName(in);
        ullink.security.x509.X500Name issuerDN = (ullink.security.x509.X500Name)issuer.get(ullink.security.x509.CertificateIssuerName.DN_NAME);
        if (issuerDN.isEmpty()) {
            throw new CertificateParsingException(
        ...
        interval = new ullink.security.x509.CertificateValidity(in);

        // subject name
        subject = new ullink.security.x509.CertificateSubjectName(in);
        ullink.security.x509.X500Name subjectDN = (ullink.security.x509.X500Name)subject.get(ullink.security.x509.CertificateSubjectName.DN_NAME);
        if ((version.compare(ullink.security.x509.CertificateVersion.V1) == 0) &&
                subjectDN.isEmpty()) {
            throw new CertificateParsingException(
        ...
        // public key
        pubKey = new ullink.security.x509.CertificateX509Key(in);
        ...
        if (tmp.isContextSpecific((byte)1)) {
            issuerUniqueId = new ullink.security.x509.CertificateIssuerUniqueIdentity(tmp);
            if (in.available() == 0)
                return;
            tmp = in.getDerValue();
        }
        ...
        if (tmp.isContextSpecific((byte)2)) {
            subjectUniqueId = new ullink.security.x509.CertificateSubjectUniqueIdentity(tmp);
            if (in.available() == 0)
                return;
            tmp = in.getDerValue();
        }
        ...
        // Get the extensions.
        if (version.compare(ullink.security.x509.CertificateVersion.V3) != 0) {
            throw new CertificateParsingException(
                      "Extensions not allowed in v2 certificate");
        }
        if (tmp.isConstructed() && tmp.isContextSpecific((byte)3)) {
            extensions = new ullink.security.x509.CertificateExtensions(tmp.data);
        }
     ...
     */
    private void verifyCert(ullink.security.x509.CertificateSubjectName subject,
        ...
        if (subjectDN.isEmpty()) {
            ...
            ullink.security.x509.SubjectAlternativeNameExtension subjectAltNameExt = null;
            ullink.security.x509.SubjectAlternativeNameExtension extValue = null;
            ullink.security.x509.GeneralNames names = null;
            try {
                subjectAltNameExt = (ullink.security.x509.SubjectAlternativeNameExtension)
                        extensions.get(ullink.security.x509.SubjectAlternativeNameExtension.NAME);
    ...
    private void setVersion(Object val) throws CertificateException {
        if (!(val instanceof ullink.security.x509.CertificateVersion)) {
            throw new CertificateException("Version class type invalid.");
        }
        version = (ullink.security.x509.CertificateVersion)val;
    }
    ...
    private void setSerialNumber(Object val) throws CertificateException {
        if (!(val instanceof ullink.security.x509.CertificateSerialNumber)) {
            throw new CertificateException("SerialNumber class type invalid.");
        }
        serialNum = (ullink.security.x509.CertificateSerialNumber)val;
    }
    ...
    private void setAlgorithmId(Object val) throws CertificateException {
        if (!(val instanceof ullink.security.x509.CertificateAlgorithmId)) {
            throw new CertificateException(
                             "AlgorithmId class type invalid.");
        }
        algId = (ullink.security.x509.CertificateAlgorithmId)val;
    }
    ...
    private void setIssuer(Object val) throws CertificateException {
        if (!(val instanceof ullink.security.x509.CertificateIssuerName)) {
            throw new CertificateException(
                             "Issuer class type invalid.");
        }
        issuer = (ullink.security.x509.CertificateIssuerName)val;
    }
    ...
    private void setValidity(Object val) throws CertificateException {
        if (!(val instanceof ullink.security.x509.CertificateValidity)) {
            throw new CertificateException(
                             "CertificateValidity class type invalid.");
        }
        interval = (ullink.security.x509.CertificateValidity)val;
    }
    ...
    private void setSubject(Object val) throws CertificateException {
        if (!(val instanceof ullink.security.x509.CertificateSubjectName)) {
            throw new CertificateException(
                             "Subject class type invalid.");
        }
        subject = (ullink.security.x509.CertificateSubjectName)val;
    }
    ...
    private void setKey(Object val) throws CertificateException {
        if (!(val instanceof ullink.security.x509.CertificateX509Key)) {
            throw new CertificateException(
                             "Key class type invalid.");
        }
        pubKey = (ullink.security.x509.CertificateX509Key)val;
    }
    ...
    private void setIssuerUniqueId(Object val) throws CertificateException {
        if (version.compare(ullink.security.x509.CertificateVersion.V2) < 0) {
            throw new CertificateException("Invalid version");
        }
        if (!(val instanceof ullink.security.x509.CertificateIssuerUniqueIdentity)) {
            throw new CertificateException(
                             "IssuerUniqueId class type invalid.");
        }
        issuerUniqueId = (ullink.security.x509.CertificateIssuerUniqueIdentity)val;
    }
    ...
    private void setSubjectUniqueId(Object val) throws CertificateException {
        ...
        if (!(val instanceof ullink.security.x509.CertificateSubjectUniqueIdentity)) {
            throw new CertificateException(
                             "SubjectUniqueId class type invalid.");
        }
        subjectUniqueId = (ullink.security.x509.CertificateSubjectUniqueIdentity)val;
    }
    ...
    private void setExtensions(Object val) throws CertificateException {
        ...
        if (!(val instanceof ullink.security.x509.CertificateExtensions)) {
          throw new CertificateException(
                             "Extensions class type invalid.");
        }
        extensions = (ullink.security.x509.CertificateExtensions)val;
    }
}
