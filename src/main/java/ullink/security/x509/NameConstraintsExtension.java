 ...
 * @see Extension
 * @see ullink.security.x509.CertAttrSet
 */
public class NameConstraintsExtension extends Extension
implements ullink.security.x509.CertAttrSet<String>, Cloneable {
    /**
    ...
    private ullink.security.x509.GeneralSubtrees permitted = null;
    private ullink.security.x509.GeneralSubtrees excluded = null;
    ...
    private void calcMinMax() throws IOException {
        ...
        if (excluded != null) {
            for (int i = 0; i < excluded.size(); i++) {
                ullink.security.x509.GeneralSubtree subtree = excluded.get(i);
        ...
        if (permitted != null) {
            for (int i = 0; i < permitted.size(); i++) {
                ullink.security.x509.GeneralSubtree subtree = permitted.get(i);
                if (subtree.getMinimum() != 0)
                    hasMin = true;
                if (subtree.getMaximum() != -1)
                    hasMax = true;
     ...
     */
    public NameConstraintsExtension(ullink.security.x509.GeneralSubtrees permitted,
                                    ullink.security.x509.GeneralSubtrees excluded)
    throws IOException {
        this.permitted = permitted;
        this.excluded = excluded;

        this.extensionId = ullink.security.x509.PKIXExtensions.NameConstraints_Id;
        this.critical = true;
        encodeThis();
    }
    ...
    throws IOException {
        ...
        while (val.data.available() != 0) {
            ...
            if (opt.isContextSpecific(TAG_PERMITTED) && opt.isConstructed()) {
                ...
                permitted = new ullink.security.x509.GeneralSubtrees(opt);
                ...
                opt.resetTag(DerValue.tag_Sequence);
                excluded = new ullink.security.x509.GeneralSubtrees(opt);
            } else
    ...
    public void set(String name, Object obj) throws IOException {
        if (name.equalsIgnoreCase(PERMITTED_SUBTREES)) {
            if (!(obj instanceof ullink.security.x509.GeneralSubtrees)) {
                throw new IOException("Attribute value should be"
                                    + " of type GeneralSubtrees.");
            }
            permitted = (ullink.security.x509.GeneralSubtrees)obj;
        } else if (name.equalsIgnoreCase(EXCLUDED_SUBTREES)) {
            if (!(obj instanceof ullink.security.x509.GeneralSubtrees)) {
                throw new IOException("Attribute value should be "
                                    + "of type GeneralSubtrees.");
            }
            excluded = (ullink.security.x509.GeneralSubtrees)obj;
        } else {
    ...
    public void merge(NameConstraintsExtension newConstraints)
        ...
        ullink.security.x509.GeneralSubtrees newExcluded =
                        (ullink.security.x509.GeneralSubtrees)newConstraints.get(EXCLUDED_SUBTREES);
        if (excluded == null) {
            excluded = (newExcluded != null) ?
                        (ullink.security.x509.GeneralSubtrees)newExcluded.clone() : null;
        } else {
            if (newExcluded != null) {
        ...
        ullink.security.x509.GeneralSubtrees newPermitted =
                (ullink.security.x509.GeneralSubtrees)newConstraints.get(PERMITTED_SUBTREES);
        if (permitted == null) {
            permitted = (newPermitted != null) ?
                        (ullink.security.x509.GeneralSubtrees)newPermitted.clone() : null;
        } else {
            if (newPermitted != null) {
                ...
                if (newExcluded != null) {
                    if (excluded != null) {
                        ...
                        excluded = (ullink.security.x509.GeneralSubtrees)newExcluded.clone();
                    }
                }
            }
        }
    ...
    public boolean verify(X509Certificate cert) throws IOException {
        ...
        ullink.security.x509.X500Name subject = ullink.security.x509.X500Name.asX500Name(subjectPrincipal);
        ...
        ullink.security.x509.GeneralNames altNames = null;
        // extract altNames
        try {
            ...
            ullink.security.x509.SubjectAlternativeNameExtension altNameExt =
                certImpl.getSubjectAlternativeNameExtension();
            if (altNameExt != null) {
                // extract altNames from extension; this call does not
                // return an IOException on null altnames
                altNames = (ullink.security.x509.GeneralNames)
        ...
        for (int i = 0; i < altNames.size(); i++) {
            ullink.security.x509.GeneralNameInterface altGNI = altNames.get(i).getName();
            if (!verify(altGNI)) {
                return false;
            }
        }
     ...
     * @throws IOException on error
     */
    public boolean verify(ullink.security.x509.GeneralNameInterface name) throws IOException {
        if (name == null) {
        ...
        if (excluded != null && excluded.size() > 0) {
            ...
            for (int i = 0; i < excluded.size(); i++) {
                ullink.security.x509.GeneralSubtree gs = excluded.get(i);
                if (gs == null)
                    continue;
                ullink.security.x509.GeneralName gn = gs.getName();
                if (gn == null)
                    continue;
                ullink.security.x509.GeneralNameInterface exName = gn.getName();
                if (exName == null)
                    continue;
                ...
                case ullink.security.x509.GeneralNameInterface.NAME_WIDENS: // name widens excluded
                case ullink.security.x509.GeneralNameInterface.NAME_SAME_TYPE:
                    break;
                case ullink.security.x509.GeneralNameInterface.NAME_MATCH:
                case ullink.security.x509.GeneralNameInterface.NAME_NARROWS: // subject name excluded
                    return false;
                }
            }
        }
        ...
        if (permitted != null && permitted.size() > 0) {
            ...
            for (int i = 0; i < permitted.size(); i++) {
                ullink.security.x509.GeneralSubtree gs = permitted.get(i);
                if (gs == null)
                    continue;
                ullink.security.x509.GeneralName gn = gs.getName();
                if (gn == null)
                    continue;
                ullink.security.x509.GeneralNameInterface perName = gn.getName();
                if (perName == null)
                    continue;
                ...
                case ullink.security.x509.GeneralNameInterface.NAME_WIDENS: // name widens permitted
                case ullink.security.x509.GeneralNameInterface.NAME_SAME_TYPE:
                    sameType = true;
     ...
     * @throws IOException on error
     */
    public boolean verifyRFC822SpecialCase(ullink.security.x509.X500Name subject) throws IOException {
        for (Iterator t = subject.allAvas().iterator(); t.hasNext(); ) {
            ullink.security.x509.AVA ava = (ullink.security.x509.AVA)t.next();
            ObjectIdentifier attrOID = ava.getObjectIdentifier();
            if (attrOID.equals(PKCS9Attribute.EMAIL_ADDRESS_OID)) {
                String attrValue = ava.getValueString();
                if (attrValue != null) {
                    ullink.security.x509.RFC822Name emailName;
                    try {
                        emailName = new ullink.security.x509.RFC822Name(attrValue);
    ...
    public Object clone() {
        try {
            ...
            if (permitted != null) {
                newNCE.permitted = (ullink.security.x509.GeneralSubtrees) permitted.clone();
            }
            if (excluded != null) {
                newNCE.excluded = (ullink.security.x509.GeneralSubtrees) excluded.clone();
            }
            return newNCE;
