...
public class IssuingDistributionPointExtension extends Extension
     ...
     */
    private ullink.security.x509.DistributionPointName distributionPoint = null;

    /*
     * The scope settings for the CRL.
     */
    private ullink.security.x509.ReasonFlags revocationReasons = null;
    ...
    public IssuingDistributionPointExtension(
        ullink.security.x509.DistributionPointName distributionPoint, ullink.security.x509.ReasonFlags revocationReasons,
        ...
        this.extensionId = ullink.security.x509.PKIXExtensions.IssuingDistributionPoint_Id;
        this.critical = true;
    ...
    public IssuingDistributionPointExtension(Boolean critical, Object value)
        ...
        while (in != null && in.available() != 0) {
            ...
            if (opt.isContextSpecific(TAG_DISTRIBUTION_POINT) &&
                opt.isConstructed()) {
                distributionPoint =
                    new ullink.security.x509.DistributionPointName(opt.data.getDerValue());
                ...
                hasOnlyCACerts = opt.getBoolean();
                       ...
                       !opt.isConstructed()) {
                revocationReasons = new ullink.security.x509.ReasonFlags(opt); // expects tag implicit
    ...
    public void set(String name, Object obj) throws IOException {
        if (name.equalsIgnoreCase(POINT)) {
            if (!(obj instanceof ullink.security.x509.DistributionPointName)) {
                throw new IOException(
                    "Attribute value should be of type DistributionPointName.");
            }
            distributionPoint = (ullink.security.x509.DistributionPointName)obj;

        } else if (name.equalsIgnoreCase(REASONS)) {
            if (!(obj instanceof ullink.security.x509.ReasonFlags)) {
                throw new IOException(
