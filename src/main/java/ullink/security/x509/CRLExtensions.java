...
public class CRLExtensions {
    ...
    private Map<String, ullink.security.x509.Extension> map = Collections.synchronizedMap(
            new TreeMap<String, ullink.security.x509.Extension>());
    ...
    private void init(DerInputStream derStrm) throws CRLException {
        try {
            ...
            for (int i = 0; i < exts.length; i++) {
                ullink.security.x509.Extension ext = new ullink.security.x509.Extension(exts[i]);
                parseExtension(ext);
            }
        } catch (IOException e) {
    ...
    private void parseExtension(ullink.security.x509.Extension ext) throws CRLException {
        try {
            ...
            ullink.security.x509.CertAttrSet crlExt = (ullink.security.x509.CertAttrSet)cons.newInstance(passed);
            if (map.put(crlExt.getName(), (ullink.security.x509.Extension)crlExt) != null) {
                throw new CRLException("Duplicate extensions not allowed");
            }
    ...
    throws CRLException {
        try {
            ...
            Collection<ullink.security.x509.Extension> allExts = map.values();
            Object[] objs = allExts.toArray();

            for (int i = 0; i < objs.length; i++) {
                if (objs[i] instanceof ullink.security.x509.CertAttrSet)
                    ((ullink.security.x509.CertAttrSet)objs[i]).encode(extOut);
                else if (objs[i] instanceof ullink.security.x509.Extension)
                    ((ullink.security.x509.Extension)objs[i]).encode(extOut);
                else
     ...
     */
    public ullink.security.x509.Extension get(String alias) {
        ullink.security.x509.X509AttributeName attr = new ullink.security.x509.X509AttributeName(alias);
        String name;
        String id = attr.getPrefix();
        if (id.equalsIgnoreCase(ullink.security.x509.X509CertImpl.NAME)) { // fully qualified
    ...
    public void set(String alias, Object obj) {
        map.put(alias, (ullink.security.x509.Extension)obj);
    }
     ...
     */
    public Enumeration<ullink.security.x509.Extension> getElements() {
     ...
     */
    public Collection<ullink.security.x509.Extension> getAllExtensions() {
        return map.values();
    }
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof CRLExtensions))
            return false;
        Collection<ullink.security.x509.Extension> otherC =
        ...
        ullink.security.x509.Extension otherExt, thisExt;
        String key = null;
        for (int i = 0; i < len; i++) {
            if (objs[i] instanceof ullink.security.x509.CertAttrSet)
                key = ((ullink.security.x509.CertAttrSet)objs[i]).getName();
            otherExt = (ullink.security.x509.Extension)objs[i];
            if (key == null)
