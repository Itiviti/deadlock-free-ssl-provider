 ...
 * @see GeneralName
 * @see GeneralNames
 * @see ullink.security.x509.GeneralNameInterface
 */

public class X500Name implements ullink.security.x509.GeneralNameInterface, Principal {
    ...
    private ullink.security.x509.RDN[] names;        // RDNs (never null)
    private X500Principal x500Principal;
    private byte[] encoded;

    // cached immutable list of the RDNs and all the AVAs
    private volatile List<ullink.security.x509.RDN> rdnList;
    ...
    throws IOException {
        names = new ullink.security.x509.RDN[6];
        /*
         * NOTE:  it's only on output that little-endian
         * ordering is used.
         */
        names[5] = new ullink.security.x509.RDN(1);
        names[5].assertion[0] = new AVA(commonName_oid,
                new DerValue(commonName));
        names[4] = new ullink.security.x509.RDN(1);
     ...
     */
    public X500Name(ullink.security.x509.RDN[] rdnArray) throws IOException {
        if (rdnArray == null) {
            names = new ullink.security.x509.RDN[0];
        } else {
     ...
     */
    public List<ullink.security.x509.RDN> rdns() {
        List<ullink.security.x509.RDN> list = rdnList;
        if (list == null) {
    ...
    public boolean equals(Object obj) {
        ...
        for (int i = 0; i < n; i++) {
            ullink.security.x509.RDN r1 = this.names[i];
            ullink.security.x509.RDN r2 = other.names[i];
     ...
     */
    public int getType() {
        return (ullink.security.x509.GeneralNameInterface.NAME_DIRECTORY);
    }
    ...
    private void parseDER(DerInputStream in) throws IOException {
        ...
        if (nameseq == null) {
            ...
            names = new ullink.security.x509.RDN[nameseq.length];
            for (int i = 0; i < nameseq.length; i++) {
                names[i] = new ullink.security.x509.RDN(nameseq[i]);
            }
        }
    }
    ...
    private void parseDN(String input, Map<String, String> keywordMap)
        ...
        List<ullink.security.x509.RDN> dnVector = new ArrayList<ullink.security.x509.RDN>();
        int dnOffset = 0;
        int rdnEnd;
        String rdnString;
        int quoteCount = 0;
        ...
        while (nextComma >=0 || nextSemiColon >=0) {
            ...
            if (rdnEnd >= 0 && quoteCount != 1 &&
                ...
                ullink.security.x509.RDN rdn = new ullink.security.x509.RDN(rdnString, keywordMap);
                dnVector.add(rdn);
        ...
        names = dnVector.toArray(new ullink.security.x509.RDN[dnVector.size()]);
    }
    ...
    private void parseRFC2253DN(String dnString) throws IOException {
        ...
        while (rdnEnd >=0) {
            ...
            if (rdnEnd > 0 && !escaped(rdnEnd, searchOffset, dnString)) {
                ...
                // Parse RDN, and store it in vector
                ullink.security.x509.RDN rdn = new ullink.security.x509.RDN(rdnString, "RFC2253");
                dnVector.add(rdn);
     ...
     */
    public int constrains(ullink.security.x509.GeneralNameInterface inputName)
    ...
    public X500Name commonAncestor(X500Name other) {
        ...
        ullink.security.x509.RDN[] ancestor = new ullink.security.x509.RDN[i];
        for (int j=0; j < i; j++) {
            ancestor[j] = names[j];
        }
