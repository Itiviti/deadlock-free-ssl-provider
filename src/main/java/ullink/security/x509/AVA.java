...
import ullink.security.pkcs.PKCS9Attribute;
 ...
 */
public class AVA implements ullink.security.util.DerEncoder {

    private static final ullink.security.util.Debug debug = ullink.security.util.Debug.getInstance("x509", "\t[AVA]");
    ...
    final ullink.security.util.ObjectIdentifier oid;
    final ullink.security.util.DerValue value;
    ...
    public AVA(ullink.security.util.ObjectIdentifier type, ullink.security.util.DerValue val) {
    ...
    AVA(Reader in, int format, Map<String, String> keywordMap)
        ...
        if (c == -1) {
            // empty value
            value = new ullink.security.util.DerValue("");
            return;
        }
     ...
     */
    public ullink.security.util.ObjectIdentifier getObjectIdentifier() {
        return oid;
    }
     ...
     */
    public ullink.security.util.DerValue getDerValue() {
        return value;
    }
    ...
    private static ullink.security.util.DerValue parseHexString
        ...
        return new ullink.security.util.DerValue(baos.toByteArray());
    }

    private ullink.security.util.DerValue parseQuotedString
        ...
        while (c != '"') {
            ...
            isPrintableString &= ullink.security.util.DerValue.isPrintableStringChar((char) c);
            temp.append((char)c);
            c = readChar(in, "Quoted string did not end in quote");
        }
        ...
        if (this.oid.equals(PKCS9Attribute.EMAIL_ADDRESS_OID) ||
            ...
            return new ullink.security.util.DerValue(ullink.security.util.DerValue.tag_IA5String,
                                        temp.toString().trim());
        } else if (isPrintableString) {
            return new ullink.security.util.DerValue(temp.toString().trim());
        } else {
            return new ullink.security.util.DerValue(ullink.security.util.DerValue.tag_UTF8String,
                                        temp.toString().trim());
        }
    }

    private ullink.security.util.DerValue parseString
        ...
        if (this.oid.equals(PKCS9Attribute.EMAIL_ADDRESS_OID) ||
            (this.oid.equals(X500Name.DOMAIN_COMPONENT_OID) &&
                PRESERVE_OLD_DC_ENCODING == false)) {
            // EmailAddress and DomainComponent must be IA5String
            return new ullink.security.util.DerValue(ullink.security.util.DerValue.tag_IA5String, temp.toString());
        } else if (isPrintableString) {
            return new ullink.security.util.DerValue(temp.toString());
        } else {
            return new ullink.security.util.DerValue(ullink.security.util.DerValue.tag_UTF8String, temp.toString());
        }
    }
    ...
    AVA(ullink.security.util.DerValue derval) throws IOException {
        // Individual attribute value assertions are SEQUENCE of two values.
        // That'd be a "struct" outside of ASN.1.
        if (derval.tag != ullink.security.util.DerValue.tag_Sequence) {
    ...
    AVA(ullink.security.util.DerInputStream in) throws IOException {
        this(in.getDerValue());
    }
     ...
     */
    public void encode(ullink.security.util.DerOutputStream out) throws IOException {
        derEncode(out);
    }
    ...
    public void derEncode(OutputStream out) throws IOException {
        ...
        ullink.security.util.DerOutputStream tmp2 = new ullink.security.util.DerOutputStream();

        tmp.putOID(oid);
        value.encode(tmp);
        tmp2.write(ullink.security.util.DerValue.tag_Sequence, tmp);
    ...
    public String toRFC2253String(Map<String, String> oidMap) {
            ...
            !isDerString(value, false))
        {
            ...
            for (int i = 0; i < valStr.length(); i++) {
                ...
                if (ullink.security.util.DerValue.isPrintableStringChar(c) ||
                ...
                } else if (debug != null && ullink.security.util.Debug.isOn("ava")) {
     ...
     */
    private static boolean isDerString(ullink.security.util.DerValue value, boolean canonical) {
        if (canonical) {
            switch (value.tag) {
                case ullink.security.util.DerValue.tag_PrintableString:
                case ullink.security.util.DerValue.tag_UTF8String:
                    return true;
                default:
                    return false;
            }
            ...
            switch (value.tag) {
                case ullink.security.util.DerValue.tag_PrintableString:
                case ullink.security.util.DerValue.tag_T61String:
                case ullink.security.util.DerValue.tag_IA5String:
                case ullink.security.util.DerValue.tag_GeneralString:
                case ullink.security.util.DerValue.tag_BMPString:
                case ullink.security.util.DerValue.tag_UTF8String:
                    return true;
                default:
                    return false;
...
class AVAKeyword {
    ...
    private static final Map<ullink.security.util.ObjectIdentifier,AVAKeyword> oidMap;
    private static final Map<String,AVAKeyword> keywordMap;

    private String keyword;
    private ullink.security.util.ObjectIdentifier oid;
    private boolean rfc1779Compliant, rfc2253Compliant;

    private AVAKeyword(String keyword, ullink.security.util.ObjectIdentifier oid,
     ...
     */
    static ullink.security.util.ObjectIdentifier getOID(String keyword, int standard)
            throws IOException {
        return getOID
     ...
     */
    static ullink.security.util.ObjectIdentifier getOID
        ...
        if (oidString == null) {
            ...
            return new ullink.security.util.ObjectIdentifier(oidString);
        }
        ...
        return new ullink.security.util.ObjectIdentifier(keyword);
    }
     ...
     * String.
     */
    static String getKeyword(ullink.security.util.ObjectIdentifier oid, int standard) {
        return getKeyword
    ...
    static String getKeyword
        (ullink.security.util.ObjectIdentifier oid, int standard, Map<String, String> extraOidMap) {
     ...
     */
    static boolean hasKeyword(ullink.security.util.ObjectIdentifier oid, int standard) {
        AVAKeyword ak = oidMap.get(oid);
        if (ak == null) {
            return false;
        }
    ...
    static {
        oidMap = new HashMap<ullink.security.util.ObjectIdentifier,AVAKeyword>();
