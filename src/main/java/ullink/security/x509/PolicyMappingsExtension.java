...
implements ullink.security.x509.CertAttrSet<String> {
    ...
    private List<ullink.security.x509.CertificatePolicyMap> maps;
    ...
    private void encodeThis() throws IOException {
        ...
        for (ullink.security.x509.CertificatePolicyMap map : maps) {
            map.encode(tmp);
        }
     ...
     */
    public PolicyMappingsExtension(List<ullink.security.x509.CertificatePolicyMap> map)
            throws IOException {
        this.maps = map;
        this.extensionId = ullink.security.x509.PKIXExtensions.PolicyMappings_Id;
        this.critical = false;
        encodeThis();
    }
    ...
    public PolicyMappingsExtension() {
        extensionId = ullink.security.x509.PKIXExtensions.KeyUsage_Id;
        critical = false;
        maps = new ArrayList<ullink.security.x509.CertificatePolicyMap>();
    }
    ...
    throws IOException {
        ...
        while (val.data.available() != 0) {
            ...
            ullink.security.x509.CertificatePolicyMap map = new ullink.security.x509.CertificatePolicyMap(seq);
            maps.add(map);
        }
    }
    ...
    public void encode(OutputStream out) throws IOException {
        ...
        if (extensionValue == null) {
            extensionId = ullink.security.x509.PKIXExtensions.PolicyMappings_Id;
            critical = false;
            encodeThis();
        }
        super.encode(tmp);
    ...
    public void set(String name, Object obj) throws IOException {
        if (name.equalsIgnoreCase(MAP)) {
            ...
            maps = (List<ullink.security.x509.CertificatePolicyMap>)obj;
        } else {
