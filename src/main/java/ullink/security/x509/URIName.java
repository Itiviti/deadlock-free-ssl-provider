 ...
 */
public class URIName implements ullink.security.x509.GeneralNameInterface {

    // private attributes
    private URI uri;
    private String host;
    private ullink.security.x509.DNSName hostDNS;
    ...
    public URIName(String name) throws IOException {
        ...
        if (host != null) {
            if (host.charAt(0) == '[') {
                ...
                try {
                    hostDNS = new ullink.security.x509.DNSName(host);
                } catch (IOException ioe) {
    ...
    public static URIName nameConstraint(DerValue value) throws IOException {
        ...
        if (uri.getScheme() == null) {
            ...
            try {
                ullink.security.x509.DNSName hostDNS;
                if (host.charAt(0) == '.') {
                    hostDNS = new ullink.security.x509.DNSName(host.substring(1));
                } else {
                    hostDNS = new ullink.security.x509.DNSName(host);
                }
    ...
    URIName(URI uri, String host, ullink.security.x509.DNSName hostDNS) {
        this.uri = uri;
        this.host = host;
     ...
     */
    public int getType() {
        return ullink.security.x509.GeneralNameInterface.NAME_URI;
    }
    ...
    public int constrains(ullink.security.x509.GeneralNameInterface inputName)
        ...
        if (inputName == null) {
            ...
            if (otherHost.equalsIgnoreCase(host)) {
                ...
                if ((hostDNS == null) ||
                    !(otherHostObject instanceof ullink.security.x509.DNSName)) {
                    ...
                    ullink.security.x509.DNSName otherDNS = (ullink.security.x509.DNSName) otherHostObject;
    ...
    public int subtreeDepth() throws UnsupportedOperationException {
        ullink.security.x509.DNSName dnsName = null;
        try {
            dnsName = new ullink.security.x509.DNSName(host);
        } catch (IOException ioe) {
