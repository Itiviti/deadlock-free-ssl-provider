...
public class GeneralSubtrees implements Cloneable {
    ...
    private final List<ullink.security.x509.GeneralSubtree> trees;

    // Private variables
    private static final int NAME_DIFF_TYPE = ullink.security.x509.GeneralNameInterface.NAME_DIFF_TYPE;
    private static final int NAME_MATCH = ullink.security.x509.GeneralNameInterface.NAME_MATCH;
    private static final int NAME_NARROWS = ullink.security.x509.GeneralNameInterface.NAME_NARROWS;
    private static final int NAME_WIDENS = ullink.security.x509.GeneralNameInterface.NAME_WIDENS;
    private static final int NAME_SAME_TYPE = ullink.security.x509.GeneralNameInterface.NAME_SAME_TYPE;

    /**
     * The default constructor for the class.
     */
    public GeneralSubtrees() {
        trees = new ArrayList<ullink.security.x509.GeneralSubtree>();
    }

    private GeneralSubtrees(GeneralSubtrees source) {
        trees = new ArrayList<ullink.security.x509.GeneralSubtree>(source.trees);
    }
    ...
    public GeneralSubtrees(DerValue val) throws IOException {
        ...
        while (val.data.available() != 0) {
            ...
            ullink.security.x509.GeneralSubtree tree = new ullink.security.x509.GeneralSubtree(opt);
            add(tree);
        }
    }

    public ullink.security.x509.GeneralSubtree get(int index) {
    ...
    public void add(ullink.security.x509.GeneralSubtree tree) {
        if (tree == null) {
    ...
    public boolean contains(ullink.security.x509.GeneralSubtree tree) {
        if (tree == null) {
            throw new NullPointerException();
        }
    ...
    public Iterator<ullink.security.x509.GeneralSubtree> iterator() {
        return trees.iterator();
    }

    public List<ullink.security.x509.GeneralSubtree> trees() {
        return trees;
    }
     ...
     */
    private ullink.security.x509.GeneralNameInterface getGeneralNameInterface(int ndx) {
        return getGeneralNameInterface(get(ndx));
    }

    private static ullink.security.x509.GeneralNameInterface getGeneralNameInterface(ullink.security.x509.GeneralSubtree gs) {
        ullink.security.x509.GeneralName gn = gs.getName();
        ullink.security.x509.GeneralNameInterface gni = gn.getName();
        return gni;
    }
    ...
    private void minimize() {
        ...
        for (int i = 0; i < size(); i++) {
            ullink.security.x509.GeneralNameInterface current = getGeneralNameInterface(i);
            boolean remove1 = false;

            /* compare current to subsequent elements */
            for (int j = i + 1; j < size(); j++) {
                ullink.security.x509.GeneralNameInterface subsequent = getGeneralNameInterface(j);
                switch (current.constrains(subsequent)) {
                    case ullink.security.x509.GeneralNameInterface.NAME_DIFF_TYPE:
                        /* not comparable; different name types; keep checking */
                        continue;
                    case ullink.security.x509.GeneralNameInterface.NAME_MATCH:
                        /* delete one of the duplicates */
                        remove1 = true;
                        break;
                    case ullink.security.x509.GeneralNameInterface.NAME_NARROWS:
                        /* subsequent narrows current */
                        /* remove narrower name (subsequent) */
                        remove(j);
                        j--; /* continue check with new subsequent */
                        continue;
                    case ullink.security.x509.GeneralNameInterface.NAME_WIDENS:
                        /* subsequent widens current */
                        /* remove narrower name current */
                        remove1 = true;
                        break;
                    case ullink.security.x509.GeneralNameInterface.NAME_SAME_TYPE:
     ...
     */
    private ullink.security.x509.GeneralSubtree createWidestSubtree(ullink.security.x509.GeneralNameInterface name) {
        try {
            ullink.security.x509.GeneralName newName;
            switch (name.getType()) {
            case ullink.security.x509.GeneralNameInterface.NAME_ANY:
                // Create new OtherName with same OID as baseName, but
                // empty value
                ObjectIdentifier otherOID = ((OtherName)name).getOID();
                newName = new ullink.security.x509.GeneralName(new OtherName(otherOID, null));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_RFC822:
                newName = new ullink.security.x509.GeneralName(new ullink.security.x509.RFC822Name(""));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_DNS:
                newName = new ullink.security.x509.GeneralName(new ullink.security.x509.DNSName(""));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_X400:
                newName = new ullink.security.x509.GeneralName(new ullink.security.x509.X400Address((byte[])null));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_DIRECTORY:
                newName = new ullink.security.x509.GeneralName(new ullink.security.x509.X500Name(""));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_EDI:
                newName = new ullink.security.x509.GeneralName(new ullink.security.x509.EDIPartyName(""));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_URI:
                newName = new ullink.security.x509.GeneralName(new ullink.security.x509.URIName(""));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_IP:
                newName = new ullink.security.x509.GeneralName(new ullink.security.x509.IPAddressName((byte[])null));
                break;
            case ullink.security.x509.GeneralNameInterface.NAME_OID:
                newName = new ullink.security.x509.GeneralName
                    (new ullink.security.x509.OIDName(new ObjectIdentifier((int[])null)));
                break;
            default:
                throw new IOException
                    ("Unsupported GeneralNameInterface type: " + name.getType());
            }
            return new ullink.security.x509.GeneralSubtree(newName, 0, -1);
        } catch (IOException e) {
    ...
    public GeneralSubtrees intersect(GeneralSubtrees other) {
        ...
        // same type in this.
        for (int i = 0; i < size(); i++) {
            ullink.security.x509.GeneralNameInterface thisEntry = getGeneralNameInterface(i);
            ...
            for (int j = 0; j < other.size(); j++) {
                ullink.security.x509.GeneralSubtree otherEntryGS = other.get(j);
                ullink.security.x509.GeneralNameInterface otherEntry =
            ...
            if (sameType) {
                ...
                for (int j = 0; j < size(); j++) {
                    ullink.security.x509.GeneralNameInterface thisAltEntry = getGeneralNameInterface(j);

                    if (thisAltEntry.getType() == thisEntry.getType()) {
                        for (int k = 0; k < other.size(); k++) {
                            ullink.security.x509.GeneralNameInterface othAltEntry =
                ...
                if (intersection == false) {
                    ...
                    ullink.security.x509.GeneralSubtree widestSubtree =
        ...
        for (int i = 0; i < other.size(); i++) {
            ullink.security.x509.GeneralSubtree otherEntryGS = other.get(i);
            ullink.security.x509.GeneralNameInterface otherEntry = getGeneralNameInterface(otherEntryGS);
            boolean diffType = false;
            for (int j = 0; j < size(); j++) {
                ullink.security.x509.GeneralNameInterface thisEntry = getGeneralNameInterface(j);
                switch (thisEntry.constrains(otherEntry)) {
                    case NAME_DIFF_TYPE:
                        diffType = true;
    ...
    public void reduce(GeneralSubtrees excluded) {
        ...
        for (int i = 0, n = excluded.size(); i < n; i++) {
            ullink.security.x509.GeneralNameInterface excludedName = excluded.getGeneralNameInterface(i);
            for (int j = 0; j < size(); j++) {
                ullink.security.x509.GeneralNameInterface permitted = getGeneralNameInterface(j);
