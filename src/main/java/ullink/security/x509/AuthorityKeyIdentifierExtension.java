...
implements ullink.security.x509.CertAttrSet<String> {
    /**
    ...
    private ullink.security.x509.KeyIdentifier id = null;
    private ullink.security.x509.GeneralNames names = null;
    private ullink.security.x509.SerialNumber serialNum = null;
     ...
     */
    public AuthorityKeyIdentifierExtension(ullink.security.x509.KeyIdentifier kid, ullink.security.x509.GeneralNames name,
                                           ullink.security.x509.SerialNumber sn)
    throws IOException {
        this.id = kid;
        this.names = name;
        this.serialNum = sn;

        this.extensionId = ullink.security.x509.PKIXExtensions.AuthorityKey_Id;
        this.critical = false;
        encodeThis();
    }
    ...
    throws IOException {
        ...
        while ((val.data != null) && (val.data.available() != 0)) {
            ...
            if (opt.isContextSpecific(TAG_ID) && !opt.isConstructed()) {
                ...
                id = new ullink.security.x509.KeyIdentifier(opt);
                ...
                names = new ullink.security.x509.GeneralNames(opt);
                ...
                serialNum = new ullink.security.x509.SerialNumber(opt);
            } else
    ...
    public void encode(OutputStream out) throws IOException {
        ...
        if (this.extensionValue == null) {
            extensionId = ullink.security.x509.PKIXExtensions.AuthorityKey_Id;
            critical = false;
            encodeThis();
        }
        super.encode(tmp);
    ...
    public void set(String name, Object obj) throws IOException {
        if (name.equalsIgnoreCase(KEY_ID)) {
            if (!(obj instanceof ullink.security.x509.KeyIdentifier)) {
              throw new IOException("Attribute value should be of " +
                                    "type KeyIdentifier.");
            }
            id = (ullink.security.x509.KeyIdentifier)obj;
        } else if (name.equalsIgnoreCase(AUTH_NAME)) {
            if (!(obj instanceof ullink.security.x509.GeneralNames)) {
              throw new IOException("Attribute value should be of " +
                                    "type GeneralNames.");
            }
            names = (ullink.security.x509.GeneralNames)obj;
        } else if (name.equalsIgnoreCase(SERIAL_NUMBER)) {
            if (!(obj instanceof ullink.security.x509.SerialNumber)) {
              throw new IOException("Attribute value should be of " +
                                    "type SerialNumber.");
            }
            serialNum = (ullink.security.x509.SerialNumber)obj;
        } else {
