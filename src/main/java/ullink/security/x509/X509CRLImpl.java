...
public class X509CRLImpl extends X509CRL {
    ...
    private ullink.security.x509.AlgorithmId sigAlgId = null; // sig alg in CRL

    // crl information
    private int              version;
    private ullink.security.x509.AlgorithmId infoSigAlgId; // sig alg in "to-be-signed" crl
    private ullink.security.x509.X500Name issuer = null;
    ...
    private ullink.security.x509.CRLExtensions extensions = null;
     ...
     */
    public X509CRLImpl(ullink.security.x509.X500Name issuer, Date thisDate, Date nextDate) {
        this.issuer = issuer;
        ...
        throws CRLException
    {
        ...
        if (badCerts != null) {
            ...
            for (int i = 0; i < badCerts.length; i++) {
                ullink.security.x509.X509CRLEntryImpl badCert = (ullink.security.x509.X509CRLEntryImpl)badCerts[i];
                try {
     ...
     */
    public X509CRLImpl(ullink.security.x509.X500Name issuer, Date thisDate, Date nextDate,
               X509CRLEntry[] badCerts, ullink.security.x509.CRLExtensions crlExts)
        throws CRLException
    {
        ...
        try {
            ...
            if (!revokedList.isEmpty()) {
                for (X509CRLEntry entry : revokedList) {
                    ((ullink.security.x509.X509CRLEntryImpl)entry).encode(rCerts);
                }
    ...
    throws CRLException, NoSuchAlgorithmException, InvalidKeyException,
        ...
        try {
            ...
            sigAlgId = ullink.security.x509.AlgorithmId.get(sigEngine.getAlgorithm());
    ...
    public String toString() {
        ...
        if (extensions != null) {
            Collection<ullink.security.x509.Extension> allExts = extensions.getAllExtensions();
    ...
    public KeyIdentifier getAuthKeyId() throws IOException {
        ullink.security.x509.AuthorityKeyIdentifierExtension aki = getAuthKeyIdExtension();
        if (aki != null) {
     ...
     * @throws IOException on error
     */
    public ullink.security.x509.AuthorityKeyIdentifierExtension getAuthKeyIdExtension()
        throws IOException {
        Object obj = getExtension(ullink.security.x509.PKIXExtensions.AuthorityKey_Id);
        return (ullink.security.x509.AuthorityKeyIdentifierExtension)obj;
    }
     ...
     * @throws IOException on error
     */
    public ullink.security.x509.CRLNumberExtension getCRLNumberExtension() throws IOException {
        Object obj = getExtension(ullink.security.x509.PKIXExtensions.CRLNumber_Id);
        return (ullink.security.x509.CRLNumberExtension)obj;
    }
    ...
    public BigInteger getCRLNumber() throws IOException {
        ullink.security.x509.CRLNumberExtension numExt = getCRLNumberExtension();
        if (numExt != null) {
     ...
     * @throws IOException on error
     */
    public ullink.security.x509.DeltaCRLIndicatorExtension getDeltaCRLIndicatorExtension()
        throws IOException {

        Object obj = getExtension(ullink.security.x509.PKIXExtensions.DeltaCRLIndicator_Id);
        return (ullink.security.x509.DeltaCRLIndicatorExtension)obj;
    }
    ...
    public BigInteger getBaseCRLNumber() throws IOException {
        ullink.security.x509.DeltaCRLIndicatorExtension dciExt = getDeltaCRLIndicatorExtension();
        if (dciExt != null) {
     ...
     * @throws IOException on error
     */
    public ullink.security.x509.IssuerAlternativeNameExtension getIssuerAltNameExtension()
        throws IOException {
        Object obj = getExtension(ullink.security.x509.PKIXExtensions.IssuerAlternativeName_Id);
        return (ullink.security.x509.IssuerAlternativeNameExtension)obj;
    }
     ...
     * @throws IOException on error
     */
    public ullink.security.x509.IssuingDistributionPointExtension
        getIssuingDistributionPointExtension() throws IOException {

        Object obj = getExtension(ullink.security.x509.PKIXExtensions.IssuingDistributionPoint_Id);
        return (ullink.security.x509.IssuingDistributionPointExtension) obj;
    }
    ...
    public Set<String> getCriticalExtensionOIDs() {
        ...
        for (ullink.security.x509.Extension ex : extensions.getAllExtensions()) {
            if (ex.isCritical()) {
    ...
    public byte[] getExtensionValue(String oid) {
        if (extensions == null)
            return null;
        try {
            String extAlias = ullink.security.x509.OIDMap.getName(new ObjectIdentifier(oid));
            ullink.security.x509.Extension crlExt = null;

            if (extAlias == null) { // may be unknown
                ObjectIdentifier findOID = new ObjectIdentifier(oid);
                ullink.security.x509.Extension ex = null;
                ObjectIdentifier inCertOID;
                for (Enumeration<ullink.security.x509.Extension> e = extensions.getElements();
                                                 e.hasMoreElements();) {
                    ex = e.nextElement();
    ...
    public Object getExtension(ObjectIdentifier oid) {
        ...
        return extensions.get(ullink.security.x509.OIDMap.getName(oid));
    }
    ...
    private void parse(DerValue val) throws CRLException, IOException {
        ...
        sigAlgId = ullink.security.x509.AlgorithmId.parse(seq[1]);
        ...
        // signature
        ullink.security.x509.AlgorithmId tmpId = ullink.security.x509.AlgorithmId.parse(tmp);
        ...
        // issuer
        issuer = new ullink.security.x509.X500Name(derStrm);
        if (issuer.isEmpty()) {
        ...
        if ((nextByte == DerValue.tag_SequenceOf)
            ...
            X500Principal badCertIssuer = crlIssuer;
            for (int i = 0; i < badCerts.length; i++) {
                ullink.security.x509.X509CRLEntryImpl entry = new ullink.security.x509.X509CRLEntryImpl(badCerts[i]);
        ...
        if (tmp.isConstructed() && tmp.isContextSpecific((byte)0)) {
            extensions = new ullink.security.x509.CRLExtensions(tmp.data);
        }
        readOnly = true;
    }
     ...
     */
    private X500Principal getCertIssuer(ullink.security.x509.X509CRLEntryImpl entry,
        X500Principal prevCertIssuer) throws IOException {

        ullink.security.x509.CertificateIssuerExtension ciExt =
            entry.getCertificateIssuerExtension();
        if (ciExt != null) {
            ullink.security.x509.GeneralNames names = (ullink.security.x509.GeneralNames)
                ciExt.get(ullink.security.x509.CertificateIssuerExtension.ISSUER);
            ullink.security.x509.X500Name issuerDN = (ullink.security.x509.X500Name) names.get(0).getName();
