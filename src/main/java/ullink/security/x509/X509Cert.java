...
public class X509Cert implements Certificate, Serializable {
    ...
    /* The algorithm id */
    transient protected ullink.security.x509.AlgorithmId algid;
     ...
     * X509.v1 data (parsed)
     */
    transient private ullink.security.x509.X500Name subject; // from subject
    ...
    transient private ullink.security.x509.X500Name issuer;
    transient private ullink.security.x509.AlgorithmId issuerSigAlg;
    ...
    public X509Cert(byte cert []) throws IOException
    {
        ...
        if (in.data.available () != 0)
            throw new ullink.security.x509.CertParseError("garbage at end");
        signedCert = cert;
    }
     ...
     */
    public X509Cert(ullink.security.x509.X500Name subjectName, ullink.security.x509.X509Key subjectPublicKey,
     ...
     *  signature algorithm
     */
    public ullink.security.x509.X500Signer getSigner (ullink.security.x509.AlgorithmId algorithmId,
    ...
    throws NoSuchAlgorithmException, InvalidKeyException
    {
        ...
        return new ullink.security.x509.X500Signer(sig, subject);
    }
     ...
     */
    public ullink.security.x509.X500Name getSubjectName ()
        { return subject; }
     ...
     * Null is returned in the case of a partially constructed cert.
     */
    public ullink.security.x509.X500Name getIssuerName ()
        { return issuer; }
     ...
     * Null is returned in the case of a partially constructed cert.
     */
    public ullink.security.x509.AlgorithmId getIssuerAlgorithmId ()
        { return issuerSigAlg; }
    ...
    private void parse (DerValue val) throws IOException
    {
        ...
        if (val.data.available () != 0)
            throw new ullink.security.x509.CertParseError("signed overrun, bytes = "
                    + val.data.available ());
        if (seq [0].tag != DerValue.tag_Sequence)
            throw new ullink.security.x509.CertParseError("signed fields invalid");

        rawCert = seq [0].toByteArray ();       // XXX slow; fixme!


        issuerSigAlg = ullink.security.x509.AlgorithmId.parse(seq[1]);
        signature = seq [2].getBitString ();

        if (seq [1].data.available () != 0) {
            // XXX why was this error check commented out?
            // It was originally part of the next check.
            throw new ullink.security.x509.CertParseError("algid field overrun");
        }

        if (seq [2].data.available () != 0)
            throw new ullink.security.x509.CertParseError("signed fields overrun");
        ...
        tmp = in.getDerValue ();
        {
            ullink.security.x509.AlgorithmId algid;


            algid = ullink.security.x509.AlgorithmId.parse(tmp);

            if (!algid.equals (issuerSigAlg))
                throw new ullink.security.x509.CertParseError("CA Algorithm mismatch!");
         ...
         * issuer name
         */
        issuer = new ullink.security.x509.X500Name(in);
        ...
        if (tmp.tag != DerValue.tag_Sequence)
            throw new ullink.security.x509.CertParseError("corrupt validity field");

        notbefore = tmp.data.getUTCTime ();
        notafter = tmp.data.getUTCTime ();
        if (tmp.data.available () != 0)
            throw new ullink.security.x509.CertParseError("excess validity data");

        /*
         * subject name and public key
         */
        subject = new ullink.security.x509.X500Name(in);

        tmp = in.getDerValue ();
        pubkey = ullink.security.x509.X509Key.parse(tmp);
     ...
     */
    private byte [] sign (ullink.security.x509.X500Signer issuer, byte data [])
    throws IOException, SignatureException
    {
        /*
