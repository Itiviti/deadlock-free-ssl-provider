...
import ullink.security.jgss.TokenTracker;
...
class Krb5Context implements GSSContextSpi {
    ...
    private ullink.security.krb5.EncryptionKey key;
    private ullink.security.jgss.krb5.Krb5NameElement myName;
    private ullink.security.jgss.krb5.Krb5NameElement peerName;
    private int lifetime;
    private boolean initiator;
    private ChannelBinding channelBinding;

    private ullink.security.jgss.krb5.Krb5CredElement myCred;
    private ullink.security.jgss.krb5.Krb5CredElement delegatedCred; // Set only on acceptor side
    ...
    // stored elsewhere
    private ullink.security.krb5.Credentials serviceCreds;
    private ullink.security.krb5.KrbApReq apReq;
    final private int caller;
    private static final boolean DEBUG = ullink.security.jgss.krb5.Krb5Util.DEBUG;

    /**
     * Constructor for Krb5Context to be called on the context initiator's
     * side.
     */
    Krb5Context(int caller, ullink.security.jgss.krb5.Krb5NameElement peerName, ullink.security.jgss.krb5.Krb5CredElement myCred,
                int lifetime)
        throws GSSException {
     ...
     * side.
     */
    Krb5Context(int caller, ullink.security.jgss.krb5.Krb5CredElement myCred)
        throws GSSException {
        this.caller = caller;
        this.myCred = myCred;
     ...
     * MessageToken.init()
     */
    final CipherHelper getCipherHelper(ullink.security.krb5.EncryptionKey ckey) throws GSSException {
         ullink.security.krb5.EncryptionKey cipherKey = null;
    ...
    final void setKey(ullink.security.krb5.EncryptionKey key) throws GSSException {
        this.key = key;
        // %%% to do: should clear old cipherHelper first
        cipherHelper = new CipherHelper(key);  // Need to use new key
    }

    private final ullink.security.krb5.EncryptionKey getKey() {
        return key;
    }
     ...
     */
    final void setDelegCred(ullink.security.jgss.krb5.Krb5CredElement delegatedCred) {
    ...
    public final Oid getMech() {
        return (ullink.security.jgss.krb5.Krb5MechFactory.GSS_KRB5_MECH_OID);
    }
    ...
    public final byte[] initSecContext(InputStream is, int mechTokenSize)
        throws GSSException {
            ...
            byte[] retVal = null;
            ullink.security.jgss.krb5.InitialToken token = null;
            ...
            try {
                if (state == STATE_NEW) {
                    ...
                    if (myCred == null) {
                        myCred = ullink.security.jgss.krb5.Krb5InitCredential.getInstance(caller, myName,
                                GSSCredential.DEFAULT_LIFETIME);
                    } else if (!myCred.isInitiatorCredential()) {
                        throw new GSSException(errorCode, -1,
                                           "No TGT available");
                    }
                    myName = (ullink.security.jgss.krb5.Krb5NameElement) myCred.getName();
                    ullink.security.krb5.Credentials tgt =
                    ((ullink.security.jgss.krb5.Krb5InitCredential) myCred).getKrb5Credentials();
                    ...
                    if (GSSUtil.useSubjectCredsOnly(caller)) {
                        ...
                        try {
                           ...
                           kerbTicket = AccessController.doPrivileged(
                                ...
                                public KerberosTicket run() throws Exception {
                                    ...
                                    return ullink.security.jgss.krb5.Krb5Util.getTicket(
                        ...
                        if (kerbTicket != null) {
                            ...
                            serviceCreds = ullink.security.jgss.krb5.Krb5Util.ticketToCreds(kerbTicket);
                        }
                    }
                    if (serviceCreds == null) {
                        ...
                        serviceCreds = ullink.security.krb5.Credentials.acquireServiceCreds(
                                peerName.getKrb5PrincipalName().getName(),
                                tgt);
                        if (GSSUtil.useSubjectCredsOnly(caller)) {
                             ...
                             */
                                final KerberosTicket kt =
                                    ullink.security.jgss.krb5.Krb5Util.credsToTicket(serviceCreds);
                                AccessController.doPrivileged (
                    ...
                    token = new ullink.security.jgss.krb5.InitSecContextToken(this, tgt, serviceCreds);
                    apReq = ((ullink.security.jgss.krb5.InitSecContextToken)token).getKrbApReq();
                    retVal = token.encode();
                    myCred = null;
                    ...
                    new ullink.security.jgss.krb5.AcceptSecContextToken(this, serviceCreds, apReq, is);
                    serviceCreds = null;
                    apReq = null;
                    state = STATE_DONE;
                } else {
    ...
    public final byte[] acceptSecContext(InputStream is, int mechTokenSize)
        ...
        try {
            if (state == STATE_NEW) {
                state = STATE_IN_PROCESS;
                if (myCred == null) {
                    myCred = ullink.security.jgss.krb5.Krb5AcceptCredential.getInstance(caller, myName);
                ...
                ullink.security.krb5.EncryptionKey[] secretKeys =
                 ((ullink.security.jgss.krb5.Krb5AcceptCredential) myCred).getKrb5EncryptionKeys();

                ullink.security.jgss.krb5.InitSecContextToken token = new ullink.security.jgss.krb5.InitSecContextToken(this,
                                                    secretKeys, is);
                ullink.security.krb5.PrincipalName clientName = token.getKrbApReq().getClient();
                peerName = ullink.security.jgss.krb5.Krb5NameElement.getInstance(clientName);
                if (getMutualAuthState()) {
                        retVal = new ullink.security.jgss.krb5.AcceptSecContextToken(this,
    ...
    public final int getWrapSizeLimit(int qop, boolean confReq,
        ...
        if (cipherHelper.getProto() == 0) {
            retVal = ullink.security.jgss.krb5.WrapToken.getSizeLimit(qop, confReq, maxTokSize,
                    getCipherHelper(null));
        } else if (cipherHelper.getProto() == 1) {
            retVal = ullink.security.jgss.krb5.WrapToken_v2.getSizeLimit(qop, confReq, maxTokSize,
                    getCipherHelper(null));
        }
        return retVal;
    }
    ...
    public final byte[] wrap(byte inBuf[], int offset, int len,
        ...
        try {
            if (cipherHelper.getProto() == 0) {
                ullink.security.jgss.krb5.WrapToken token =
                        new ullink.security.jgss.krb5.WrapToken(this, msgProp, inBuf, offset, len);
                encToken = token.encode();
            } else if (cipherHelper.getProto() == 1) {
                ullink.security.jgss.krb5.WrapToken_v2 token =
                        new ullink.security.jgss.krb5.WrapToken_v2(this, msgProp, inBuf, offset, len);
                encToken = token.encode();
            }
            if (DEBUG) {
    ...
    public final int wrap(byte inBuf[], int inOffset, int len,
        ...
        try {
            if (cipherHelper.getProto() == 0) {
                ullink.security.jgss.krb5.WrapToken token =
                        new ullink.security.jgss.krb5.WrapToken(this, msgProp, inBuf, inOffset, len);
                retVal = token.encode(outBuf, outOffset);
            } else if (cipherHelper.getProto() == 1) {
                ullink.security.jgss.krb5.WrapToken_v2 token =
                        new ullink.security.jgss.krb5.WrapToken_v2(this, msgProp, inBuf, inOffset, len);
                retVal = token.encode(outBuf, outOffset);
            }
            if (DEBUG) {
                System.out.println("Krb5Context.wrap: token=["
    ...
    public final byte[] unwrap(byte inBuf[], int offset, int len,
        ...
        throws GSSException {
            ...
            if (cipherHelper.getProto() == 0) {
                ullink.security.jgss.krb5.WrapToken token =
                        new ullink.security.jgss.krb5.WrapToken(this, inBuf, offset, len, msgProp);
                data = token.getData();
                setSequencingAndReplayProps(token, msgProp);
            } else if (cipherHelper.getProto() == 1) {
                ullink.security.jgss.krb5.WrapToken_v2 token =
                        new ullink.security.jgss.krb5.WrapToken_v2(this, inBuf, offset, len, msgProp);
                data = token.getData();
                setSequencingAndReplayProps(token, msgProp);
            }
    ...
    public final int unwrap(byte inBuf[], int inOffset, int len,
        ...
        if (cipherHelper.getProto() == 0) {
            ullink.security.jgss.krb5.WrapToken token =
                        new ullink.security.jgss.krb5.WrapToken(this, inBuf, inOffset, len, msgProp);
            len = token.getData(outBuf, outOffset);
            setSequencingAndReplayProps(token, msgProp);
        } else if (cipherHelper.getProto() == 1) {
            ullink.security.jgss.krb5.WrapToken_v2 token =
                        new ullink.security.jgss.krb5.WrapToken_v2(this, inBuf, inOffset, len, msgProp);
            len = token.getData(outBuf, outOffset);
            setSequencingAndReplayProps(token, msgProp);
        }
        return len;
    ...
    public final int unwrap(InputStream is,
        ...
        int len = 0;
        if (cipherHelper.getProto() == 0) {
            ullink.security.jgss.krb5.WrapToken token = new ullink.security.jgss.krb5.WrapToken(this, is, msgProp);
            len = token.getData(outBuf, outOffset);
            setSequencingAndReplayProps(token, msgProp);
        } else if (cipherHelper.getProto() == 1) {
            ullink.security.jgss.krb5.WrapToken_v2 token = new ullink.security.jgss.krb5.WrapToken_v2(this, is, msgProp);
            len = token.getData(outBuf, outOffset);
            setSequencingAndReplayProps(token, msgProp);
        }
        return len;
    ...
    public final byte[] getMIC(byte []inMsg, int offset, int len,
        ...
        throws GSSException {
            ...
            try {
                if (cipherHelper.getProto() == 0) {
                    ullink.security.jgss.krb5.MicToken token =
                        new ullink.security.jgss.krb5.MicToken(this, msgProp, inMsg, offset, len);
                    micToken = token.encode();
                } else if (cipherHelper.getProto() == 1) {
                    ullink.security.jgss.krb5.MicToken_v2 token =
                        new ullink.security.jgss.krb5.MicToken_v2(this, msgProp, inMsg, offset, len);
                    micToken = token.encode();
                }
                return micToken;
            } catch (IOException e) {
    ...
    public final void verifyMIC(byte []inTok, int tokOffset, int tokLen,
        ...
        if (cipherHelper.getProto() == 0) {
            ullink.security.jgss.krb5.MicToken token =
                new ullink.security.jgss.krb5.MicToken(this, inTok, tokOffset, tokLen, msgProp);
            token.verify(inMsg, msgOffset, msgLen);
            setSequencingAndReplayProps(token, msgProp);
        } else if (cipherHelper.getProto() == 1) {
            ullink.security.jgss.krb5.MicToken_v2 token =
                new ullink.security.jgss.krb5.MicToken_v2(this, inTok, tokOffset, tokLen, msgProp);
            token.verify(inMsg, msgOffset, msgLen);
            setSequencingAndReplayProps(token, msgProp);
        }
    }
    ...
    private void verifyMIC(InputStream is,
        ...
        if (cipherHelper.getProto() == 0) {
            ullink.security.jgss.krb5.MicToken token = new ullink.security.jgss.krb5.MicToken(this, is, msgProp);
            token.verify(inMsg, msgOffset, msgLen);
            setSequencingAndReplayProps(token, msgProp);
        } else if (cipherHelper.getProto() == 1) {
            ullink.security.jgss.krb5.MicToken_v2 token = new ullink.security.jgss.krb5.MicToken_v2(this, is, msgProp);
            token.verify(inMsg, msgOffset, msgLen);
            setSequencingAndReplayProps(token, msgProp);
        }
    }
    ...
    public final Provider getProvider() {
        return ullink.security.jgss.krb5.Krb5MechFactory.PROVIDER;
    }
     ...
     * form the peer.
     */
    private void setSequencingAndReplayProps(ullink.security.jgss.krb5.MessageToken token,
                                             MessageProp prop) {
     ...
     * form the peer.
     */
    private void setSequencingAndReplayProps(ullink.security.jgss.krb5.MessageToken_v2 token,
                                             MessageProp prop) {
        if (replayDetState || sequenceDetState) {
            int seqNum = token.getSequenceNumber();
            peerTokenTracker.getProps(seqNum, prop);
