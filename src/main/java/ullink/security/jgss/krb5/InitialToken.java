...
abstract class InitialToken extends Krb5Token {
    ...
    protected class OverloadedChecksum {
        ...
        private ullink.security.krb5.Credentials delegCreds = null;
        private int flags = 0;
         ...
         * InitSecContextToken.
         */
        public OverloadedChecksum(ullink.security.jgss.krb5.Krb5Context context,
            ...
            if (context.getCredDelegState()) {
                if (context.getCaller() == GSSUtil.CALLER_HTTP_NEGOTIATE &&
                    ...
                    ullink.security.krb5.KrbCred krbCred = null;
                    ullink.security.jgss.krb5.CipherHelper cipherHelper =
                        context.getCipherHelper(serviceTicket.getSessionKey());
                    if (useNullKey(cipherHelper)) {
                        krbCred = new ullink.security.krb5.KrbCred(tgt, serviceTicket,
                                                  ullink.security.krb5.EncryptionKey.NULL_KEY);
                    } else {
                        krbCred = new ullink.security.krb5.KrbCred(tgt, serviceTicket,
            ...
            if (context.getCredDelegState()) {
                ...
                ullink.security.krb5.PrincipalName delegateTo =
        ...
        // internal.
        public OverloadedChecksum(ullink.security.jgss.krb5.Krb5Context context,
                                  ullink.security.krb5.Checksum checksum, ullink.security.krb5.EncryptionKey key)
            throws GSSException, ullink.security.krb5.KrbException, IOException {
            ...
            if (checksum == null) {
                ...
                ge.initCause(new ullink.security.krb5.KrbException(Krb5.KRB_AP_ERR_INAPP_CKSUM));
                throw ge;
            }
            ...
            if ((flags & CHECKSUM_DELEG_FLAG) > 0) {
                ...
                ullink.security.jgss.krb5.CipherHelper cipherHelper = context.getCipherHelper(key);
                if (useNullKey(cipherHelper)) {
                    delegCreds =
                        new ullink.security.krb5.KrbCred(credBytes, ullink.security.krb5.EncryptionKey.NULL_KEY).
                        getDelegatedCreds()[0];
                } else {
                    delegCreds =
                        new ullink.security.krb5.KrbCred(credBytes, key).
                        getDelegatedCreds()[0];
                }
            }
        }
        ...
        private boolean useNullKey(ullink.security.jgss.krb5.CipherHelper ch) {
            boolean flag = true;
        ...
        public ullink.security.krb5.Checksum getChecksum() throws ullink.security.krb5.KrbException {
            return new ullink.security.krb5.Checksum(checksumBytes, CHECKSUM_TYPE);
        }
        ...
        public void setContextFlags(ullink.security.jgss.krb5.Krb5Context context) {
