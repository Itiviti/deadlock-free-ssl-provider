...
import ullink.security.krb5.internal.crypto.Aes128;
import ullink.security.krb5.internal.crypto.Aes256;
import ullink.security.krb5.internal.crypto.ArcFourHmac;
import ullink.security.krb5.internal.crypto.Des3;
...
class CipherHelper {
    ...
    CipherHelper(EncryptionKey key) throws GSSException {
        ...
        case EncryptedData.ETYPE_DES_CBC_MD5:
            sgnAlg = ullink.security.jgss.krb5.MessageToken.SGN_ALG_DES_MAC_MD5;
            sealAlg = ullink.security.jgss.krb5.MessageToken.SEAL_ALG_DES;
            break;

        case EncryptedData.ETYPE_DES3_CBC_HMAC_SHA1_KD:
            sgnAlg = ullink.security.jgss.krb5.MessageToken.SGN_ALG_HMAC_SHA1_DES3_KD;
            sealAlg = ullink.security.jgss.krb5.MessageToken.SEAL_ALG_DES3_KD;
            break;

        case EncryptedData.ETYPE_ARCFOUR_HMAC:
            sgnAlg = ullink.security.jgss.krb5.MessageToken.SGN_ALG_HMAC_MD5_ARCFOUR;
            sealAlg = ullink.security.jgss.krb5.MessageToken.SEAL_ALG_ARCFOUR_HMAC;
            break;
    ...
    byte[] calculateChecksum(int alg, byte[] header, byte[] trailer,
        ...
        switch (alg) {
        case ullink.security.jgss.krb5.MessageToken.SGN_ALG_DES_MAC_MD5:
            /*
        ...
        case ullink.security.jgss.krb5.MessageToken.SGN_ALG_DES_MAC:
            return getDesCbcChecksum(keybytes, header, data, start, len);

        case ullink.security.jgss.krb5.MessageToken.SGN_ALG_HMAC_SHA1_DES3_KD:
            byte[] buf;
            int offset, total;
        ...
        case ullink.security.jgss.krb5.MessageToken.SGN_ALG_HMAC_MD5_ARCFOUR:
            byte[] buffer;
            int off, tot;
            if (header == null && trailer == null) {
                buffer = data;
            ...
            try {
                ...
                */
                ...
                if (tokenId == ullink.security.jgss.krb5.Krb5Token.MIC_ID) {
    ...
    void decryptData(ullink.security.jgss.krb5.WrapToken token, byte[] ciphertext, int cStart, int cLen,
        ...
        */
        ...
        switch (sealAlg) {
        case ullink.security.jgss.krb5.MessageToken.SEAL_ALG_DES:
            desCbcDecrypt(token, getDesEncryptionKey(keybytes),
                ciphertext, cStart, cLen, plaintext, pStart);
            break;

        case ullink.security.jgss.krb5.MessageToken.SEAL_ALG_DES3_KD:
            des3KdDecrypt(token, ciphertext, cStart, cLen, plaintext, pStart);
            break;

        case ullink.security.jgss.krb5.MessageToken.SEAL_ALG_ARCFOUR_HMAC:
    ...
    void decryptData(ullink.security.jgss.krb5.WrapToken_v2 token, byte[] ciphertext, int cStart,
    ...
    void decryptData(ullink.security.jgss.krb5.WrapToken token, InputStream cipherStream, int cLen,
        ...
        case ullink.security.jgss.krb5.MessageToken.SEAL_ALG_DES3_KD:
            ...
            try {
                ullink.security.jgss.krb5.Krb5Token.readFully(cipherStream, ciphertext, 0, cLen);
            } catch (IOException e) {
        ...
        case ullink.security.jgss.krb5.MessageToken.SEAL_ALG_ARCFOUR_HMAC:
            ...
            try {
                ullink.security.jgss.krb5.Krb5Token.readFully(cipherStream, ctext, 0, cLen);
            } catch (IOException e) {
                GSSException ge = new GSSException(
                    GSSException.DEFECTIVE_TOKEN, -1,
                    "Cannot read complete token");
    ...
    void decryptData(ullink.security.jgss.krb5.WrapToken_v2 token, InputStream cipherStream, int cLen,
    ...
    void encryptData(ullink.security.jgss.krb5.WrapToken token, byte[] confounder, byte[] plaintext,
     ...
     */
    void encryptData(ullink.security.jgss.krb5.WrapToken_v2 token, byte[] confounder, byte[] tokenHeader,
     ...
     * HMAC is not encrypted; it is appended at the end.
     */
    int encryptData(ullink.security.jgss.krb5.WrapToken_v2 token, byte[] confounder, byte[] tokenHeader,
     ...
     * data
     */
    private void desCbcDecrypt(ullink.security.jgss.krb5.WrapToken token, byte[] key, byte[] cipherText,
        ...
        try {
             ...
             */
            temp = des.update(cipherText, offset, ullink.security.jgss.krb5.WrapToken.CONFOUNDER_SIZE,
                              token.confounder);
            // temp should be CONFOUNDER_SIZE
            // debug("\n\ttemp is " + temp + " and CONFOUNDER_SIZE is "
            //  + CONFOUNDER_SIZE);

            offset += ullink.security.jgss.krb5.WrapToken.CONFOUNDER_SIZE;
            len -= ullink.security.jgss.krb5.WrapToken.CONFOUNDER_SIZE;
            ...
            token.padding = ullink.security.jgss.krb5.WrapToken.pads[padSize];
            blockSize -= padSize;
     ...
     * data
     */
    private void desCbcDecrypt(ullink.security.jgss.krb5.WrapToken token, byte[] key,
    ...
    // ---- DES3-KD methods
    private void des3KdDecrypt(ullink.security.jgss.krb5.WrapToken token, byte[] ciphertext,
        ...
        */
        ...
        token.padding = ullink.security.jgss.krb5.WrapToken.pads[padSize];
        int len = ptext.length - ullink.security.jgss.krb5.WrapToken.CONFOUNDER_SIZE - padSize;

        System.arraycopy(ptext, ullink.security.jgss.krb5.WrapToken.CONFOUNDER_SIZE,
            plaintext, pStart, len);

        // Needed to calculate checksum
        System.arraycopy(ptext, 0, token.confounder,
            0, ullink.security.jgss.krb5.WrapToken.CONFOUNDER_SIZE);
    }
    ...
    // ---- RC4-HMAC methods
    private void arcFourDecrypt(ullink.security.jgss.krb5.WrapToken token, byte[] ciphertext,
        int cStart, int cLen, byte[] plaintext, int pStart)
        throws GSSException {
    ...
    private byte[] arcFourEncrypt(ullink.security.jgss.krb5.WrapToken token, byte[] confounder,
    ...
    private void aes128Decrypt(ullink.security.jgss.krb5.WrapToken_v2 token, byte[] ciphertext,
        ...
        */
        ...
        int len = ptext.length - ullink.security.jgss.krb5.WrapToken_v2.CONFOUNDER_SIZE -
                        ullink.security.jgss.krb5.WrapToken_v2.TOKEN_HEADER_SIZE;
        System.arraycopy(ptext, ullink.security.jgss.krb5.WrapToken_v2.CONFOUNDER_SIZE,
                                plaintext, pStart, len);
    ...
    private void aes256Decrypt(ullink.security.jgss.krb5.WrapToken_v2 token, byte[] ciphertext,
        int cStart, int cLen, byte[] plaintext, int pStart, int key_usage)
        throws GSSException {
