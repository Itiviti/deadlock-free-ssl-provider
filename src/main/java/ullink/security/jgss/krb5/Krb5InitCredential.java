...
public class Krb5InitCredential
    ...
    private ullink.security.jgss.krb5.Krb5NameElement name;
    private ullink.security.krb5.Credentials krb5Credentials;

    private Krb5InitCredential(ullink.security.jgss.krb5.Krb5NameElement name,
                               byte[] asn1Encoding,
        ...
        try {
            ...
            krb5Credentials = new ullink.security.krb5.Credentials(asn1Encoding,
                                              client.getName(),
                                              server.getName(),
                                              sessionKey,
                                              keyType,
    ...
    private Krb5InitCredential(ullink.security.jgss.krb5.Krb5NameElement name,
                               ullink.security.krb5.Credentials delegatedCred,
                               byte[] asn1Encoding,
                               KerberosPrincipal client,
                               KerberosPrincipal server,
                               byte[] sessionKey,
    ...
    static Krb5InitCredential getInstance(int caller, ullink.security.jgss.krb5.Krb5NameElement name,
                                   int initLifetime)
        throws GSSException {
        ...
        if (name == null) {
            ...
            name = ullink.security.jgss.krb5.Krb5NameElement.getInstance(fullName,
                    ullink.security.jgss.krb5.Krb5MechFactory.NT_GSS_KRB5_PRINCIPAL);
        }
    ...
    static Krb5InitCredential getInstance(ullink.security.jgss.krb5.Krb5NameElement name,
                                   ullink.security.krb5.Credentials delegatedCred)
        throws GSSException {

        ullink.security.krb5.EncryptionKey sessionKey = delegatedCred.getSessionKey();
        ...
        ullink.security.krb5.PrincipalName cPrinc = delegatedCred.getClient();
        ullink.security.krb5.PrincipalName sPrinc = delegatedCred.getServer();

        KerberosPrincipal client = null;
        KerberosPrincipal server = null;

        ullink.security.jgss.krb5.Krb5NameElement credName = null;

        if (cPrinc != null) {
            String fullName = cPrinc.getName();
            credName = ullink.security.jgss.krb5.Krb5NameElement.getInstance(fullName,
                    ullink.security.jgss.krb5.Krb5MechFactory.NT_GSS_KRB5_PRINCIPAL);
    ...
    /**
     * Returns a Credentials instance so that it maybe
     * used in that package for th Kerberos protocol.
     */
    ullink.security.krb5.Credentials getKrb5Credentials() {
    ...
    private static KerberosTicket getTgt(int caller, ullink.security.jgss.krb5.Krb5NameElement name,
                                                 int initLifetime)
        throws GSSException {
        ...
        if (name != null) {
            ...
            try {
                ullink.security.krb5.Config config = ullink.security.krb5.Config.getInstance();
