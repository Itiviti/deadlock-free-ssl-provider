...
public class GSSCredElement implements GSSCredentialSpi {
    ...
    private ullink.security.jgss.wrapper.GSSNameElement name = null;
    private ullink.security.jgss.wrapper.GSSLibStub cStub;
        ...
        if (GSSUtil.isKerberosMech(cStub.getMech())) {
            if (System.getSecurityManager() != null) {
                ...
                if (isAcceptorCredential() &&
                    name != ullink.security.jgss.wrapper.GSSNameElement.DEF_ACCEPTOR) {
    ...
    GSSCredElement(long pCredentials, ullink.security.jgss.wrapper.GSSNameElement srcName, Oid mech)
        throws GSSException {
        pCred = pCredentials;
        cStub = ullink.security.jgss.wrapper.GSSLibStub.getInstance(mech);
        usage = GSSCredential.INITIATE_ONLY;
        name = srcName;
    }

    GSSCredElement(ullink.security.jgss.wrapper.GSSNameElement name, int lifetime, int usage,
                   ullink.security.jgss.wrapper.GSSLibStub stub) throws GSSException {
        cStub = stub;
        this.usage = usage;
        ...
        if (name != null) { // Could be GSSNameElement.DEF_ACCEPTOR
            ...
            this.name = new ullink.security.jgss.wrapper.GSSNameElement(cStub.getCredName(pCred), cStub);
            doServicePermCheck();
        }
    }
    ...
    public ullink.security.jgss.wrapper.GSSNameElement getName() throws GSSException {
        return (name == ullink.security.jgss.wrapper.GSSNameElement.DEF_ACCEPTOR ?
            null : name);
    }
