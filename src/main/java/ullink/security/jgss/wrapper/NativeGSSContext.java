...
package ullink.security.jgss.wrapper;

import org.ietf.jgss.*;
import ullink.security.jgss.GSSExceptionImpl;
import ullink.security.jgss.GSSHeader;
...
import ullink.security.jgss.spnego.NegTokenInit;
import ullink.security.jgss.spnego.NegTokenTarg;
...
class NativeGSSContext implements GSSContextSpi {
    ...
    private ullink.security.jgss.wrapper.GSSNameElement srcName;
    private ullink.security.jgss.wrapper.GSSNameElement targetName;
    private ullink.security.jgss.wrapper.GSSCredElement cred;
    private boolean isInitiator;
    private boolean isEstablished;
    private Oid actualMech; // Assigned during context establishment

    private ChannelBinding cb;
    private ullink.security.jgss.wrapper.GSSCredElement delegatedCred;
    private int flags;
    private int lifetime = GSSCredential.DEFAULT_LIFETIME;
    private final ullink.security.jgss.wrapper.GSSLibStub cStub;
        ...
        if (System.getSecurityManager() != null) {
            ...
            if (GSSUtil.isSpNegoMech(cStub.getMech()) && isInitiator
                ...
                if (srcName == null) {
                    ...
                    ullink.security.jgss.wrapper.GSSCredElement tempCred =
                        new ullink.security.jgss.wrapper.GSSCredElement(null, lifetime,
                                           GSSCredential.INITIATE_ONLY,
                                           ullink.security.jgss.wrapper.GSSLibStub.getInstance(GSSUtil.GSS_KRB5_MECH_OID));
                    tempCred.dispose();
                } else {
    ...
    private void doDelegPermCheck() throws GSSException {
        ...
        if (sm != null) {
            ...
            ullink.security.jgss.wrapper.SunNativeProvider.debug("Checking DelegationPermission (" +
                    krbPrincPair + ")");
    ...
    private byte[] retrieveToken(InputStream is, int mechTokenLen)
        ...
        try {
            ...
            if (mechTokenLen != -1) {
                ...
                ullink.security.jgss.wrapper.SunNativeProvider.debug("Precomputed mechToken length: " +
                        mechTokenLen);
            ...
            ullink.security.jgss.wrapper.SunNativeProvider.debug("Complete Token length: " +
                    result.length);
            return result;
        } catch (IOException ioe) {
            throw new GSSExceptionImpl(GSSException.FAILURE, ioe);
    ...
    NativeGSSContext(ullink.security.jgss.wrapper.GSSNameElement peer, ullink.security.jgss.wrapper.GSSCredElement myCred,
                     int time, ullink.security.jgss.wrapper.GSSLibStub stub) throws GSSException {
        if (peer == null) {
        ...
        if (GSSUtil.isKerberosMech(cStub.getMech())) {
            doServicePermCheck();
            if (cred == null) {
                cred = new ullink.security.jgss.wrapper.GSSCredElement(null, lifetime,
    ...
    NativeGSSContext(ullink.security.jgss.wrapper.GSSCredElement myCred, ullink.security.jgss.wrapper.GSSLibStub stub)
        throws GSSException {
        cStub = stub;
        cred = myCred;
    ...
    NativeGSSContext(long pCtxt, ullink.security.jgss.wrapper.GSSLibStub stub) throws GSSException {
        assert(pContext != 0);
        pContext = pCtxt;
        cStub = stub;
        ...
        srcName = new ullink.security.jgss.wrapper.GSSNameElement(info[0], cStub);
        targetName = new ullink.security.jgss.wrapper.GSSNameElement(info[1], cStub);
    ...
    public Provider getProvider() {
        return ullink.security.jgss.wrapper.SunNativeProvider.INSTANCE;
    }
    ...
    public byte[] initSecContext(InputStream is, int mechTokenLen)
        ...
        if ((!isEstablished) && (isInitiator)) {
            ...
            if (pContext != 0) {
                ...
                ullink.security.jgss.wrapper.SunNativeProvider.debug("initSecContext=> inToken len=" +
                        inToken.length);
            }
            ...
            outToken = cStub.initContext(pCred, targetName.pName,
                                         cb, inToken, this);
            ullink.security.jgss.wrapper.SunNativeProvider.debug("initSecContext=> outToken len=" +
            ...
            if (isEstablished) {
                if (srcName == null) {
                    srcName = new ullink.security.jgss.wrapper.GSSNameElement
                        (cStub.getContextName(pContext, true), cStub);
                }
                if (cred == null) {
                    cred = new ullink.security.jgss.wrapper.GSSCredElement(srcName, lifetime,
                                              GSSCredential.INITIATE_ONLY,
                                              cStub);
                }
            }
    ...
    public byte[] acceptSecContext(InputStream is, int mechTokenLen)
        ...
        if ((!isEstablished) && (!isInitiator)) {
            ...
            ullink.security.jgss.wrapper.SunNativeProvider.debug("acceptSecContext=> inToken len=" +
                    inToken.length);
            long pCred = (cred == null? 0 : cred.pCred);
            outToken = cStub.acceptContext(pCred, cb, inToken, this);
            ullink.security.jgss.wrapper.SunNativeProvider.debug("acceptSecContext=> outToken len=" +
                    (outToken == null ? 0 : outToken.length));

            if (targetName == null) {
                targetName = new ullink.security.jgss.wrapper.GSSNameElement
                ...
                cred = new ullink.security.jgss.wrapper.GSSCredElement(targetName, lifetime,
