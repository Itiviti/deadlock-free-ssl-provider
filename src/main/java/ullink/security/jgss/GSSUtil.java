...
import ullink.security.jgss.spnego.SpNegoCredElement;
...
public class GSSUtil {
    ...
    public static Subject getSubject(GSSName name,
        ...
        if (creds instanceof ullink.security.jgss.GSSCredentialImpl) {
            gssCredentials = ((ullink.security.jgss.GSSCredentialImpl) creds).getElements();
     ...
     * to Krb5InitCredential which extends
    ...
    public static Subject login(int caller, Oid mech) throws LoginException {
        ...
        LoginContext lc = new LoginContext("", null, cb,
                new ullink.security.jgss.LoginConfigImpl(caller, mech));
        lc.login();
                ...
                (new PrivilegedExceptionAction<Vector>() {
                    public Vector run() throws Exception {
                        ...
                        if (accSubj != null) {
                            ...
                            Iterator<ullink.security.jgss.GSSCredentialImpl> iterator =
                                accSubj.getPrivateCredentials
                                (ullink.security.jgss.GSSCredentialImpl.class).iterator();
                            while (iterator.hasNext()) {
                                ullink.security.jgss.GSSCredentialImpl cred = iterator.next();
