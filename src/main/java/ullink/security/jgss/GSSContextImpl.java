...
class GSSContextImpl implements GSSContext {
    ...
    private ullink.security.jgss.GSSManagerImpl gssManager = null;
    ...
    private ullink.security.jgss.GSSCredentialImpl myCred = null;
    private ullink.security.jgss.GSSCredentialImpl delegCred = null;

    private ullink.security.jgss.GSSNameImpl srcName = null;
    private ullink.security.jgss.GSSNameImpl targName = null;
     ...
     */
    public GSSContextImpl(ullink.security.jgss.GSSManagerImpl gssManager, GSSName peer, Oid mech,
                          GSSCredential myCred, int lifetime)
        throws GSSException {
        if ((peer == null) || !(peer instanceof ullink.security.jgss.GSSNameImpl)) {
            throw new GSSException(GSSException.BAD_NAME);
        }
        if (mech == null) mech = ProviderList.DEFAULT_MECH_OID;

        this.gssManager = gssManager;
        this.myCred = (ullink.security.jgss.GSSCredentialImpl) myCred;  // XXX Check first
        reqLifetime = lifetime;
        targName = (ullink.security.jgss.GSSNameImpl)peer;
        this.mechOid = mech;
        initiator = true;
    }
     ...
     */
    public GSSContextImpl(ullink.security.jgss.GSSManagerImpl gssManager, GSSCredential myCred)
        throws GSSException {
        this.gssManager = gssManager;
        this.myCred = (ullink.security.jgss.GSSCredentialImpl) myCred; // XXX Check first
        initiator = false;
     ...
     * @see #isTransferable
     */
    public GSSContextImpl(ullink.security.jgss.GSSManagerImpl gssManager, byte[] interProcessToken)
        throws GSSException {
        this.gssManager = gssManager;
    ...
    public int initSecContext(InputStream inStream,
        ...
        if (mechCtxt != null && currentState != IN_PROGRESS) {
            throw new ullink.security.jgss.GSSExceptionImpl(GSSException.FAILURE,
                                   "Illegal call to initSecContext");
        }

        ullink.security.jgss.GSSHeader gssHeader = null;
        int inTokenLen = -1;
        ...
        try {
            if (mechCtxt == null) {
                if (myCred != null) {
                    try {
                        ...
                        if (ullink.security.jgss.GSSUtil.isSpNegoMech(mechOid) &&
                ...
                if (mechCtxt.getProvider().getName().equals("SunNativeGSS") ||
                    ullink.security.jgss.GSSUtil.isSpNegoMech(mechOid)) {
                    // do not parse GSS header for native provider or SPNEGO
                    // mech
                } else {
                    // parse GSS header
                    gssHeader = new ullink.security.jgss.GSSHeader(inStream);
                    if (!gssHeader.getOid().equals((Object) objId))
                        throw new ullink.security.jgss.GSSExceptionImpl
            ...
            if (obuf != null) {
                retVal = obuf.length;
                if (mechCtxt.getProvider().getName().equals("SunNativeGSS") ||
                    (!firstToken && ullink.security.jgss.GSSUtil.isSpNegoMech(mechOid))) {
                    // do not add GSS header for native provider or SPNEGO
                    // except for the first SPNEGO token
                } else {
                    // add GSS header
                    gssHeader = new ullink.security.jgss.GSSHeader(objId, obuf.length);
            ...
            throw new ullink.security.jgss.GSSExceptionImpl(GSSException.DEFECTIVE_TOKEN,
                                   e.getMessage());
        }
    }
    ...
    public int getWrapSizeLimit(int qop, boolean confReq,
        ...
        if (mechCtxt != null)
            ...
            throw new ullink.security.jgss.GSSExceptionImpl(GSSException.NO_CONTEXT,
    ...
    public GSSName getSrcName() throws GSSException {
        if (srcName == null) {
            srcName = ullink.security.jgss.GSSNameImpl.wrapElement
    ...
    public GSSName getTargName() throws GSSException {
        if (targName == null) {
            targName = ullink.security.jgss.GSSNameImpl.wrapElement
    ...
    public GSSCredential getDelegCred() throws GSSException {
        ...
        return (delCredElement == null ?
            null : new ullink.security.jgss.GSSCredentialImpl(gssManager, delCredElement));
    }
