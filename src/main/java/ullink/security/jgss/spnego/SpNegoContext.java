...
package ullink.security.jgss.spnego;

import org.ietf.jgss.*;
import ullink.security.action.GetBooleanAction;
import ullink.security.jgss.GSSCredentialImpl;
import ullink.security.jgss.GSSNameImpl;
...
import ullink.security.util.BitArray;
...
public class SpNegoContext implements GSSContextSpi {
    ...
    private ullink.security.jgss.spnego.SpNegoCredElement myCred = null;
    ...
    public SpNegoContext(SpNegoMechFactory factory, GSSNameSpi peerName,
        ...
        if ((myCred != null) && !(myCred instanceof ullink.security.jgss.spnego.SpNegoCredElement)) {
            throw new IllegalArgumentException("Wrong cred element type");
        }
        this.peerName = peerName;
        this.myCred = (ullink.security.jgss.spnego.SpNegoCredElement) myCred;
    ...
    public final byte[] initSecContext(InputStream is, int mechTokenSize)
        ...
        byte[] retVal = null;
        ullink.security.jgss.spnego.NegTokenInit initToken = null;
        byte[] mechToken = null;
        ...
        try {
            if (state == STATE_NEW) {
                ...
                initToken = new ullink.security.jgss.spnego.NegTokenInit(DER_mechTypes, getContextFlags(),
                                        mechToken, micToken);
                if (DEBUG) {
                    System.out.println("SpNegoContext.initSecContext: " +
                                "sending token of type = " +
                                ullink.security.jgss.spnego.SpNegoToken.getTokenName(initToken.getType()));
                }
                ...
                ullink.security.jgss.spnego.SpNegoToken.readFully(is, server_token);
                if (DEBUG) {
                    System.out.println("SpNegoContext.initSecContext: " +
                                        "process received token = " +
                                        ullink.security.jgss.spnego.SpNegoToken.getHexBytes(server_token));
                }

                // read the SPNEGO token
                // token will be validated when parsing
                ullink.security.jgss.spnego.NegTokenTarg targToken = new ullink.security.jgss.spnego.NegTokenTarg(server_token);

                if (DEBUG) {
                    System.out.println("SpNegoContext.initSecContext: " +
                                "received token of type = " +
                                ullink.security.jgss.spnego.SpNegoToken.getTokenName(targToken.getType()));
                }
                ...
                ullink.security.jgss.spnego.SpNegoToken.NegoResult negoResult = null;
                int result = targToken.getNegotiatedResult();
                switch (result) {
                    case 0:
                        negoResult = ullink.security.jgss.spnego.SpNegoToken.NegoResult.ACCEPT_COMPLETE;
                        state = STATE_DONE;
                        break;
                    case 1:
                        negoResult = ullink.security.jgss.spnego.SpNegoToken.NegoResult.ACCEPT_INCOMPLETE;
                        state = STATE_IN_PROCESS;
                        break;
                    case 2:
                        negoResult = ullink.security.jgss.spnego.SpNegoToken.NegoResult.REJECT;
                        state = STATE_DELETED;
                        break;
                    default:
                        state = STATE_DONE;
                ...
                if (negoResult == ullink.security.jgss.spnego.SpNegoToken.NegoResult.REJECT) {
                    throw new GSSException(errorCode, -1,
                ...
                if ((negoResult == ullink.security.jgss.spnego.SpNegoToken.NegoResult.ACCEPT_COMPLETE) ||
                    (negoResult == ullink.security.jgss.spnego.SpNegoToken.NegoResult.ACCEPT_INCOMPLETE)) {
                    ...
                    if (isMechContextEstablished()) {
                        ...
                        // generate SPNEGO token
                        initToken = new ullink.security.jgss.spnego.NegTokenInit(null, null,
                                                mechToken, null);
                        if (DEBUG) {
                            System.out.println("SpNegoContext.initSecContext:" +
            ...
            if (DEBUG) {
                if (retVal != null) {
                    System.out.println("SNegoContext.initSecContext: " +
                        "sending token = " + ullink.security.jgss.spnego.SpNegoToken.getHexBytes(retVal));
                }
            }
    ...
    public final byte[] acceptSecContext(InputStream is, int mechTokenSize)
        ...
        byte[] retVal = null;
        ullink.security.jgss.spnego.SpNegoToken.NegoResult negoResult;
        boolean valid = true;
        ...
        try {
            if (state == STATE_NEW) {
                ...
                ullink.security.jgss.spnego.SpNegoToken.readFully(is, token);
                if (DEBUG) {
                    System.out.println("SpNegoContext.acceptSecContext: " +
                                        "receiving token = " +
                                        ullink.security.jgss.spnego.SpNegoToken.getHexBytes(token));
                }

                // read the SPNEGO token
                // token will be validated when parsing
                ullink.security.jgss.spnego.NegTokenInit initToken = new ullink.security.jgss.spnego.NegTokenInit(token);
                ...
                // generate SPNEGO token
                ullink.security.jgss.spnego.NegTokenTarg targToken = new ullink.security.jgss.spnego.NegTokenTarg(negoResult.ordinal(),
                ...
                ullink.security.jgss.spnego.SpNegoToken.readFully(is, client_token);
        ...
        try {
            ...
            if (DEBUG) {
                System.out.println("SpNegoContext: getMIC = " +
                                        ullink.security.jgss.spnego.SpNegoToken.getHexBytes(mic));
            }
        } catch (GSSException e) {
            mic = null;
            if (DEBUG) {
    ...
    public final GSSCredentialSpi getDelegCred() throws GSSException {
        ...
        if (mechContext != null) {
            ...
            ullink.security.jgss.spnego.SpNegoCredElement cred = new ullink.security.jgss.spnego.SpNegoCredElement(mechCred);
