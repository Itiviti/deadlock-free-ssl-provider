...
public class NegTokenInit extends SpNegoToken {
    ...
    private ullink.security.util.BitArray reqFlags = null;
    private byte[] mechToken = null;
    private byte[] mechListMIC = null;

    NegTokenInit(byte[] mechTypes, ullink.security.util.BitArray flags,
    ...
    final byte[] encode() throws GSSException {
        try {
            // create negInitToken
            ullink.security.util.DerOutputStream initToken = new ullink.security.util.DerOutputStream();

            // DER-encoded mechTypes with CONTEXT 00
            if (mechTypes != null) {
                initToken.write(ullink.security.util.DerValue.createTag(ullink.security.util.DerValue.TAG_CONTEXT,
                        true, (byte) 0x00), mechTypes);
            }

            // write context flags with CONTEXT 01
            if (reqFlags != null) {
                ullink.security.util.DerOutputStream flags = new ullink.security.util.DerOutputStream();
            ...
            if (mechToken != null) {
                ullink.security.util.DerOutputStream dataValue = new ullink.security.util.DerOutputStream();
            ...
            if (mechListMIC != null) {
                ...
                ullink.security.util.DerOutputStream mic = new ullink.security.util.DerOutputStream();
            ...
            out.write(ullink.security.util.DerValue.tag_Sequence, initToken);
    ...
    private void parseToken(byte[] in) throws GSSException {
        try {
            ullink.security.util.DerValue der = new ullink.security.util.DerValue(in);
            ...
            ullink.security.util.DerValue tmp1 = der.data.getDerValue();
            if (tmp1.tag != ullink.security.util.DerValue.tag_Sequence) {
                throw new IOException("SPNEGO NegoTokenInit : " +
            ...
            if (tmp1.data.available() > 0) {
                ullink.security.util.DerValue tmp2 = tmp1.data.getDerValue();
                ...
                ullink.security.util.DerInputStream mValue = tmp2.data;
                mechTypes = mValue.toByteArray();

                // read all the mechTypes
                ullink.security.util.DerValue[] mList = mValue.getSequence(0);
                mechTypeList = new Oid[mList.length];
                ullink.security.util.ObjectIdentifier mech = null;
            ...
            if (tmp1.data.available() > 0) {
                ullink.security.util.DerValue tmp3 = tmp1.data.getDerValue();
            ...
            if (!GSSUtil.useMSInterop() && (tmp1.data.available() > 0)) {
                ...
                ullink.security.util.DerValue tmp6 = tmp1.data.getDerValue();
    ...
    ullink.security.util.BitArray getReqFlags() {
        return reqFlags;
    }
